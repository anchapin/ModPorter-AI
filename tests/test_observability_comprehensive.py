...
class TracingCollector:
    ...
    def log_event(self, span: Dict[str, Any], event: str, fields: Dict = None):
        """Log event in span."""
        span["logs"].append({"timestamp": time.time(), "event": event, "fields": fields or {}})
...
class TestAlerting:
    ...
    def test_multiple_alert_rules(self, alert_manager):
        ...
        metrics = {"p99_latency_ms": 600, "uptime_percent": 99.0, "queue_depth": 50}
        alert_manager.evaluate_rules(metrics)
...
class TestObservabilityIntegration:
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_observability(
        self, metrics_collector, tracing_collector, structured_logger, alert_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_observability_with_errors(
        self, metrics_collector, structured_logger, alert_manager
    ):
...
class TestMetricsPercentiles:
    ...
    def test_metric_cardinality(self, metrics_collector):
        ...
        for user_id in range(100):
            metrics_collector.increment_counter("user_requests", 1, {"user_id": str(user_id)})
