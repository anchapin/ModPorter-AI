...
class Button(Component):
    ...
    def __init__(self, label: str, on_click=None):
        super().__init__("button", {"label": label, "onClick": on_click})
        self.click_count = 0
...
class Modal(Component):
    ...
    def render(self) -> str:
        ...
        if not self.is_open:
            return ""
        return (
            f"<div class='modal'><h2>{self.props['title']}</h2><p>{self.props['content']}</p></div>"
        )
...
class TestUserInteractions:
    ...
    def test_button_click_callback(self):
        ...
        button = Button("Submit", on_click=on_click)
        button.handle_click()
...
class TestVisualRegression:
    ...
    def test_regression_detection_multiple_components(self, visual_regression_detector):
        ...
        visual_regression_detector.add_current(
            "button", Screenshot("button", "hash2_new")
        )  # Changed
