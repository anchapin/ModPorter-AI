...
class FileProcessor:
    ...
    def validate_upload(self, file: UploadFile) -> ValidationResult:
        ...
        if file.size > self.MAX_FILE_SIZE:
            ...
            logger.warning(msg)
            return ValidationResult(is_valid=False, message=msg, sanitized_filename=sanitized_filename)
        ...
        try:
            ...
            if magic_bytes == self.ZIP_MAGIC_NUMBER:
                ...
                logger.info(f"Magic bytes match ZIP/JAR for uploaded file. Content-type: {file.content_type}. Determined type: {determined_file_type}")
            else:
            ...
            logger.info(f"Uploaded file {sanitized_filename} validated successfully as type: {determined_file_type}.")
    ...
    async def validate_downloaded_file(self, file_path: Path, original_url: str) -> ValidationResult:
        """
        ...
        logger.info(f"Starting validation for downloaded file: {sanitized_filename} from URL: {original_url}")
        ...
        try:
            ...
            if magic_bytes == self.ZIP_MAGIC_NUMBER:
                ...
                if file_extension == ".jar":
                    ...
                    logger.warning(f"File {sanitized_filename} has ZIP magic numbers but unexpected extension '{file_extension}'. Treating as 'zip'.")
                    determined_file_type = "zip"
                logger.info(f"Magic bytes match ZIP/JAR for downloaded file {sanitized_filename}. Extension: {file_extension}. Determined type: {determined_file_type}")
            else:
            ...
            logger.info(f"Downloaded file {sanitized_filename} validated successfully as type: {determined_file_type}.")
            return ValidationResult(
                is_valid=True,
    ...
    async def scan_for_malware(self, file_path: Path, file_type: str) -> ScanResult:
                ...
                with zipfile.ZipFile(file_path, 'r') as archive:
                    ...
                    for member in archive.infolist():
                        ...
                        if "../" in member.filename or member.filename.startswith("/"):
                            ...
                            logger.warning(msg)
                            return ScanResult(is_safe=False, message=msg, details={"filename": member.filename})
                        ...
                        abs_member_path = Path(os.path.abspath(os.path.join(HYPOTHETICAL_TARGET_EXTRACTION_DIR, member.filename)))
                        if not str(abs_member_path).startswith(str(os.path.abspath(HYPOTHETICAL_TARGET_EXTRACTION_DIR))):
                        ...
                        if member.file_size > 0 and member.compress_size > 0: # Avoid division by zero
                            ratio = member.file_size / member.compress_size
                            if ratio > MAX_COMPRESSION_RATIO and member.file_size > MAX_UNCOMPRESSED_FILE_SIZE:
                                msg = (
                                ...
                                logger.warning(msg)
                                return ScanResult(is_safe=False, message=msg, details={"filename": member.filename, "ratio": ratio, "size": member.file_size})
                    ...
                    if num_files > MAX_TOTAL_FILES:
                        ...
                        logger.warning(msg)
                        return ScanResult(is_safe=False, message=msg, details={"num_files": num_files})

                    logger.info(f"Archive {file_path} passed ZIP bomb and path traversal checks. Total files: {num_files}, Total uncompressed size: {total_uncompressed_size}")
        ...
        logger.info(f"Placeholder for external malware scan (e.g., ClamAV) for file: {file_path}. Integration would occur here.")
        ...
        logger.info(f"File {file_path} passed all implemented security checks (basic archive checks; no external scan performed).")
        return ScanResult(is_safe=True, message="File passed implemented security checks.")

    async def extract_mod_files(self, archive_path: Path, job_id: str, file_type: str) -> ExtractionResult:
        """
        Extracts files from the archive, performs path traversal checks, and looks for manifest files.
        """
        logger.info(f"Starting extraction for archive: {archive_path} (job_id: {job_id}, type: {file_type})")
        ...
        if file_type not in ["zip", "jar"]:
            return ExtractionResult(success=False, message=f"Unsupported file type for extraction: {file_type}")
            ...
            with zipfile.ZipFile(archive_path, 'r') as zip_ref:
                ...
                for member in archive_members:
                    ...
                    if member.filename.startswith('/') or '..' in member.filename:
                        logger.warning(f"Skipping potentially unsafe path in archive: {member.filename} for job_id: {job_id}")
                    ...
                    if not str(target_path).startswith(str(extraction_dir.resolve())):
                        logger.warning(f"Skipping file that would extract outside target directory: {member.filename} for job_id: {job_id}")
                        continue
                ...
                logger.info(f"Extracted {extracted_files_count} files to {extraction_dir} for job_id: {job_id}")
        ...
        for manifest_name in manifest_files_priority:
            ...
            if potential_manifest_path.is_file():
                ...
                try:
                    with open(potential_manifest_path, 'rb') as f: # Open in binary for tomllib
                        if manifest_name.endswith(".json"):
                            ...
                            try:
                                ...
                                logger.warning(f"Could not parse {manifest_name} as JSON for job_id: {job_id}. It might be a different format or malformed.")
                                # Add custom parsing for .info if needed, or treat as plain text
                                manifest_data = {"raw_content": open(potential_manifest_path, 'r').read()}
                                found_manifest_type = "mcmod.info (raw)"

                        if manifest_data:
                            logger.info(f"Successfully parsed {manifest_name} as {found_manifest_type} for job_id: {job_id}")
                            break # Found and parsed, stop searching
                except Exception as e:
                    logger.error(f"Error parsing manifest file {manifest_name} for job_id {job_id}: {e}", exc_info=True)
                 ...
                 logger.debug(f"Manifest file {manifest_name} not found at {potential_manifest_path} for job_id: {job_id}")
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if not Path(sanitized_filename).suffix:
                    ...
                    elif "java-archive" in content_type or "jar" in content_type : # application/java-archive or application/x-jar
                         sanitized_filename += ".jar"
                    else: # Default or unknown
                        logger.warning(f"Could not determine extension for {url} from Content-Type: {content_type}. Defaulting to no specific extension or .bin if needed.")
                ...
                if file_size == 0:
                    logger.warning(f"Downloaded file {downloaded_file_path} is empty for URL: {url}, job_id: {job_id}")
                    # Optionally return success=False or a specific message

                logger.info(f"Successfully downloaded {sanitized_filename} ({file_size} bytes) to {downloaded_file_path} for job_id: {job_id}")
    ...
    def cleanup_temp_files(self, job_id: str) -> bool:
        ...
        logger.info(f"Attempting to cleanup temporary directory: {temp_job_path} for job_id: {job_id}")

        if not temp_job_path.exists():
            logger.info(f"Temporary directory {temp_job_path} not found for job_id: {job_id}. No cleanup needed.")
            return True

        try:
            shutil.rmtree(temp_job_path)
            logger.info(f"Successfully cleaned up temporary directory: {temp_job_path} for job_id: {job_id}")
            return True
        except PermissionError:
            logger.error(f"Permission error while trying to delete {temp_job_path} for job_id: {job_id}. Manual cleanup may be required.", exc_info=True)
            return False
        except Exception as e:
            logger.error(f"An unexpected error occurred while deleting {temp_job_path} for job_id: {job_id}: {e}", exc_info=True)
            return False
