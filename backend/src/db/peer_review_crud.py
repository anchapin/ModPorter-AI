...
class PeerReviewCRUD:
    ...
    async def get_by_contribution(
        db: AsyncSession, contribution_id: str
    ) -> List[PeerReviewModel]:
        """Get all reviews for a contribution."""
        try:
            query = (
                select(PeerReviewModel)
                .where(PeerReviewModel.contribution_id == contribution_id)
    ...
    async def get_by_reviewer(
        db: AsyncSession, reviewer_id: str, status: Optional[str] = None
    ) -> List[PeerReviewModel]:
        """Get reviews by reviewer."""
        try:
            query = select(PeerReviewModel).where(
                PeerReviewModel.reviewer_id == reviewer_id
            )
            if status:
    ...
    async def update_status(
        db: AsyncSession, review_id: str, status: str, review_data: Dict[str, Any]
    ) -> bool:
        """Update review status and data."""
        try:
            query = (
                update(PeerReviewModel)
                .where(PeerReviewModel.id == review_id)
                .values(status=status, **review_data, updated_at=datetime.utcnow())
            )
    ...
    async def get_pending_reviews(
        ...
        try:
            query = (
                ...
                .order_by(PeerReviewModel.created_at)
                .limit(limit)
            )
            result = await db.execute(query)
            return result.scalars().all()
...
class ReviewWorkflowCRUD:
    ...
    async def update_stage(
        db: AsyncSession, workflow_id: str, stage: str, history_entry: Dict[str, Any]
    ) -> bool:
        """Update workflow stage."""
        try:
            # First get current workflow to update stage history
            current_query = select(ReviewWorkflowModel).where(
            ...
            # Update workflow
            query = (
                update(ReviewWorkflowModel)
                .where(ReviewWorkflowModel.id == workflow_id)
                .values(
                    current_stage=stage,
    ...
    async def get_active_workflows(
        ...
        try:
            query = (
                select(ReviewWorkflowModel)
                .where(ReviewWorkflowModel.status == "active")
                .order_by(ReviewWorkflowModel.created_at)
                .limit(limit)
            )
            result = await db.execute(query)
            return result.scalars().all()
...
class ReviewerExpertiseCRUD:
    ...
    async def create_or_update(
        db: AsyncSession, reviewer_id: str, expertise_data: Dict[str, Any]
    ) -> Optional[ReviewerExpertiseModel]:
        ...
        try:
            ...
            if reviewer:
                ...
                reviewer = ReviewerExpertiseModel(
                    reviewer_id=reviewer_id, **expertise_data
                )
                db.add(reviewer)
    ...
    @staticmethod
    async def get_by_id(
        db: AsyncSession, reviewer_id: str
    ) -> Optional[ReviewerExpertiseModel]:
    ...
    async def find_available_reviewers(
        db: AsyncSession, expertise_area: str, version: str, limit: int = 10
    ) -> List[ReviewerExpertiseModel]:
        """Find available reviewers with specific expertise."""
        try:
            query = (
                select(ReviewerExpertiseModel)
                .where(
                    and_(
                        ReviewerExpertiseModel.is_active_reviewer,
                        ReviewerExpertiseModel.current_reviews
                        < ReviewerExpertiseModel.max_concurrent_reviews,
                        ReviewerExpertiseModel.expertise_areas.contains(
                            [expertise_area]
                        ),
    ...
    @staticmethod
    async def update_review_metrics(
        db: AsyncSession, reviewer_id: str, metrics: Dict[str, Any]
    ) -> bool:
        """Update reviewer performance metrics."""
        try:
            query = (
                update(ReviewerExpertiseModel)
                .where(ReviewerExpertiseModel.reviewer_id == reviewer_id)
                .values(
                    **metrics,
                    updated_at=datetime.utcnow(),
            ...
            query = (
                ...
                .values(
                    current_reviews=func.greatest(
                        ReviewerExpertiseModel.current_reviews - 1, 0
                    ),
                    updated_at=datetime.utcnow(),
                )
            )
...
class ReviewTemplateCRUD:
    ...
    async def get_by_type(
        db: AsyncSession, template_type: str, contribution_type: Optional[str] = None
    ) -> List[ReviewTemplateModel]:
    ...
    async def increment_usage(db: AsyncSession, template_id: str) -> bool:
        ...
        try:
            query = (
                update(ReviewTemplateModel)
                .where(ReviewTemplateModel.id == template_id)
                .values(
...
class ReviewAnalyticsCRUD:
    ...
    async def create_daily_analytics(
        db: AsyncSession, analytics_date: date, data: Dict[str, Any]
    ) -> Optional[ReviewAnalyticsModel]:
        """Create daily analytics entry."""
        try:
            db_analytics = ReviewAnalyticsModel(date=analytics_date, **data)
            db.add(db_analytics)
            await db.commit()
    ...
    async def get_or_create_daily(
        db: AsyncSession, analytics_date: date
    ...
    async def update_daily_metrics(
        db: AsyncSession, analytics_date: date, metrics: Dict[str, Any]
    ) -> bool:
        """Update daily analytics metrics."""
        try:
            # Get or create
            analytics = await ReviewAnalyticsCRUD.get_or_create_daily(
                db, analytics_date
            )
    ...
    async def get_date_range(
        ...
        try:
            query = (
                ...
                .where(ReviewAnalyticsModel.date.between(start_date, end_date))
    ...
    async def get_review_summary(db: AsyncSession, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            analytics_list = await ReviewAnalyticsCRUD.get_date_range(
            ...
            total_needing_revision = sum(
                a.contributions_needing_revision for a in analytics_list
            )
            ...
            avg_review_scores = [
                a.avg_review_score for a in analytics_list if a.avg_review_score
            ]
            ...
            return {
                ...
                "avg_review_time": sum(avg_review_times) / len(avg_review_times)
                if avg_review_times
                else 0.0,
                "avg_review_score": sum(avg_review_scores) / len(avg_review_scores)
                if avg_review_scores
                else 0.0,
                "active_reviewers": sum(a.active_reviewers for a in analytics_list)
                // len(analytics_list)
                if analytics_list
                else 0,
            }
