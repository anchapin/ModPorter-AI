...
class Neo4jPerformanceConfig:
    ...
    def __post_init__(self):
        ...
        self.max_connection_pool_size = int(os.getenv("NEO4J_MAX_POOL_SIZE", self.max_connection_pool_size))
        self.connection_acquisition_timeout = int(os.getenv("NEO4J_ACQUISITION_TIMEOUT", self.connection_acquisition_timeout))
        self.max_connection_lifetime = int(os.getenv("NEO4J_MAX_LIFETIME", self.max_connection_lifetime))
        self.connection_idle_timeout = int(os.getenv("NEO4J_IDLE_TIMEOUT", self.connection_idle_timeout))
...
class Neo4jQueryBuilder:
    ...
    @staticmethod
    def with_index_hints(query: str, node_labels: list[str] = None, properties: list[str] = None) -> str:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    ...
                    logger.warning(f"Neo4j operation failed (attempt {attempt + 1}/{self.max_retries + 1}), retrying in {delay}s: {e}")
                    time.sleep(delay)
                else:
...
class Neo4jConnectionManager:
    ...
    def __init__(self, config: Neo4jPerformanceConfig):
        ...
        logger.info(f"Initialized Neo4j connection manager with {len(self.endpoints.replicas) + 1} endpoints")
    ...
    def get_primary_uri(self) -> str:
        ...
        for replica in self.endpoints.replicas:
            if self._is_healthy(replica):
                logger.warning(f"Primary endpoint unavailable, failing over to replica: {replica}")
                return replica
