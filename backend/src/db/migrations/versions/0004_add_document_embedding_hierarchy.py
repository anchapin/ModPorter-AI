...
revision: str = "0004"
down_revision: str | None = "0003_add_behavior_templates"
...
def upgrade() -> None:
    ...
    op.add_column("document_embeddings", sa.Column("chunk_index", sa.Integer(), nullable=True))
    op.add_column(
        "document_embeddings",
        sa.Column("hierarchy_level", sa.Integer(), nullable=False, server_default="2"),
    )
    op.add_column("document_embeddings", sa.Column("title", sa.String(), nullable=True))
    op.add_column("document_embeddings", sa.Column("metadata", postgresql.JSONB(), nullable=True))

    # Create indexes for new fields
    op.create_index(
        "ix_document_embeddings_parent_document_id", "document_embeddings", ["parent_document_id"]
    )
    op.create_index("ix_document_embeddings_chunk_index", "document_embeddings", ["chunk_index"])
    op.create_index("ix_document_embeddings_title", "document_embeddings", ["title"])
