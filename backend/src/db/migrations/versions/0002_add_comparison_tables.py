...
def upgrade():
    ...
    sa.Column('assumptions_applied', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    sa.Column('confidence_scores', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=True), # Added timezone=True
    sa.ForeignKeyConstraint(['conversion_id'], ['conversion_jobs.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('feature_mappings',
    sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
