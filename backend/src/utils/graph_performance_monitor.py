...
class GraphPerformanceMonitor:
    ...
    def __init__(self, 
        ...
        self.operation_metrics: Dict[str, deque] = defaultdict(lambda: deque(maxlen=window_size))
    ...
    def _check_thresholds(self, metric: PerformanceMetric):
        ...
        if metric.duration > thresholds.max_duration:
            alerts.append(f"Duration {metric.duration:.3f}s exceeds threshold {thresholds.max_duration:.3f}s")
        
        # Check memory delta
        if metric.memory_delta > thresholds.max_memory_delta:
            alerts.append(f"Memory delta {metric.memory_delta:.1f}MB exceeds threshold {thresholds.max_memory_delta:.1f}MB")
        
        # Check success rate
        if self.failure_counts[metric.operation] >= thresholds.alert_after_failures:
            alerts.append(f"Operation failed {self.failure_counts[metric.operation]} times consecutively")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if (current_time - self._stats_cache_time < self._stats_cache_ttl and 
            self._stats_cache):
    ...
    def get_recent_metrics(self, operation: Optional[str] = None, count: int = 100) -> List[Dict[str, Any]]:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if scope["type"] == "http" and "/api/knowledge-graph" in scope["path"]:
            ...
            if "nodes" in path_parts:
                operation = "node_creation" if scope["method"] == "POST" else "node_query"
            elif "relationships" in path_parts:
                operation = "relationship_creation" if scope["method"] == "POST" else "relationship_query"
