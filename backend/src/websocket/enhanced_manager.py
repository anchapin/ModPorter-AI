...
class RateLimiter:
    ...
    def is_allowed(self, client_id: str) -> bool:
        ...
        self._message_times[client_id] = [
            t for t in self._message_times[client_id] if now - t < 60.0
        ]
...
class EnhancedConnectionManager:
    ...
    async def connect(
        self, websocket: WebSocket, conversion_id: str, client_id: Optional[str] = None
    ) -> ConnectionInfo:
        """
        ...
        await self._send_message(
            websocket,
            {
                ...
                "data": {"client_id": info.client_id, "conversion_id": conversion_id},
            },
        )
    ...
    async def handle_message(
        self, websocket: WebSocket, conversion_id: str, message: Dict[str, Any]
        ...
        if not self._rate_limiter.is_allowed(info.client_id):
            ...
            return {
                ...
                "data": {"error": "Rate limit exceeded", "retry_after": wait_time},
            }
    ...
    async def broadcast(
        self, message: Dict[str, Any], conversion_id: str, throttle: bool = True
    ) -> int:
        """
    ...
    async def send_personal_message(
        self, message: Dict[str, Any], websocket: WebSocket, conversion_id: str
    ) -> bool:
        """
        ...
        try:
            ...
            logger.error("Failed to send personal message: %s", str(e), exc_info=True)
            return False
    ...
    async def _heartbeat_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.error("Error in heartbeat loop: %s", str(e), exc_info=True)
