"""
Fabric documentation source adapter.

Fetches documentation from the official Fabric wiki.
"""
...
class FabricDocsAdapter(BaseSourceAdapter):
    """
    Adapter for fetching Minecraft Fabric documentation.

    Base URL: https://fabricmc.net/wiki
    """
    ...
    async def fetch(self, config: Dict[str, Any]) -> List[RawDocument]:
        """
        Fetch Fabric documentation.

        Args:
            config: Configuration dict
                - version: Fabric/Yarn version (default: "1.20.1")
        ...
        sections = config.get("sections", ["intro", "blocks", "items"])
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                for section in sections:
                    try:
                        ...
                        url = f"{self.BASE_URL}/{section}:intro"

                        logger.info(f"Fetching Fabric docs from: {url}")
            ...
            logger.error(f"Failed to fetch Fabric documentation: {e}")

        return documents

    def validate_config(self, config: Dict[str, Any]) -> bool:
        """
        Validate Fabric documentation configuration.
