...
class GraphVersionControlService:
    ...
    async def create_commit(
        ...
        try:
            ...
            return {"success": False, "error": f"Commit creation failed: {str(e)}"}
    ...
    async def merge_branch(
        ...
        try:
            ...
            merge_change = GraphChange(
                ...
                metadata={"merge_commit": True, "source_commits": len(source_commits)},
            )
    ...
    async def generate_diff(
        ...
        try:
            ...
            self.commits[base_hash]
            self.commits[target_hash]

            # Create diff object
            diff = GraphDiff(base_hash=base_hash, target_hash=target_hash)
                ...
                if change.change_type == ChangeType.CREATE:
                    ...
                    if change.item_type == ItemType.NODE:
                        diff.deleted_nodes.append(
                            {"item_id": change.item_id, "data": change.previous_data}
                        )
                    elif change.item_type == ItemType.RELATIONSHIP:
            ...
            return GraphDiff(
                base_hash=base_hash, target_hash=target_hash, metadata={"error": str(e)}
            )
    ...
    async def create_tag(
        ...
        try:
            ...
            return {"success": False, "error": f"Tag creation failed: {str(e)}"}
    ...
    async def revert_commit(
        ...
        try:
            ...
            return {"success": False, "error": f"Commit revert failed: {str(e)}"}

    async def get_branch_status(
        self, branch_name: str, db: AsyncSession = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            recent_changes = [
                change
                for change in self.changes[-100:]
            ...
            return {
                ...
                "recent_activity": len(
                    [
                        ...
                        if (datetime.utcnow() - c.timestamp).total_seconds() < 86400
                    ]
                ),
                ...
                "last_activity": max(
                    (c.timestamp for c in recent_changes), default=branch.created_at
                ).isoformat(),
            }

        except Exception as e:
            logger.error(f"Error getting branch status: {e}")
            return {"success": False, "error": f"Failed to get branch status: {str(e)}"}
    ...
    async def _calculate_tree_hash(
        self, changes: List[GraphChange], db: AsyncSession
    ) -> str:
    ...
    async def _update_graph_from_commit(self, commit: GraphCommit, db: AsyncSession):
            ...
            for change in commit.changes:
                if change.item_type == ItemType.NODE:
                    if change.change_type == ChangeType.CREATE:
                        ...
                        await KnowledgeNodeCRUD.update(
                            db, change.item_id, change.new_data
                        )
                    elif change.change_type == ChangeType.DELETE:
    ...
    async def _get_commits_since_base(
        self, source_hash: str, base_hash: str
    ) -> List[GraphCommit]:
    ...
    async def _detect_merge_conflicts(
        self, source_commits: List[GraphCommit], target_hash: str, db: AsyncSession
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if target_commit:
                target_changes = {
                    ...
                    for change in target_commit.changes
    ...
    async def _auto_resolve_conflict(
        self, conflict: Dict[str, Any], merge_strategy: str
    ...
    async def _get_changes_between_commits(
        self, base_hash: str, target_hash: str
    ) -> List[GraphChange]:
    ...
    async def _get_ahead_behind(
        self, branch_name: str, target_branch: str
    ) -> Dict[str, int]:
