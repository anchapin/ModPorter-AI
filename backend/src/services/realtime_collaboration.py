...
class RealtimeCollaborationService:
    ...
    async def leave_collaboration_session(
        ...
        try:
            ...
            if not session_id or session_id not in self.active_sessions:
                return {
                    "success": False,
                    "error": "User not in active session"
                }
            ...
            return {
                "success": False,
                "error": f"Failed to leave session: {str(e)}"
            }
    ...
    async def apply_operation(
        ...
        try:
            ...
            if target_id and operation_type in [OperationType.UPDATE_NODE, OperationType.UPDATE_RELATIONSHIP, OperationType.UPDATE_PATTERN]:
    ...
    async def get_user_activity(
        ...
        try:
            ...
            return {
                ...
                ).total_seconds() < 300  # Active if last activity < 5 minutes ago
            }
        ...
        try:
            ...
            if message_type == "cursor_position":
                ...
                return await self.resolve_conflict(
                    session_id, user_id, conflict_id, resolution_strategy, resolution_data, db
                )
    ...
    async def handle_websocket_disconnect(
        ...
        try:
            ...
            return {
                "success": True,
                "message": "WebSocket disconnection handled"
            }
            ...
            return {
                "success": False,
                "error": f"Disconnect handling failed: {str(e)}"
            }
    ...
    async def _get_current_data(
        ...
        try:
            ...
            elif operation_type in [OperationType.UPDATE_RELATIONSHIP, OperationType.DELETE_RELATIONSHIP]:
                # This would get relationship data
                pass
            
            elif operation_type in [OperationType.UPDATE_PATTERN, OperationType.DELETE_PATTERN]:
        ...
        try:
            ...
            for pending_id, pending_op in session.pending_changes.items():
                ...
                if operation.target_id == pending_op.target_id:
                    if operation.operation_type in [OperationType.UPDATE_NODE, OperationType.UPDATE_RELATIONSHIP, OperationType.UPDATE_PATTERN]:
                        if pending_op.operation_type in [OperationType.UPDATE_NODE, OperationType.UPDATE_RELATIONSHIP, OperationType.UPDATE_PATTERN]:
                            conflicts.append({
                    ...
                    elif operation.operation_type in [OperationType.DELETE_NODE, OperationType.DELETE_RELATIONSHIP, OperationType.DELETE_PATTERN]:
                        if pending_op.operation_type in [OperationType.UPDATE_NODE, OperationType.UPDATE_RELATIONSHIP, OperationType.UPDATE_PATTERN]:
                            conflicts.append({
            ...
            if operation.operation_type == OperationType.CREATE_RELATIONSHIP:
                ...
                for recent_op in session.operations[-10:]:
                    if recent_op.operation_type == OperationType.CREATE_RELATIONSHIP:
                        ...
                        if (recent_data.get("source_id") == source_id and 
                            recent_data.get("target_id") == target_id):
                            conflicts.append({
    ...
    async def _execute_operation(
        ...
        try:
            ...
            if operation.operation_type == OperationType.CREATE_NODE:
                ...
                node_data = operation.data
                success = await KnowledgeNodeCRUD.update(db, operation.target_id, node_data)
                result = {
                    "type": "node_updated",
