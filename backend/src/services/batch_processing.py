...
class BatchProcessingService:
    ...
    def __init__(self):
        ...
        self.default_processing_mode = ProcessingMode.SEQUENTIAL
    ...
    async def get_job_status(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            with self.lock:
                if job_id not in self.active_jobs and job_id not in [
                    j.job_id for j in self.job_history
                ]:
    ...
    async def cancel_job(
        ...
        try:
            ...
            return {"success": False, "error": f"Job cancellation failed: {str(e)}"}

    async def pause_job(
        self, job_id: str, reason: str = "User requested pause"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {"success": False, "error": f"Job pause failed: {str(e)}"}
    ...
    async def resume_job(self, job_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Job resume failed: {str(e)}"}
    ...
    async def get_active_jobs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to get active jobs: {str(e)}"}

    async def get_job_history(
        self, limit: int = 50, operation_type: Optional[BatchOperationType] = None
    ) -> Dict[str, Any]:
        ...
        try:
            with self.lock:
                ...
                if operation_type:
                    history = [
                        job for job in history if job.operation_type == operation_type
                    ]
                ...
                return {
                    ...
                    "filter_operation_type": operation_type.value
                    if operation_type
                    else None,
                    "limit_applied": limit,
                    "timestamp": datetime.utcnow().isoformat(),
            ...
            return {"success": False, "error": f"Failed to get job history: {str(e)}"}
                ...
                while not self.stop_processing:
                    try:
                        with self.lock:
                            ...
                            if (
                                self.job_queue
                                and len(self.active_jobs) < self.max_concurrent_jobs
                            ):
        ...
        try:
            ...
            with self.lock:
                if job_id in self.active_jobs:
                    job = self.active_jobs[job_id]
                    job.status = (
                        BatchStatus.COMPLETED if result.success else BatchStatus.FAILED
                    )
                    job.completed_at = datetime.utcnow()
    ...
    async def _process_nodes_chunk(
        self, nodes_chunk: List[Dict[str, Any]], db: AsyncSession
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for node_data in nodes_chunk:
                try:
                    ...
                    failed += 1
                    errors.append(
                        f"Failed to create node {node_data.get('id', 'unknown')}: {str(e)}"
                    )
    ...
    async def _estimate_total_items(
        ...
        try:
            if operation_type in [
                ...
                BatchOperationType.DELETE_RELATIONSHIPS,
            ]:
                # For deletes, count matching items
                if db:
                    if operation_type == BatchOperationType.DELETE_NODES:
                        filters = parameters.get("filters", {})
                        await KnowledgeNodeCRUD.search(db, "", limit=1, **filters)
    ...
    async def _estimate_duration(
        self, operation_type: BatchOperationType, total_items: int
    ) -> float:
...
batch_processor = batch_processing_service
