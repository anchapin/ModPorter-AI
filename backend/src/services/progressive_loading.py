...
class ProgressiveLoadingService:
        ...
        try:
            ...
            if viewport:
                ...
                viewport_info.visible_bounds = {
                    "min_x": viewport_info.center_x
                    - (viewport_info.width / 2) * zoom_factor,
                    "max_x": viewport_info.center_x
                    + (viewport_info.width / 2) * zoom_factor,
                    "min_y": viewport_info.center_y
                    - (viewport_info.height / 2) * zoom_factor,
                    "max_y": viewport_info.center_y
                    + (viewport_info.height / 2) * zoom_factor,
                }
            ...
            task = LoadingTask(
                ...
                chunk_size=min(
                    parameters.get("chunk_size", self.default_chunk_size),
                    self.max_items_per_chunk,
                ),
            ...
            task.total_chunks = max(
                1, (total_items + task.chunk_size - 1) // task.chunk_size
            )
            ...
            with self.lock:
                ...
                if viewport_info:
                    ...
                    if len(self.viewport_history[visualization_id]) > 10:
                        self.viewport_history[visualization_id] = self.viewport_history[
                            visualization_id
                        ][-10:]
    ...
    async def get_loading_progress(self, task_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            with self.lock:
                if task_id not in self.active_tasks:
                    return {"success": False, "error": "Loading task not found"}
                ...
                if task.started_at and task.loaded_items > 0:
                    ...
                    if elapsed_time > 0:
                        ...
                        estimated_remaining = (
                            remaining_items / loading_rate if loading_rate > 0 else 0
                        )
                ...
                if viewport:
                    ...
                    if viz_id:
                        ...
                        if len(self.viewport_history[viz_id]) > 10:
                            self.viewport_history[viz_id] = self.viewport_history[
                                viz_id
                            ][-10]
    ...
    async def preload_adjacent_areas(
        ...
        try:
            ...
            task = LoadingTask(
                ...
                metadata={"visualization_id": visualization_id, "cache_id": cache_id},
            )
            ...
            # Execute preloading
            asyncio.create_task(
                self._execute_preloading_task(preload_task_id, cache_id, db)
            )
    ...
    async def get_loading_statistics(
        self, visualization_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
            ...
            with self.lock:
                    ...
                    if not visualization_id or viz_id == visualization_id:
                        cache_stats[viz_id] = {
                            ...
                            "last_accessed_seconds": (
                                datetime.utcnow() - cache.last_accessed
                            ).total_seconds(),
                        }
                ...
                return {
                    ...
                    "statistics": {
                        ...
                        "viewport_history": {
                            "total_viewports": sum(
                                len(vph) for vph in self.viewport_history.values()
                            ),
    ...
    def _start_background_loading(self):
        ...
        try:
            ...
            self.background_thread = threading.Thread(
                target=background_loading_task, daemon=True
            )
    ...
    async def _execute_loading_task(self, task_id: str, db: AsyncSession):
        ...
        try:
            ...
            if task.loading_strategy == LoadingStrategy.LOD_BASED:
                ...
                result = {
                    "success": False,
                    "error": f"Unsupported loading strategy: {task.loading_strategy.value}",
                }
    ...
    async def _execute_lod_loading(
        self, task: LoadingTask, db: AsyncSession
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": f"LOD loading failed: {str(e)}"}

    async def _execute_distance_based_loading(
        self, task: LoadingTask, db: AsyncSession
    ) -> Dict[str, Any]:
    ...
    async def _execute_importance_based_loading(
        self, task: LoadingTask, db: AsyncSession
    ) -> Dict[str, Any]:
    ...
    async def _execute_cluster_based_loading(
        self, task: LoadingTask, db: AsyncSession
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for chunk_index in range(task.total_chunks):
                ...
                chunk_data = await self._load_cluster_chunk(
                    chunk_index, task.chunk_size, task.detail_level, task.metadata, db
                )
    ...
    async def _estimate_total_items(
        ...
        try:
            ...
            if viewport:
                ...
                viewport_factor = min(
                    1.0, viewport_area / (1920 * 1080)
                )  # Normalize to Full HD
    ...
    def _optimize_loading_parameters(self):
        ...
        try:
            ...
            if self.total_loads > 0:
                logger.debug(
                    f"Loading performance: {self.average_load_time:.2f}ms average over {self.total_loads} loads"
                )
