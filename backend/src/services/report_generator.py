...
MOCK_CONVERSION_RESULT_SUCCESS = {
    ...
    "converted_mods_data": [
        {"name": "AwesomeMod", "version": "1.2.0", "status": "Converted", "warnings": ["Minor texture issue on BlockX"], "errors": []},
        {"name": "UtilityHelpers", "version": "2.1.0", "status": "Converted", "warnings": [], "errors": []}
    ],
    "failed_mods_data": [
        {"name": "SuperRender", "version": "1.0.0", "status": "Failed", "reason": "Incompatible rendering engine hooks"}
    ],
    ...
    "features_data": [
        {"feature_name": "BlockAwesome", "status": "Success", "compatibility_notes": "Fully compatible.", "impact_of_assumption": None},
        {"feature_name": "EntityHelper", "status": "Partial Success", "compatibility_notes": "Some methods deprecated.", "impact_of_assumption": "Minor performance impact due to workaround for deprecated methods."},
        {"feature_name": "MagicSpellX", "status": "Success", "compatibility_notes": "Particle effect adapted.", "impact_of_assumption": "Visuals of spell changed due to particle system substitution (see smart assumptions)."}
    ],
...
MOCK_CONVERSION_RESULT_FAILURE = {
    ...
    "converted_mods_data": [],
    "failed_mods_data": [
        {"name": "CoreMod", "version": "3.0.0", "status": "Failed", "reason": "Critical dependency 'XYZ' could not be resolved or translated."}
    ],
    "smart_assumptions_data": [],
    "features_data": [
        {"feature_name": "BasicBlock", "status": "Failed", "compatibility_notes": "Depends on CoreMod.", "impact_of_assumption": None}
    ],
    "assumptions_detail_data": [],
    "developer_logs_data": {
        "code_translation": [],
        "api_mapping": [],
        "file_processing": [],
        "performance": {"total_time_seconds": 15.7, "memory_peak_mb": 64, "cpu_usage_avg_percentage": 15.0},
        "errors": [{"error_message": "NullPointerException in JavaAnalyzer", "stack_trace": "Trace...", "module": "JavaAnalyzer"}]
    }
}
...
class ConversionReportGenerator:
    ...
    def generate_assumptions_report(self, assumptions_detail_data: List[Dict[str, Any]]) -> AssumptionsReport:
        assumption_details: List[AssumptionDetail] = []
        for ad in assumptions_detail_data:
            detail = AssumptionDetail(
                assumption_id=ad.get("assumption_id", f"SA_{time.time()}"), # Generate an ID if missing
    ...
    def generate_developer_log(self, dev_logs_data: Dict[str, Any]) -> DeveloperLog:
        return DeveloperLog(
            code_translation_details=[LogEntry(**log) for log in dev_logs_data.get("code_translation", [])],
            api_mapping_issues=[LogEntry(**log) for log in dev_logs_data.get("api_mapping", [])],
            file_processing_log=[LogEntry(**log) for log in dev_logs_data.get("file_processing", [])],
    ...
    def _map_smart_assumptions_prd(self, smart_assumptions_data: List[Dict[str, Any]]) -> List[SmartAssumption]:
    ...
    def create_interactive_report(self, conversion_result: Dict[str, Any], job_id: str) -> InteractiveReport:
        ...
        converted_mods_list = self._map_mod_statuses(conversion_result.get("converted_mods_data", []))
        ...
        assumptions_report_obj = self.generate_assumptions_report(conversion_result.get("assumptions_detail_data", []))
        ...
        feature_analysis_obj = self.generate_feature_analysis(conversion_result.get("features_data", []))
    ...
    def create_full_conversion_report_prd_style(self, conversion_result: Dict[str, Any]) -> FullConversionReport:
        ...
        smart_assumptions_list_for_prd = self._map_smart_assumptions_prd(conversion_result.get("smart_assumptions_data", []))
...
if __name__ == "__main__":
    ...
    interactive_report_success = generator.create_interactive_report(MOCK_CONVERSION_RESULT_SUCCESS, "job_123_success")
    # import json
    ...
    prd_style_report_success = generator.create_full_conversion_report_prd_style(MOCK_CONVERSION_RESULT_SUCCESS)
    # print(json.dumps(prd_style_report_success, indent=2))


    print("\n--- Generating Failure Report (Interactive Style) ---")
    interactive_report_failure = generator.create_interactive_report(MOCK_CONVERSION_RESULT_FAILURE, "job_456_failure")
    # print(json.dumps(interactive_report_failure, indent=2))

    print("\n--- Generating Failure Report (PRD JSON Style) ---")
    prd_style_report_failure = generator.create_full_conversion_report_prd_style(MOCK_CONVERSION_RESULT_FAILURE)
    ...
    feature_analysis_data = generator.generate_feature_analysis(MOCK_CONVERSION_RESULT_SUCCESS["features_data"])
    ...
    assumptions_report_data = generator.generate_assumptions_report(MOCK_CONVERSION_RESULT_SUCCESS["assumptions_detail_data"])
    if assumptions_report_data["assumptions"]:
        print(f"First assumption impact: {assumptions_report_data['assumptions'][0]['impact_level']}")

    dev_log_data = generator.generate_developer_log(MOCK_CONVERSION_RESULT_SUCCESS["developer_logs_data"])
    if dev_log_data["code_translation_details"]:
        print(f"First code translation log: {dev_log_data['code_translation_details'][0]['message']}")
