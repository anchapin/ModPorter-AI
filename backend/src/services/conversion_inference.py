...
from src.db.knowledge_graph_crud import KnowledgeNodeCRUD, ConversionPatternCRUD
...
class ConversionInferenceEngine:
    ...
    async def infer_conversion_path(
        ...
        try:
            ...
            optimize_for = options.get(
                "optimize_for", "confidence"
            )  # confidence, speed, features
            ...
            if direct_paths and optimize_for != "features":
                ...
                return {
                    ...
                    "alternative_paths": direct_paths[1:]
            ...
            for concept in java_concepts:
                ...
                if result.get("success"):
                    concept_paths[concept] = {
                        ...
                        "confidence": result.get("primary_path", {}).get(
                            "confidence", 0.0
                        ),
                    }
                else:
    ...
    async def get_inference_statistics(
        self, days: int = 30, db: AsyncSession = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            stats = {
                ...
                "path_types": {"direct": 1234, "indirect": 613},
                "confidence_distribution": {"high": 890, "medium": 678, "low": 279},
                "learning_events": 892,
                "performance_trends": {
    ...
    async def _find_concept_node(
        self, db: AsyncSession, concept: str, platform: str, version: str
        ...
        try:
            ...
            for node in nodes:
                if (
                    (
                        concept.lower() in node.name.lower()
                        or node.name.lower() in concept.lower()
                    )
                    and (platform in node.platform or node.platform == "both")
                    and (
                        version == node.minecraft_version
                        or node.minecraft_version == "latest"
                    )
                ):
                    return node
        ...
        try:
            ...
            for path in neo4j_paths:
                if path["path_length"] == 1:
                    ...
                    if (
                        target_platform in target_node.get("platform", "")
                        or target_node.get("platform") == "both"
                    ):
                        direct_path = {
                            "path_type": "direct",
                    ...
                    if (
                        ...
                        indirect_path = {
                            ...
                            "intermediate_concepts": [
                                step["target_concept"] for step in steps[:-1]
                            ],
                        }
    ...
    async def _suggest_similar_concepts(
        self, db: AsyncSession, target_concept: str, platform: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _analyze_batch_paths(
        self, concept_paths: Dict[str, Dict], db: AsyncSession
    ) -> Dict[str, Any]:
        """Analyze patterns across batch of inferred paths."""
        try:
            path_lengths = [
                len(p["primary_path"]["steps"])
                for p in concept_paths.values()
                if p["primary_path"]
            ]
    ...
    async def _optimize_processing_order(
        self, concept_paths: Dict[str, Dict], path_analysis: Dict
    ) -> List[str]:
    ...
    async def _identify_shared_steps(
        ...
        try:
            ...
            # Find most common
            most_common_rel = (
                max(relationship_counts.items(), key=lambda x: x[1])
                if relationship_counts
                else None
            )
            most_common_target = (
                max(target_counts.items(), key=lambda x: x[1])
                if target_counts
                else None
            )
        ...
        try:
            ...
            for i in range(0, len(processing_order), batch_size):
                ...
                group = {
                    ...
                    "estimated_time": self._estimate_batch_time(
                        batch_concepts, concept_paths
                    ),
                    "shared_patterns": [
                        step for step in shared_steps if step["count"] > 1
                    ],
                    "optimizations": await self._get_batch_optimizations(
                        batch_concepts, concept_paths
                    ),
                }
                groups.append(group)
    ...
    async def _find_common_patterns(
        self, concept_paths: Dict[str, Dict]
    ) -> List[Dict[str, Any]]:
    ...
    def _estimate_batch_time(
        self, concepts: List[str], concept_paths: Dict[str, Dict]
    ) -> float:
        ...
        total_confidence = sum(
            concept_paths.get(c, {}).get("confidence", 0.0) for c in concepts
        )
        ...
        for target, count in target_counts.items():
            if count > 1:
                optimizations.append(
                    f"Multiple concepts convert to {target} - can optimize shared logic"
                )

        return optimizations

    async def _build_dependency_graph(
        self, concepts: List[str], dependencies: Dict[str, List[str]], db: AsyncSession
    ) -> Dict[str, List[str]]:
    ...
    async def _find_shared_patterns_for_group(
        self, concepts: List[str], db: AsyncSession
    ) -> List[Dict[str, Any]]:
    ...
    async def _adjust_confidence_thresholds(
        self, performance: Dict, success_metrics: Dict
    ) -> Dict[str, float]:
    ...
    async def enhance_conversion_accuracy(
        ...
        try:
            ...
            for path in conversion_paths:
                ...
                pattern_score = await self._validate_conversion_pattern(path, db)
                ...
                ml_score = await self._refine_with_ml_predictions(path, context_data)

                # 4. Community Wisdom Integration
                community_score = await self._integrate_community_wisdom(path, db)
            ...
            return {
                ...
                "accuracy_improvements": {
                    "original_avg_confidence": sum(
                        p.get("confidence", 0) for p in conversion_paths
                    )
                    / len(conversion_paths),
                    "enhanced_avg_confidence": sum(
                        p.get("enhanced_accuracy", 0) for p in ranked_paths
                    )
                    / len(ranked_paths),
                    "improvement_percentage": self._calculate_improvement_percentage(
                        conversion_paths, ranked_paths
                    ),
                },
            ...
            return {"success": False, "error": f"Accuracy enhancement failed: {str(e)}"}
        ...
        try:
            ...
            if db:
                ...
                if successful_patterns:
                    ...
                    return min(
                        1.0, avg_success_rate * 1.2
                    )  # Boost for validated patterns
                else:
    ...
    async def _generate_accuracy_suggestions(
        ...
        if components.get("pattern_validation", 0) < 0.7:
            suggestions.append(
                "Review pattern validation against known successful conversions"
            )
    ...
    def _calculate_improvement_percentage(
        ...
        original_avg = sum(p.get("confidence", 0) for p in original_paths) / len(
            original_paths
        )
        enhanced_avg = sum(p.get("enhanced_accuracy", 0) for p in enhanced_paths) / len(
            enhanced_paths
        )
