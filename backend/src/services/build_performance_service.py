...
class BuildPerformanceService:
    ...
    def start_tracking(
        ...
        logger.info(
            f"Started performance tracking for build {build.build_id}, conversion {request.conversion_id}"
        )
    ...
    def start_stage(
        self, build_id: str, stage_name: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Optional[BuildPerformanceMetrics]:
        """
    ...
    def end_tracking(
        ...
        with self._lock:
            ...
            if build.stages:
                ...
                if first_stage.start_time and last_stage.end_time:
                    build.total_duration_ms = (
                        last_stage.end_time - first_stage.start_time
                    ).total_seconds() * 1000
            ...
            logger.info(
                f"Ended performance tracking for build {build_id}, status: {request.status}, duration: {build.total_duration_ms}ms"
            )
    ...
    def get_snapshot(self, build_id: str) -> Optional[BuildPerformanceSnapshot]:
        ...
        if build.stages and build.stages[0].start_time:
            elapsed_ms = (
                datetime.now(timezone.utc) - build.stages[0].start_time
            ).total_seconds() * 1000
