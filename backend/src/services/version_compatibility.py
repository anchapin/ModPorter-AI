...
from src.db.knowledge_graph_crud import VersionCompatibilityCRUD, ConversionPatternCRUD
...
class VersionCompatibilityService:
    ...
    async def get_compatibility(
        self, java_version: str, bedrock_version: str, db: AsyncSession
    ...
    async def get_by_java_version(
        self, java_version: str, db: AsyncSession
    ...
    async def get_supported_features(
        ...
        try:
            ...
            patterns = await ConversionPatternCRUD.get_by_version(
                db, minecraft_version=java_version, validation_status="validated"
            )
            ...
            if feature_type:
                relevant_patterns = [
                    p for p in patterns if feature_type in p.get("tags", [])
                ]
            else:
            ...
            return {"supported": False, "features": [], "error": str(e)}
    ...
    async def update_compatibility(
        ...
        try:
            ...
            if existing:
                ...
                update_data = {
                    "compatibility_score": compatibility_data.get(
                        "compatibility_score", 0.0
                    ),
                    "features_supported": compatibility_data.get(
                        "features_supported", []
                    ),
                    "deprecated_patterns": compatibility_data.get(
                        "deprecated_patterns", []
                    ),
                    "migration_guides": compatibility_data.get("migration_guides", {}),
                    "auto_update_rules": compatibility_data.get(
                        "auto_update_rules", {}
                    ),
                ...
                new_compatibility = await VersionCompatibilityCRUD.create(
                    db, create_data
                )
    ...
    async def get_matrix_overview(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
                ...
                for bv in bedrock_versions:
                    ...
                    compat = next(
                        (
                            ...
                            if c.java_version == jv and c.bedrock_version == bv
                        ),
                        None,
                    )
            ...
            return {
                ...
                "matrix": matrix,
                "last_updated": max(
                    (c.updated_at for c in compatibilities)
                ).isoformat(),
            }

        except Exception as e:
            logger.error(f"Error getting matrix overview: {e}")
            return {"error": str(e), "message": "Failed to get matrix overview"}
    ...
    async def generate_migration_guide(
        ...
        try:
            ...
            return {"error": str(e), "message": "Failed to generate migration guide"}

    async def _find_closest_compatibility(
        self, db: AsyncSession, java_version: str, bedrock_version: str
    ) -> Optional[VersionCompatibility]:
        ...
        try:
            ...
            closest_bedrock = self._find_closest_version(
                bedrock_version, bedrock_versions
            )
            ...
            closest_compat = next(
                (
                    ...
                    if c.java_version == closest_java
                    and c.bedrock_version == closest_bedrock
                ),
                None,
            )
    ...
    def _find_closest_version(
        self, target_version: str, available_versions: List[str]
    ) -> str:
        ...
        try:
            ...
            best_version = (
                available_versions[0] if available_versions else target_version
            )
    ...
    async def _find_optimal_conversion_path(
        ...
        try:
            ...
            try:
                ...
                return {
                    "path_type": "failed",
                    "message": f"Source Java version {java_version} not found",
                }
            ...
            try:
                ...
                return {
                    "path_type": "failed",
                    "message": f"Target Bedrock version {bedrock_version} not found",
                }
            ...
            if not intermediate_bedrock:
                ...
                for bv in bedrock_versions:
                    compat = await self.get_compatibility(intermediate_java, bv, db)
            ...
            if not intermediate_bedrock:
                return {
                    "path_type": "failed",
                    "message": "No suitable intermediate versions found",
                }
                ...
                "steps": [
                    {
                        ...
                        "compatibility_score": first_compat.compatibility_score
                        if first_compat
                        else 0.0,
                        "patterns": await self._get_relevant_patterns(
                            db, java_version, feature_type
    ...
    async def _get_relevant_patterns(
        self, db: AsyncSession, version: str, feature_type: str
    ) -> List[Dict[str, Any]]:
        """Get relevant conversion patterns for version and feature type."""
        try:
            patterns = await ConversionPatternCRUD.get_by_version(
                db, minecraft_version=version, validation_status="validated"
            )

            # Filter by feature type
            relevant = []
            for pattern in patterns:
                if feature_type in pattern.tags or pattern.name.lower().contains(
                    feature_type.lower()
                ):
                    relevant.append(
                        {
    ...
    async def _find_best_bedrock_match(
        self, db: AsyncSession, java_version: str, feature_type: str
    ) -> Optional[str]:
