...
class AdvancedVisualizationService:
    ...
    async def create_visualization(
        ...
        try:
            ...
            # Apply layout
            await self._apply_layout(nodes, edges, layout)
    ...
    async def update_visualization_filters(
        ...
        try:
            ...
            nodes, edges = await self._create_visualization_elements(
                filtered_data,
                VisualizationType(visualization.metadata.get("visualization_type")),
            )

            # Reapply layout (maintaining positions where possible)
            await self._reapply_layout(
                visualization.nodes,
                visualization.edges,
                nodes,
                edges,
            ...
            return {"success": False, "error": f"Filter update failed: {str(e)}"}

    async def change_layout(
        self, visualization_id: str, layout: LayoutAlgorithm, animate: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            original_positions = {
                node.id: {"x": node.x, "y": node.y} for node in visualization.nodes
            }
    ...
    async def focus_on_node(
        self, visualization_id: str, node_id: str, radius: int = 2, animate: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for hop in range(radius):
                ...
                for edge in visualization.edges:
                    if edge.source in current_nodes and edge.target not in focus_nodes:
                        ...
                        edge.target in current_nodes and edge.source not in focus_nodes
                    ):
            ...
            viewport = await self._calculate_viewport(
                focus_nodes_list, focus_edges_list
            )
            ...
            return {"success": False, "error": f"Focus operation failed: {str(e)}"}

    async def create_filter_preset(
        self, preset_name: str, filters: List[Dict[str, Any]], description: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {"success": False, "error": f"Preset creation failed: {str(e)}"}

    async def export_visualization(
        self, visualization_id: str, format: str = "json", include_metadata: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    async def get_visualization_metrics(self, visualization_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "basic_metrics": {
                    ...
                    "visible_nodes": sum(
                        1 for node in visualization.nodes if node.visibility
                    ),
                    "visible_edges": sum(
                        1 for edge in visualization.edges if edge.visibility
                    ),
                },
    ...
    async def _get_graph_data(self, graph_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            if db:
                ...
                # Get relationships
                db_relationships = await KnowledgeRelationshipCRUD.get_all(
                    db, limit=2000
                )
    ...
    async def _apply_filters(
        self, graph_data: Dict[str, Any], filters: Optional[List[Dict[str, Any]]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for filter_data in filters:
                ...
                if filter_obj.filter_type == FilterType.NODE_TYPE:
                    ...
                    filtered_nodes = [
                        node
                        for node in filtered_nodes
                        if search_value in node.get("name", "").lower()
                        or search_value in node.get("description", "").lower()
                    ]
            ...
            filtered_edges = [
                edge
                for edge in filtered_edges
                if edge.get("source_id") in node_ids
    ...
    def _matches_filter(
        self, item: Dict[str, Any], filter_obj: VisualizationFilter
    ) -> bool:
    ...
    async def _create_visualization_elements(
        self, graph_data: Dict[str, Any], visualization_type: VisualizationType
    ...
    async def _apply_layout(
        ...
        try:
            ...
            return {
                node["id"]: {"x": i * 100, "y": i * 100} for i, node in enumerate(nodes)
            }
