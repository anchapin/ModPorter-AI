    ...
    for asset in addon_assets:
        if asset.type == "texture":  # Assuming a way to identify block textures
            ...
            original_name = (
                ...
                else _sanitize_filename(os.path.basename(asset.path))
            )
...
def generate_sounds_json(
    sound_assets: List[pydantic_addon_models.AddonAsset],
) -> Dict[str, Any]:
    """
    ...
    for asset in sound_assets:
        ...
        sounds_data[sound_name] = {
            "sounds": [{"name": f"sounds/{sound_name}", "volume": 1.0, "pitch": 1.0}]
        }
