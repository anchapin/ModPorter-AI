...
class AsyncTaskQueue:
    ...
    async def fail(
        ...
        if should_retry:
            ...
            logger.info(
                f"Task {task_id} scheduled for retry ({retry_count + 1}/{max_retries}) at {next_retry}"
            )
            return True
        else:
    ...
    async def process_retry_queue(self) -> int:
        ...
        task_ids = await redis.zrangebyscore("task_queue:retry", min=0, max=now)
    ...
    async def get_queue_health(self) -> QueueHealth:
        ...
        try:
                ...
                if count > 0:
                    ...
                    if oldest:
                        ...
                        health.oldest_queued_age_seconds = max(
                            health.oldest_queued_age_seconds, age
                        )
            ...
            if health.oldest_queued_age_seconds > 3600:  # 1 hour
                issues.append(
                    f"Oldest queued task is {health.oldest_queued_age_seconds / 60:.1f} minutes old"
                )
