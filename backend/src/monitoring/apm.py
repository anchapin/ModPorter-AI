...
class APMManager:
    ...
    def __init__(self, service_name: str, redis_client: redis.Redis = None):
        ...
        self.system_metrics = {
            'cpu_usage': Gauge(f'{service_name}_cpu_usage_percent', 'CPU usage percentage'),
            'memory_usage': Gauge(f'{service_name}_memory_usage_bytes', 'Memory usage in bytes'),
            'disk_usage': Gauge(f'{service_name}_disk_usage_percent', 'Disk usage percentage'),
            'gc_collection': Counter(f'{service_name}_gc_collections_total', 'GC collections', ['generation'])
        }
        ...
        try:
            ...
            if span.duration_ms:
                self.request_duration.observe(
                    ...
                    labels={'method': span.tags.get('method', 'UNKNOWN'), 'endpoint': span.operation_name}
                )
    ...
    def record_business_metric(self, metric_name: str, value: float, tags: Dict[str, Any] = None):
    ...
    async def _store_business_metric(self, metric_name: str, value: float, tags: Dict[str, Any]):
        ...
        try:
            ...
            await self.redis.lpush(
                f"apm:business_metrics:{metric_name}",
                json.dumps(metric_data)
            )
            await self.redis.ltrim(f"apm:business_metrics:{metric_name}", 0, 9999)  # Keep last 10k
            await self.redis.expire(f"apm:business_metrics:{metric_name}", 86400)  # 24 hours
    ...
    async def get_business_metrics(self, metric_name: str, minutes: int = 60) -> List[Dict[str, Any]]:
        ...
        try:
            cutoff_time = datetime.utcnow() - timedelta(minutes=minutes)
            metric_data = await self.redis.lrange(f"apm:business_metrics:{metric_name}", 0, 1000)
