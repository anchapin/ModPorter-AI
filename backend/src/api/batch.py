...
async def submit_batch_job(
    job_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Invalid operation_type: {operation_type_str}"
            )
...
async def cancel_job(job_id: str, cancel_data: Dict[str, Any] = None):
    """Cancel a running batch job."""
    try:
        reason = (
            cancel_data.get("reason", "User requested cancellation")
            if cancel_data
            else "User requested cancellation"
        )
...
async def pause_job(job_id: str, pause_data: Dict[str, Any] = None):
    """Pause a running batch job."""
    try:
        reason = (
            pause_data.get("reason", "User requested pause")
            if pause_data
            else "User requested pause"
        )
    ...
    try:
        ...
        if operation_type:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid operation_type: {operation_type}"
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get job history: {str(e)}"
        )
...
async def import_nodes(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Invalid processing_mode: {processing_mode}"
            )
        ...
        try:
            if file.filename.endswith(".json"):
                ...
                raise HTTPException(
                    status_code=400, detail="Unsupported file format. Use JSON or CSV."
                )
        except Exception as e:
            raise HTTPException(
...
async def export_graph(export_data: Dict[str, Any], db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        result = await batch_processing_service.submit_batch_job(
            BatchOperationType.EXPORT_GRAPH,
            parameters,
            mode,
            chunk_size,
            parallel_workers,
...
async def batch_delete_nodes(
    delete_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        if not filters:
            raise HTTPException(
                status_code=400, detail="filters are required for deletion"
            )
        ...
        result = await batch_processing_service.submit_batch_job(
            BatchOperationType.DELETE_NODES,
            parameters,
            mode,
            chunk_size,
            parallel_workers,
...
async def batch_validate_graph(
    validation_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        for rule in validation_rules:
            if rule not in valid_rules:
                raise HTTPException(
                    status_code=400, detail=f"Invalid validation rule: {rule}"
                )
        ...
        result = await batch_processing_service.submit_batch_job(
            BatchOperationType.VALIDATE_GRAPH,
            parameters,
            mode,
            chunk_size,
            parallel_workers,
...
async def get_operation_types():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get operation types: {str(e)}"
        )
...
async def get_processing_modes():
    ...
    try:
        ...
        return {"success": True, "processing_modes": modes, "total_modes": len(modes)}

    except Exception as e:
        logger.error(f"Error getting processing modes: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get processing modes: {str(e)}"
        )
    ...
    try:
        ...
        if active_result["success"]:
            for job in active_result["active_jobs"]:
                ...
                operation_type_counts[op_type] = (
                    operation_type_counts.get(op_type, 0) + 1
                )
        ...
        return {
            ...
            "summary": {
                ...
                "total_active": active_result["total_active"]
                if active_result["success"]
                else 0,
                "queue_size": active_result["queue_size"]
                if active_result["success"]
                else 0,
                "max_concurrent": active_result["max_concurrent_jobs"]
                if active_result["success"]
                else 0,
                "recent_history": len(history_result.get("job_history", []))
                if history_result["success"]
                else 0,
            },
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get status summary: {str(e)}"
        )
