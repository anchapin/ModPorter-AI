...
async def create_comparison(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Invalid conversion_id format: '{request.conversion_id}'. Must be a UUID.",
            )
        ...
        for fm_db in db_comparison.feature_mappings:
            feature_mappings_list.append(
                FeatureMappingResponse(
                    ...
                    confidence_score=(
                        float(fm_db.confidence_score)
                        if fm_db.confidence_score is not None
                        else None
                    ),
                )
            )
