...
async def create_knowledge_node(
    node_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
...
async def get_node_relationships(
    ...
    relationships = [
        ...
        if (
            node_id is None
            or e.get("source_id") == node_id
            or e.get("target_id") == node_id
        )
        and (not relationship_type or e.get("relationship_type") == relationship_type)
    ]
    return {
        "relationships": relationships,
        "graph_data": {"nodes": list(mock_nodes.values()), "edges": mock_edges},
        "node_id": node_id,
...
async def create_knowledge_relationship(
    relationship_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    if not source_id or not target_id:
        raise HTTPException(
            status_code=422,
            detail="source_id/target_id (or source/target) are required",
        )
...
async def create_conversion_pattern(
    pattern_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
...
async def create_version_compatibility(
    compatibility_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
...
async def search_graph(
    ...
    # Mock implementation for now
    return {"neo4j_results": [], "postgresql_results": []}
...
async def update_node_validation(
    node_id: str, validation_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
...
async def get_knowledge_node(node_id: str, db: AsyncSession = Depends(get_db)):
...
async def update_knowledge_node(
    node_id: str, update_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        return {
            ...
            "metadata": {
                ...
                "community_rating": float(updated_node.community_rating)
                if updated_node.community_rating
                else None,
            },
            "updated_at": updated_node.updated_at.isoformat()
            if updated_node.updated_at
            else None,
        }
...
async def get_node_neighbors(node_id: str, db: AsyncSession = Depends(get_db)):
...
async def extract_subgraph(
    node_id: str, depth: int = 1, db: AsyncSession = Depends(get_db)
):
    ...
    for edge in mock_edges:
        if edge.get("source_id") == node_id:
            ...
            neighbor = mock_nodes.get(
                target_id, {"id": target_id, "name": f"Neighbor_{target_id[:6]}"}
            )
    ...
    for _ in range(needed):
        ...
        neighbor_nodes.append(
            {"id": fake_id, "name": f"Neighbor_{len(neighbor_nodes) + 1}"}
        )
        edges.append(
            {
                "source_id": node_id,
...
async def query_knowledge_graph(
    query_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
...
async def get_visualization_data(
    layout: str = "force_directed", limit: int = 10, db: AsyncSession = Depends(get_db)
):
...
async def get_graph_insights(
    ...
    analysis_types: Optional[Any] = Query(
        ["patterns", "gaps", "connections"], description="Analysis types to include"
    ),
    db: AsyncSession = Depends(get_db),
):
    ...
    patterns = [
        {
            "focus": "Block Registration",
            "pattern": "deferred_registration",
            "prevalence": 0.65,
        },
        {
            "focus": "Block Properties",
            "pattern": "use_block_states",
            "prevalence": 0.52,
        },
        {
            "focus": "Block Performance",
            "pattern": "tick_optimization",
            "prevalence": 0.41,
        },
    ]
    knowledge_gaps = [
...
async def batch_create_nodes(
    batch_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
