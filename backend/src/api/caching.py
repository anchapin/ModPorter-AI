...
async def get_cache_stats(
    cache_type: Optional[str] = Query(None, description="Type of cache to get stats for")
):
...
async def optimize_cache(
    ...
    try:
        strategy = optimization_data.get("strategy", "adaptive") if optimization_data else "adaptive"
...
async def invalidate_cache(
    ...
    try:
        ...
        result = await graph_caching_service.invalidate(
            cache_type, pattern, cascade
        )
    ...
    try:
        with graph_caching_service.lock:
            if cache_type not in graph_caching_service.l1_cache:
                raise HTTPException(
                    status_code=404,
                    detail=f"Cache type '{cache_type}' not found"
                )
    ...
    try:
        ...
        if existing_config:
            ...
            if "invalidation_strategy" in config_data:
                try:
                    new_config.invalidation_strategy = CacheInvalidationStrategy(config_data["invalidation_strategy"])
                except ValueError:
                    raise HTTPException(
                        status_code=400,
            ...
            try:
                ...
                new_config = {
                    ...
                    "refresh_interval_seconds": config_data.get("refresh_interval_seconds", 300),
        ...
        raise HTTPException(status_code=500, detail=f"Cache config update failed: {str(e)}")
                ...
                performance_metrics[cache_type] = {
                    ...
                    "efficiency": {
                        "operations_per_second": (
                            (cache_stats.hits + cache_stats.misses) / max(cache_stats.avg_access_time_ms / 1000, 0.001)
                        ),
                        "bytes_per_hit": cache_stats.total_size_bytes / max(cache_stats.hits, 1),
                        "hit_per_mb": cache_stats.hits / max(cache_stats.memory_usage_mb, 0.001)
                    }
                }
...
async def get_cache_history(
    ...
    try:
        with graph_caching_service.lock:
            ...
            avg_access_time = sum(avg_access_times) / len(avg_access_times) if avg_access_times else 0
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get cache history: {str(e)}")
...
async def get_cache_strategies():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get cache strategies: {str(e)}")
...
async def get_invalidation_strategies():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get invalidation strategies: {str(e)}")
    ...
    try:
        ...
        return {
            ...
            "cache_performance": {
                ...
                "operations_per_second": iterations / (test_time / 1000) if test_time > 0 else 0
            },
        ...
        raise HTTPException(status_code=500, detail=f"Cache performance test failed: {str(e)}")


@router.delete("/cache/clear")
async def clear_cache(
    cache_type: Optional[str] = Query(None, description="Type of cache to clear (None for all)")
):
    """Clear cache entries."""
    try:
        with graph_caching_service.lock:
    ...
    try:
        with graph_caching_service.lock:
            ...
            return {
                ...
                "cleanup_thread_running": graph_caching_service.cleanup_thread is not None,
...
def _get_strategy_use_cases(strategy) -> List[str]:
    ...
    use_cases = {
        CacheStrategy.LRU: ["General purpose caching", "Web page caching", "Database query results"],
        CacheStrategy.LFU: ["Access pattern prediction", "Frequently accessed data", "Hot data management"],
        CacheStrategy.FIFO: ["Simple caching", "Ordered data", "Session data"],
        CacheStrategy.TTL: ["Time-sensitive data", "API responses", "Configuration data"],
        CacheStrategy.WRITE_THROUGH: ["Critical data", "Financial data", "Real-time updates"],
        CacheStrategy.WRITE_BEHIND: ["High write volume", "Logging systems", "Analytics data"],
        CacheStrategy.REFRESH_AHEAD: ["Predictable access patterns", "Preloading data", "Background updates"]
    }
...
def _get_invalidation_use_cases(strategy) -> List[str]:
    ...
    use_cases = {
        CacheInvalidationStrategy.TIME_BASED: ["API responses", "News feeds", "Market data"],
        CacheInvalidationStrategy.EVENT_DRIVEN: ["User updates", "System changes", "Data modifications"],
        CacheInvalidationStrategy.MANUAL: ["Administrative control", "Data migrations", "System maintenance"],
        CacheInvalidationStrategy.PROACTIVE: ["Predictive caching", "Load balancing", "Performance optimization"],
        CacheInvalidationStrategy.ADAPTIVE: ["Dynamic systems", "Variable workloads", "Smart caching"]
    }
    return use_cases.get(strategy, ["General use"])
