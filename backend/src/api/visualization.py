...
async def create_visualization(
    viz_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Invalid visualization_type: {viz_type_str}"
            )
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid layout: {layout_str}")
        ...
        raise HTTPException(
            status_code=500, detail=f"Visualization creation failed: {str(e)}"
        )
...
async def get_visualization(visualization_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get visualization: {str(e)}"
        )
...
async def change_visualization_layout(
    visualization_id: str, layout_data: Dict[str, Any]
):
...
async def focus_on_node(visualization_id: str, focus_data: Dict[str, Any]):
...
async def create_filter_preset(preset_data: Dict[str, Any]):
    ...
    try:
        ...
        if not all([preset_name, filters]):
            raise HTTPException(
                status_code=400, detail="preset_name and filters are required"
            )
...
async def get_filter_presets():
    ...
    try:
        ...
        for (
            preset_name,
            filters,
        ) in advanced_visualization_service.filter_presets.items():
            presets.append(
                {
                    "name": preset_name,
        ...
        return {"success": True, "presets": presets, "total_presets": len(presets)}
...
async def export_visualization(visualization_id: str, export_data: Dict[str, Any]):
...
async def get_visualization_metrics(visualization_id: str):
    ...
    try:
        result = await advanced_visualization_service.get_visualization_metrics(
            visualization_id
        )
...
async def get_layout_algorithms():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get algorithms: {str(e)}"
        )
...
async def get_filter_types():
    ...
    try:
        ...
        return {"success": True, "filter_types": types, "total_types": len(types)}

    except Exception as e:
        logger.error(f"Error getting filter types: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get filter types: {str(e)}"
        )
...
async def get_active_visualizations():
    ...
    try:
        ...
        for (
            viz_id,
            viz_state,
        ) in advanced_visualization_service.visualization_cache.items():
            visualizations.append(
                {
                    ...
                    "created_at": viz_state.created_at.isoformat(),
                    "last_updated": viz_state.metadata.get(
                        "last_updated", viz_state.created_at.isoformat()
                    ),
                }
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get visualizations: {str(e)}"
        )
...
async def delete_visualization(visualization_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Visualization deletion failed: {str(e)}"
        )
