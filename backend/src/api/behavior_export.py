...
async def export_behavior_pack(
    ...
    if request.include_templates:
        template_info = {"used_templates": [], "generated_from_templates": []}
    ...
    if request.export_format == "json":
        ...
        try:
            ...
            zip_bytes_io = addon_exporter.create_mcaddon_zip(
                addon_pydantic=addon_details, asset_base_path=asset_base_path
            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to create MCADDON: {str(e)}"
            )
...
@router.get("/export/preview/{conversion_id}", summary="Preview export data")
async def preview_export(
    ...
    file_analysis = {
        ...
        "file_types": {},
        "template_usage": {"files_with_templates": 0, "unique_templates": set()},
    }
