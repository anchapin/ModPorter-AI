...
async def create_collaboration_session(
    session_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        if not all([graph_id, user_id, user_name]):
            raise HTTPException(
                status_code=400, detail="graph_id, user_id, and user_name are required"
            )
...
async def join_collaboration_session(
    session_id: str, join_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        if not all([user_id, user_name]):
            raise HTTPException(
                status_code=400, detail="user_id and user_name are required"
            )
...
async def leave_collaboration_session(
    session_id: str, leave_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        result = await realtime_collaboration_service.leave_collaboration_session(
            user_id, db
        )
...
async def apply_operation(
    session_id: str, operation_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        if not all([user_id, operation_type_str]):
            raise HTTPException(
                status_code=400, detail="user_id and operation_type are required"
            )
...
async def resolve_conflict(
    ...
    try:
        ...
        if not all([user_id, resolution_strategy]):
            raise HTTPException(
                status_code=400, detail="user_id and resolution_strategy are required"
            )
...
async def get_user_activity(
    session_id: str, user_id: str, minutes: int = 30, db: AsyncSession = Depends(get_db)
):
...
async def get_active_sessions():
    ...
    try:
        ...
        for (
            session_id,
            session,
        ) in realtime_collaboration_service.active_sessions.items():
...
async def get_conflict_types():
    ...
    try:
        ...
        return {"success": True, "conflict_types": conflict_types}

    except Exception as e:
        logger.error(f"Error getting conflict types: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get conflict types: {str(e)}"
        )
...
async def websocket_collaboration(
    websocket: WebSocket, session_id: str, user_id: str, user_name: str
):
        ...
        async with get_async_session() as db:
            ...
            if not result["success"]:
                await websocket.send_text(
                    json.dumps({"type": "error", "error": result["error"]})
                )
            ...
            try:
                while True:
                    ...
                    # Handle message
                    response = (
                        await realtime_collaboration_service.handle_websocket_message(
                ...
                await realtime_collaboration_service.handle_websocket_disconnect(
                    user_id
                )
                ...
                await websocket.send_text(
                    json.dumps({"type": "error", "error": f"WebSocket error: {str(e)}"})
                )
    ...
    try:
        ...
        return {
            ...
            "stats": {
                ...
                "average_users_per_session": total_users / total_sessions
                if total_sessions > 0
                else 0,
            },
        }
