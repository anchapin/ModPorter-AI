...
async def create_commit(
    commit_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
...
async def get_commit_changes(commit_hash: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get commit changes: {str(e)}"
        )
...
async def get_branch_history(
    ...
    since: Optional[str] = Query(
        None, description="Get commits since this commit hash"
    ),
    until: Optional[str] = Query(
        None, description="Get commits until this commit hash"
    ),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get branch history: {str(e)}"
        )
...
async def get_branch_status(branch_name: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get branch status: {str(e)}"
        )
...
async def merge_branch(
    source_branch: str, merge_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
...
@router.post("/diff")
async def generate_diff(diff_data: Dict[str, Any], db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if not all([base_hash, target_hash]):
            raise HTTPException(
                status_code=400, detail="base_hash and target_hash are required"
            )
            ...
            "diff": {
                ...
                "summary": {
                    ...
                    "total_changes": (
                        ...
                        + len(diff.deleted_nodes)
                        + len(diff.added_relationships)
                        + len(diff.modified_relationships)
                        + len(diff.deleted_relationships)
                        + len(diff.added_patterns)
                        + len(diff.modified_patterns)
                        + len(diff.deleted_patterns)
                    ),
...
async def revert_commit(
    commit_hash: str, revert_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        if not all([author_id, author_name]):
            raise HTTPException(
                status_code=400, detail="author_id and author_name are required"
            )
...
async def get_tags():
    ...
    try:
        ...
        return {"success": True, "tags": tags, "total_tags": len(tags)}
...
async def get_version_control_status():
    ...
    try:
        ...
        if head_branch in graph_version_control_service.branches:
            head_commit = graph_version_control_service.branches[
                head_branch
            ].head_commit
        ...
        return {
            ...
            "status": {
                ...
                "protected_branches": [
                    name
                    for name, branch in graph_version_control_service.branches.items()
...
async def get_version_control_stats():
    ...
    try:
        ...
        # Sort for ranking
        top_authors = sorted(
            commits_by_author.items(), key=lambda x: x[1], reverse=True
        )[:10]
        active_branches = sorted(
            commits_by_branch.items(), key=lambda x: x[1], reverse=True
        )
        ...
        return {
            ...
            "stats": {
                ...
                "average_commits_per_author": sum(commits_by_author.values())
                / len(commits_by_author)
                if commits_by_author
                else 0,
                "average_commits_per_branch": sum(commits_by_branch.values())
                / len(commits_by_branch)
...
async def search_commits(
    ...
    try:
        ...
        return {
            "success": True,
            "query": query,
            "filters": {"author": author, "branch": branch},
...
async def get_changelog(
    ...
    since: Optional[str] = Query(
        None, description="Get changes since this commit hash"
    ),
    ...
    try:
        ...
        for commit in commits:
            entry = {
                ...
                "message": commit["message"],
                "changes_summary": {"total": commit["changes_count"], "by_type": {}},
            }
        ...
        return {
            ...
            "summary": {
                "total_changes": sum(
                    entry["changes_summary"]["total"] for entry in changelog
                ),
                "date_range": {
        ...
        raise HTTPException(
            status_code=500, detail=f"Changelog generation failed: {str(e)}"
        )
