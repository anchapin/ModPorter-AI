...
def mock_conversion_options():
    ...
    return ConversionOptions(assumptions="conservative", target_version="1.20.0")
...
def mock_security_scanner():
    ...
    scanner.scan_file = Mock(return_value={"safe": True, "threats": [], "details": "File is safe"})
    return scanner
...
class TestConversionCreateAPI:
    ...
    async def test_create_conversion_success(self, test_db, mock_jar_file, mock_conversion_options):
        ...
        conversion_request = ConversionCreateRequest(
            mod_file=mock_jar_file.name, options=mock_conversion_options.dict()
        )
    ...
    async def test_create_conversion_invalid_file(self, test_db):
        ...
        with pytest.raises(ValueError):
            ConversionCreateRequest(mod_file="test.txt", options={"assumptions": "conservative"})
    ...
    async def test_create_conversion_missing_file(self, test_db):
        ...
        with pytest.raises(FileNotFoundError):
            ConversionCreateRequest(
                mod_file="/nonexistent/test.jar", options={"assumptions": "conservative"}
            )
    ...
    async def test_conversion_options_validation(self):
        ...
        # Valid options
        valid_opts = ConversionOptions(assumptions="aggressive", target_version="1.21.0")
    ...
    async def test_create_conversion_with_metadata(self, mock_jar_file):
        ...
        metadata = {"author": "test", "description": "test mod", "tags": ["block", "item"]}

        conversion_request = ConversionCreateRequest(
            mod_file=mock_jar_file.name, options={"assumptions": "conservative"}, metadata=metadata
        )
...
class TestConversionRetrieval:
    ...
    async def test_list_conversions_paginated(self, test_db):
        ...
        conversions = [
            {"id": str(uuid4()), "status": "completed", "created_at": datetime.now(timezone.utc)}
            for _ in range(5)
        ]
...
class TestConversionExecution:
    ...
    async def test_execute_conversion_with_progress(self, mock_conversion_service):
        ...
        mock_conversion_service.convert = AsyncMock(return_value={"success": True})
    ...
    async def test_execute_conversion_timeout(self, mock_conversion_service):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await asyncio.wait_for(
                mock_conversion_service.convert("test.jar", "conservative"), timeout=0.1
            )

    @pytest.mark.asyncio
    async def test_execute_conversion_invalid_input(self, mock_conversion_service):
        """Test conversion with invalid input."""
        mock_conversion_service.convert = AsyncMock(side_effect=ValueError("Invalid JAR format"))
...
class TestConversionSecurity:
    ...
    def test_scan_file_permission_denied(self, mock_security_scanner):
        ...
        mock_security_scanner.scan_file = Mock(side_effect=PermissionError("Cannot read file"))
    ...
    def test_scan_file_not_found(self, mock_security_scanner):
        ...
        mock_security_scanner.scan_file = Mock(side_effect=FileNotFoundError("File not found"))
...
class TestConversionCache:
    ...
    async def test_cache_conversion_result(self):
        ...
        cached_data = {"registry_name": "test_block", "output_file": "/tmp/test.mcaddon"}
