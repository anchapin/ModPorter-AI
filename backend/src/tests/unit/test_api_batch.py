...
class TestBatchJobEndpoints:
    ...
    async def test_submit_batch_job_success(self, mock_db, mock_batch_service):
        ...
        # Setup mock response
        mock_batch_service.submit_batch_job.return_value = {
            "success": True,
            "job_id": "test-job-123",
    ...
    @pytest.mark.asyncio
    async def test_submit_batch_job_missing_operation_type(self, mock_db, mock_batch_service):
    ...
    async def test_get_job_status_success(self, mock_batch_service):
        ...
        mock_batch_service.get_job_status.return_value = {
            "success": True,
            "job_id": "test-job-123",
            "status": "running",
            "progress": 45.5,
    ...
    @pytest.mark.asyncio
    async def test_get_job_status_not_found(self, mock_batch_service):
    ...
    @pytest.mark.asyncio
    async def test_cancel_job_success(self, mock_batch_service):
        """Test successful job cancellation"""
        mock_batch_service.cancel_job.return_value = {
            "success": True,
            "job_id": "test-job-123",
            "status": "cancelled",
        ...
        mock_batch_service.cancel_job.assert_called_once_with("test-job-123", "Test cancellation")
        assert result["success"] is True
        assert result["status"] == "cancelled"
    
    @pytest.mark.asyncio
    async def test_cancel_job_failure(self, mock_batch_service):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await cancel_job("test-job-123")
        
        assert exc_info.value.status_code == 400
        assert "cannot be cancelled" in str(exc_info.value.detail)
    
    @pytest.mark.asyncio
    async def test_pause_job_success(self, mock_batch_service):
        """Test successful job pausing"""
        mock_batch_service.pause_job.return_value = {
            "success": True,
            "job_id": "test-job-123",
            "status": "paused",
        ...
        mock_batch_service.pause_job.assert_called_once_with("test-job-123", "User requested pause")
        assert result["success"] is True
        assert result["status"] == "paused"
    
    @pytest.mark.asyncio
    async def test_resume_job_success(self, mock_batch_service):
        """Test successful job resuming"""
        mock_batch_service.resume_job.return_value = {
            "success": True,
            "job_id": "test-job-123",
            "status": "running",
        ...
        mock_batch_service.resume_job.assert_called_once_with("test-job-123")
        assert result["success"] is True
        assert result["status"] == "running"
    
    @pytest.mark.asyncio
    async def test_get_active_jobs_success(self, mock_batch_service):
        """Test successful active jobs retrieval"""
        mock_batch_service.get_active_jobs.return_value = {
            "success": True,
            "active_jobs": [
        ...
        mock_batch_service.get_active_jobs.assert_called_once()
        assert result["success"] is True
        assert len(result["active_jobs"]) == 2
        assert result["total_active"] == 2
    
    @pytest.mark.asyncio
    async def test_get_job_history_success(self, mock_batch_service):
        """Test successful job history retrieval"""
        mock_batch_service.get_job_history.return_value = {
            "success": True,
            "job_history": [
                {"job_id": "job1", "status": "completed", "operation_type": "import_nodes"},
                {"job_id": "job2", "status": "completed", "operation_type": "export_graph"}
            ],
            "total_history": 2
        }
        
        from src.api.batch import get_job_history
        result = await get_job_history(limit=50, operation_type="import_nodes")
        
        mock_batch_service.get_job_history.assert_called_once_with(50, BatchOperationType.IMPORT_NODES)
        assert result["success"] is True
        assert len(result["job_history"]) == 2
    
    @pytest.mark.asyncio
    async def test_get_job_history_invalid_operation_type(self, mock_batch_service):
        """Test job history retrieval with invalid operation type"""
        from src.api.batch import get_job_history
        from fastapi import HTTPException
        
        with pytest.raises(HTTPException) as exc_info:
            await get_job_history(limit=50, operation_type="invalid_type")
        
        assert exc_info.value.status_code == 400
        assert "Invalid operation_type" in str(exc_info.value.detail)


class TestBatchImportExportEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_import_nodes_json_success(self, mock_db, mock_batch_service):
        ...
        mock_file.filename = "test_nodes.json"
        mock_file.read = AsyncMock(return_value=json.dumps([
            {"name": "test_node", "node_type": "mod", "platform": "fabric"}
        ]).encode())
        ...
        call_args = mock_batch_service.submit_batch_job.call_args[0]
        
        assert call_args[0] == BatchOperationType.IMPORT_NODES
        assert call_args[2] == ProcessingMode.SEQUENTIAL
        assert call_args[3] == 100
        assert call_args[4] == 4
        assert call_args[5] == mock_db
        
        # Check parameters contain nodes data
        parameters = call_args[1]
        assert "nodes" in parameters
        assert parameters["source_file"] == "test_nodes.json"
        assert len(parameters["nodes"]) == 1
        
        # Verify response
        assert result["success"] is True
        assert result["job_id"] == "import-job-123"
        assert "test_nodes.json" in result["message"]
    
    @pytest.mark.asyncio
    async def test_import_nodes_csv_success(self, mock_db, mock_batch_service):
        ...
        # Mock upload file
        csv_content = """name,node_type,platform,description,minecraft_version,expert_validated,community_rating
Test Mod,fmod,fabric,A test mod,1.20.1,true,4.5
Another Mod,mod,forge,Another mod,1.19.4,false,3.2"""
        ...
        assert result["success"] is True
        assert result["job_id"] == "import-job-123"
        assert result["estimated_total_items"] == 50
    
    @pytest.mark.asyncio
    async def test_import_nodes_invalid_format(self, mock_db, mock_batch_service):
        """Test nodes import with unsupported file format"""
        mock_file = Mock(spec=UploadFile)
        mock_file.filename = "test_nodes.txt"
    ...
    @pytest.mark.asyncio
    async def test_import_relationships_success(self, mock_db, mock_batch_service):
        """Test successful relationships import"""
        mock_batch_service.submit_batch_job.return_value = {
            "success": True,
            "job_id": "import-rel-123",
        ...
        mock_file = Mock(spec=UploadFile)
        mock_file.filename = "test_relationships.json"
        mock_file.read = AsyncMock(return_value=json.dumps(relationships_data).encode())
        ...
        assert call_args[0] == BatchOperationType.IMPORT_RELATIONSHIPS
        
        # Verify response
        assert result["success"] is True
        assert result["job_id"] == "import-rel-123"
        assert "test_relationships.json" in result["message"]
    
    @pytest.mark.asyncio
    async def test_export_graph_success(self, mock_db, mock_batch_service):
        """Test successful graph export"""
        mock_batch_service.submit_batch_job.return_value = {
            "success": True,
            "job_id": "export-job-123",
            "estimated_total_items": 1500
        }
        
        export_data = {
            "format": "json",
            "filters": {"node_type": "mod"},
        ...
        from src.api.batch import export_graph
        result = await export_graph(export_data, mock_db)
        
        # Verify service was called correctly
        mock_batch_service.submit_batch_job.assert_called_once()
        call_args = mock_batch_service.submit_batch_job.call_args[0]
        
        assert call_args[0] == BatchOperationType.EXPORT_GRAPH
        assert call_args[2] == ProcessingMode.PARALLEL
        
        parameters = call_args[1]
        assert parameters["format"] == "json"
        assert parameters["include_relationships"] is True
        assert parameters["include_patterns"] is False
    ...
    @pytest.mark.asyncio
    async def test_export_graph_invalid_format(self, mock_db, mock_batch_service):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await export_graph(export_data, mock_db)
...
class TestBatchOperationEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_batch_delete_nodes_success(self, mock_db, mock_batch_service):
        """Test successful batch node deletion"""
        mock_batch_service.submit_batch_job.return_value = {
            "success": True,
            "job_id": "delete-job-123",
            "estimated_total_items": 75
        }
        
        delete_data = {
            "filters": {"node_type": "obsolete_mod"},
            "dry_run": False,
            "processing_mode": "sequential",
            "chunk_size": 50,
            "parallel_workers": 2
        ...
        result = await batch_delete_nodes(delete_data, mock_db)
        
        # Verify service was called correctly
        mock_batch_service.submit_batch_job.assert_called_once()
        call_args = mock_batch_service.submit_batch_job.call_args[0]
        
        assert call_args[0] == BatchOperationType.DELETE_NODES
        
        parameters = call_args[1]
        assert parameters["filters"]["node_type"] == "obsolete_mod"
        assert parameters["dry_run"] is False
        
        # Verify response
        assert result["success"] is True
        assert result["job_id"] == "delete-job-123"
        assert result["dry_run"] is False
    
    @pytest.mark.asyncio
    async def test_batch_delete_nodes_missing_filters(self, mock_db, mock_batch_service):
        """Test batch node deletion without filters"""
        delete_data = {
            "dry_run": False
        }
        ...
        with pytest.raises(HTTPException) as exc_info:
            await batch_delete_nodes(delete_data, mock_db)
        
        assert exc_info.value.status_code == 400
        assert "filters are required" in str(exc_info.value.detail)
    
    @pytest.mark.asyncio
    async def test_batch_validate_graph_success(self, mock_db, mock_batch_service):
        """Test successful batch graph validation"""
        mock_batch_service.submit_batch_job.return_value = {
            "success": True,
            "job_id": "validate-job-123",
        ...
        from src.api.batch import batch_validate_graph
        result = await batch_validate_graph(validation_data, mock_db)
        
        # Verify service was called correctly
        mock_batch_service.submit_batch_job.assert_called_once()
        call_args = mock_batch_service.submit_batch_job.call_args[0]
        
        assert call_args[0] == BatchOperationType.VALIDATE_GRAPH
        
        parameters = call_args[1]
        assert parameters["rules"] == ["nodes", "relationships", "consistency"]
        assert parameters["scope"] == "full"
        
        # Verify response
        assert result["success"] is True
        assert result["job_id"] == "validate-job-123"
        assert "consistency" in result["validation_rules"]
    
    @pytest.mark.asyncio
    async def test_batch_validate_graph_invalid_rules(self, mock_db, mock_batch_service):
        """Test batch graph validation with invalid rules"""
        validation_data = {
            "rules": ["invalid_rule"]
        }
        ...
        with pytest.raises(HTTPException) as exc_info:
            await batch_validate_graph(validation_data, mock_db)
        
        assert exc_info.value.status_code == 400
        assert "Invalid validation rule" in str(exc_info.value.detail)
...
class TestBatchUtilityEndpoints:
    ...
    async def test_get_performance_stats(self):
        ...
        # Check operation type performance data
        op_performance = stats["operation_type_performance"]
        assert "import_nodes" in op_performance
        assert "export_graph" in op_performance
        assert "validate_graph" in op_performance
        
        for op in op_performance.values():
            assert "total_jobs" in op
            assert "success_rate" in op
            assert "avg_time_per_1000_items" in op
...
Another Mod,mod,forge,Another mod,1.19.4,false,3.2,"{}\"""
        ...
        assert node1["properties"]["author"] == "test"
        
        # Check second node
        node2 = nodes[1]
        assert node2["name"] == "Another Mod"
        assert node2["expert_validated"] is False
        assert node2["community_rating"] == 3.2
...
node2,node3,depends_on,0.9,"{}""""
        ...
        assert rel1["properties"]["weight"] == 0.5
        
        # Check second relationship
        rel2 = relationships[1]
        assert rel2["source_node_id"] == "node2"
        assert rel2["target_node_id"] == "node3"
        assert rel2["relationship_type"] == "depends_on"
        assert rel2["confidence_score"] == 0.9
    
    @pytest.mark.asyncio
    async def test_parse_csv_nodes_invalid_format(self):
        """Test CSV nodes parsing with invalid format"""
        invalid_csv = "invalid,csv,format"
        
        from src.api.batch import _parse_csv_nodes
        with pytest.raises(ValueError, match="Failed to parse CSV nodes"):
            await _parse_csv_nodes(invalid_csv)
    
    @pytest.mark.asyncio
    async def test_parse_csv_relationships_invalid_format(self):
        """Test CSV relationships parsing with invalid format"""
        invalid_csv = "invalid,csv,format"
        
        from src.api.batch import _parse_csv_relationships
        with pytest.raises(ValueError, match="Failed to parse CSV relationships"):
            await _parse_csv_relationships(invalid_csv)
    ...
    def test_get_processing_mode_use_cases(self):
        """Test processing mode use cases helper"""
        ...
        assert "performance" in str(use_cases).lower()
    
    def test_get_processing_mode_recommendations(self):
        """Test processing mode recommendations helper"""
        from src.api.batch import _get_processing_mode_recommendations
        
        recommendations = _get_processing_mode_recommendations(ProcessingMode.SEQUENTIAL)
        assert "small" in recommendations[0].lower()
        
        recommendations = _get_processing_mode_recommendations(ProcessingMode.PARALLEL)
        assert "multi-core" in recommendations[0].lower()
