...
class TestConversionEvent:
    ...
    def test_conversion_event_with_timestamps(self):
        ...
        assert 299.9 < event.conversion_duration_seconds < 300.1  # ~5 minutes (allowing for timing precision)
...
class TestAutomationMetricsService:
    ...
    def test_get_current_metrics_period_filter(self, service):
        ...
        service._events.append(ConversionEvent(
    ...
    def test_get_dashboard_data_structure(self, service):
        ...
        # Check status values
        assert dashboard["status"]["overall"] in ["excellent", "good", "needs_improvement", "critical"]
...
class TestAutomationMetricsServiceClass:
    ...
    def test_concurrent_event_recording(self):
        ...
        # Create 4 threads
        threads = [
            threading.Thread(target=record_events, name=f"thread-{i}")
            for i in range(4)
        ]
