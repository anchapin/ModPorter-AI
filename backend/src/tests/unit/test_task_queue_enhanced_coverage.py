...
class TestAsyncTaskQueueCoverage:
    ...
    async def test_fail_dead_letter(self, queue, mock_redis):
        ...
        mock_redis.zadd.assert_called_once_with(queue._dead_letter_queue, {task_id: pytest.approx(time.time(), abs=1)})
    ...
    async def test_process_retry_queue(self, queue, mock_redis):
        ...
        mock_redis.zadd.assert_any_call("task_queue:high", {"t1": pytest.approx(time.time(), abs=1)})
        mock_redis.zadd.assert_any_call("task_queue:normal", {"t2": pytest.approx(time.time(), abs=1)})
