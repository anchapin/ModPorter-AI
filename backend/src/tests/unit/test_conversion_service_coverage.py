...
from services.conversion_service import ConversionService, get_conversion_service, process_conversion_task
...
class TestConversionServiceCoverage:
    ...
    async def test_process_conversion_success(self, service):
        ...
        async def mock_poll(*args, **kwargs):
            yield {"status": "in_progress", "progress": 50, "message": "Working", "current_stage": "test"}
        ...
        with patch.object(service, '_transfer_file_to_ai_engine', return_value="/ai/path"), \
    ...
    async def test_handle_error(self, service):
        with patch('services.conversion_service.ProgressHandler', new_callable=AsyncMock) as mock_ph:
