...
class TestTask:
    ...
    def test_task_creation(self):
        ...
        task = Task(id="test-id", name="test_task", payload={"key": "value"})
...
class TestTaskLifecycle:
    ...
    def test_task_state_transitions(self):
        ...
        # Initial state
        task = Task(id="test-id", name="test_task", payload={})
        assert task.status == TaskStatus.QUEUED
...
class TestRetryLogic:
    ...
    def test_max_delay_cap(self):
        ...
        policy = RetryPolicy(
            initial_delay_seconds=1.0, max_delay_seconds=10.0, backoff_multiplier=10.0
        )
