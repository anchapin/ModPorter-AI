...
async def test_update_progress(queue, mock_redis):
    ...
    progress_call = next(c for c in mock_redis.hset.call_args_list if f"{queue.PROGRESS_KEY}:{job_id}" in c[0][0])
...
async def test_complete_job(queue, mock_redis):
    ...
    results_call = next(c for c in mock_redis.hset.call_args_list if f"{queue.RESULTS_KEY}:{job_id}" in c[0][0])
    assert results_call[1]["mapping"]["bedrock_code"] == bedrock_code
    
    # Check job status update
    job_call = next(c for c in mock_redis.hset.call_args_list if f"{queue.JOBS_KEY}:{job_id}" in c[0][0])
