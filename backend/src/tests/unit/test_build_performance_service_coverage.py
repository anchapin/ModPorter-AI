...
class TestBuildPerformanceServiceComprehensive:
    ...
    def test_update_stage_new_and_existing(self, service):
        ...
        service.update_stage(build_id, BuildStages.INITIALIZATION, status="completed", metadata={"foo": "bar"})
        ...
        service.update_stage(build_id, BuildStages.FILE_TRANSFER, status="failed", error_message="Disk full")
    ...
    def test_end_tracking_success(self, service):
        ...
        with patch('services.metrics.record_build_performance', create=True) as mock_metrics, \
    ...
    def test_get_stats_comprehensive(self, service):
        ...
        build1 = BuildPerformanceMetrics(
            conversion_id="c1", status="completed", total_duration_ms=1000.0, performance_score=90.0,
            created_at=now - timedelta(minutes=10), completed_at=now - timedelta(minutes=9)
        )
        build1.stages.append(BuildStageTiming(stage_name="s1", start_time=now, end_time=now+timedelta(seconds=1), duration_ms=1000.0, status="completed"))
        
        build2 = BuildPerformanceMetrics(
            conversion_id="c1", status="failed", total_duration_ms=2000.0, performance_score=40.0,
            created_at=now - timedelta(minutes=5), completed_at=now - timedelta(minutes=4)
        )
        build2.stages.append(BuildStageTiming(stage_name="s1", start_time=now, end_time=now+timedelta(seconds=2), duration_ms=2000.0, status="failed"))
    ...
    def test_calculate_build_efficiency_edge_cases(self, service):
        ...
        build.stages.append(BuildStageTiming(stage_name="s1", start_time=datetime.now(timezone.utc), duration_ms=500.0))
