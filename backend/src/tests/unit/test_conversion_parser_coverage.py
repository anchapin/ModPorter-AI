...
class TestTransformPackToAddonData:
    ...
    @patch('os.listdir')
    @patch('os.walk')
    def test_transform_success_full(self, mock_walk, mock_listdir, mock_isdir, mock_parse_json, mock_find_folder, mock_uuid):
        ...
        def parse_json_side_effect(path):
            if "BP/manifest.json" in path: return bp_manifest
            if "RP/manifest.json" in path: return rp_manifest
        ...
        mock_isdir.side_effect = lambda p: p.endswith("BP/blocks") or p.endswith("BP/recipes") or p.endswith("RP/textures")
        mock_listdir.side_effect = lambda p: ["custom_block.json"] if p.endswith("blocks") else (["my_recipe.json"] if p.endswith("recipes") else [])
        ...
        assert any(a["type"] == "texture_block" and a["original_filename"] == "texture1.png" for a in assets_info)
        assert any(a["type"] == "texture_item" and a["original_filename"] == "item1.jpg" for a in assets_info)
    ...
    def test_transform_rp_only(self, mock_parse_json, mock_find_folder, mock_uuid):
        ...
        mock_find_folder.side_effect = lambda root, suffix: f"{root}/{suffix}" if suffix == "RP" else None
        
        rp_manifest = {
            "header": {
                "name": "My Pack Resource Pack",
                "description": "RP only description"
            }
        }
    ...
    def test_transform_nothing_found(self, mock_find_folder, mock_uuid):
        ...
        addon_data, assets_info = transform_pack_to_addon_data("/root", "Fallback", mock_uuid, user_id="test_user")
    ...
    @patch('os.path.isdir')
    @patch('os.listdir')
    def test_transform_block_missing_identifier(self, mock_listdir, mock_isdir, mock_parse_json, mock_find_folder, mock_uuid):
        """Test transform skip block if it missing identifier."""
        mock_find_folder.side_effect = lambda root, suffix: f"{root}/{suffix}" if suffix == "BP" else None
    ...
    @patch('os.path.isdir')
    @patch('os.listdir')
    def test_transform_block_missing_minecraft_block(self, mock_listdir, mock_isdir, mock_parse_json, mock_find_folder, mock_uuid):
