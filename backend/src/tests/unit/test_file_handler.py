...
async def test_validate_jar_valid(file_handler):
    ...
    with patch("os.path.exists", return_value=True), \
         patch("os.path.getsize", return_value=100), \
         patch("zipfile.ZipFile", return_value=MagicMock(__enter__=MagicMock(return_value=mock_zip))):
...
async def test_identify_mod_loader_fabric(file_handler):
    ...
    with patch("zipfile.ZipFile", return_value=MagicMock(__enter__=MagicMock(return_value=mock_zip))):
...
async def test_process_file_success(file_handler):
    ...
    with patch.object(file_handler, "validate_jar", return_value=ValidationResult(is_valid=True)), \
