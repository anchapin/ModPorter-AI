...
*deps: db, db.base, fastapi, pydantic, services.asset_conversion_service, sqlalchemy.ext.asyncio*
...
`async def get_asset(asset_id: str = Path(..., description='ID of the asset'), db: AsyncSession = Depends(get_db))`
...
`async def delete_asset(asset_id: str = Path(..., description='ID of the asset'), db: AsyncSession = Depends(get_db))`
  *Delete an asset and its associated file.*

`async def trigger_asset_conversion(asset_id: str = Path(..., description='ID of the asset to convert'), db: AsyncSession = Depends(get_db))`
  *Trigger conversion for a specific asset.*

`async def convert_all_conversion_assets(conversion_id: str = Path(..., description='ID of the conversion job'), db: AsyncSession = Depends(get_db))`
...
*deps: config, db.base, db.models, fastapi, fastapi.security, pydantic, security.auth, services.email_service*
...
`async def register(request_data: RegisterRequest, db: AsyncSession = Depends(get_db), _: bool = Depends(require_feature_flag('user_accou...)`
  *Register a new user account.*

`async def login(request_data: LoginRequest, db: AsyncSession = Depends(get_db), _: bool = Depends(require_feature_flag('user_accounts')))`
...
`async def start_batch_conversion(request: BatchConversionRequest, user_id: str, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db))`
...
`async def get_batch_status(batch_id: str, user_id: str, db: AsyncSession = Depends(get_db))`
  *Get batch conversion status.*

`async def get_batch_results(batch_id: str, user_id: str, db: AsyncSession = Depends(get_db))`
  *Get batch conversion results.*

`async def download_all_batch(batch_id: str, user_id: str, db: AsyncSession = Depends(get_db))`
  *Download all successful conversions as ZIP.*

`async def cancel_batch(batch_id: str, user_id: str, db: AsyncSession = Depends(get_db))`
  *Cancel batch conversion.*

---

### `backend/src/api/behavior_export.py`
*deps: db, db.base, fastapi, fastapi.responses, pydantic, services, services.cache, sqlalchemy.ext.asyncio*
...
`async def export_behavior_pack(request: ExportRequest, db: AsyncSession = Depends(get_db)) -> ExportResponse`
...
`async def preview_export(conversion_id: str = Path(..., description='Conversion job ID'), db: AsyncSession = Depends(get_db))`
...
`async def get_conversion_behavior_files(conversion_id: str = Path(..., description='Conversion job ID'), db: AsyncSession = Depends(get_db)) -> List[BehaviorFileTreeNode]`
...
`async def get_behavior_file(file_id: str = Path(..., description='Behavior file ID'), db: AsyncSession = Depends(get_db)) -> BehaviorFileResponse`
...
`async def delete_behavior_file(file_id: str = Path(..., description='Behavior file ID'), db: AsyncSession = Depends(get_db))`
...
`async def get_behavior_template(template_id: str = Path(..., description='Template ID'), db: AsyncSession = Depends(get_db)) -> BehaviorTemplateResponse`
  *Get a specific behavior template by ID.*

`async def create_behavior_template(request: BehaviorTemplateCreate = ..., user_id: Optional[str] = None, db: AsyncSession = Depends(get_db)) -> BehaviorTemplateResponse`
...
`async def delete_behavior_template(template_id: str = Path(..., description='Template ID'), db: AsyncSession = Depends(get_db))`
...
`async def create_behavioral_test(test_request: BehavioralTestRequest, background_tasks: BackgroundTasks)`
...
`async def get_admin_usage_metrics(db: AsyncSession = Depends(get_db))`
  *Get aggregate usage metrics for admin dashboard (Issue #977)*

`async def get_usage_by_tier(db: AsyncSession = Depends(get_db))`
...
*deps: db.base, db.models, engines.comparison_engine, fastapi, models.comparison, pydantic, services.error_handler, sqlalchemy.ext.asyncio*

**class CreateComparisonRequest(BaseModel):**

**class ComparisonResponse(BaseModel):**

`async def create_comparison(request: CreateComparisonRequest, session: AsyncSession = Depends(get_db)) -> ComparisonResponse`

**class FeatureMappingResponse(BaseModel):**

**class ComparisonResultResponse(BaseModel):**

`async def get_comparison_result(comparison_id_str: str, session: AsyncSession = Depends(get_db)) -> ComparisonResultResponse`

---

### `backend/src/api/conversions.py`
> Conversion API endpoints.
*deps: db, db.base, db.models, fastapi, fastapi.responses, fastapi.security, pydantic, security.auth*
...
`async def get_conversion(conversion_id: str, db: AsyncSession = Depends(get_db))`
...
`async def delete_conversion(conversion_id: str, db: AsyncSession = Depends(get_db))`
...
`async def complete_chunked_upload(upload_id: UUID, db: AsyncSession = Depends(get_db))`
...
*deps: ai_engine.search.cross_modal_retriever, ai_engine.search.multimodal_search_engine, ai_engine.utils.model_metadata_extractor, ai_engine.utils.texture_metadata_extractor, db, db.base, db.models, fastapi*
...
*deps: fastapi, pydantic, services.task_queue*
...
`async def list_tasks(status: Optional[str] = Query(None, description='Filter by status'), limit: int = Query(100, ge=1, le=1000, descripti...)`
...
*deps: api.auth, core.storage, fastapi, fastapi.responses, pydantic, services.audit_logger, services.file_handler*
...
*deps: models.conversion_mode, services.error_handler*
...
*deps: celery, celery.exceptions, redis, redis.asyncio, services.celery_config*
...
`def enqueue_task(name: str, payload: Dict[str, Any], priority: int = 1, max_retries: int = 3, timeout_seconds: int = 300) -> Dict[str, Any]`
...
`async def celery_enqueue(name: str, payload: Dict[str, Any], priority: TaskPriority = TaskPriority.NORMAL, max_retries: int = 3, timeout_secon...) -> TaskData`
...
### `backend/src/services/conversion_failure_analysis.py`
...
`def log_conversion_failure(job_id: str, error: Exception, error_category: str, conversion_stage: Optional[str] = None, mod_type: Optional[str] =...) -> ConversionFailure`
...
`def log_retry_failure(job_id: str, attempt: int, error: Exception, error_category: str)`
  *Log when a retry attempt fails*

---

### `backend/src/services/conversion_parser.py`
*deps: models*
...
`def transform_pack_to_addon_data(pack_root_path: str, addon_name_fallback: str, addon_id_override: uuid.UUID, user_id: Optional[str] = None) -> tuple[pydantic_addon_models.AddonDataUpload, List[Dict[str, str]]]`
  *Parses a converted addon pack directory (containing RP & BP)*

---

### `backend/src/services/conversion_queue.py`
...
**class ConversionJobQueue:**
  *Redis-based job queue for conversions.*
  ...
  `async def enqueue_job(self, user_id: str, java_code: str, mod_info: Dict[str, Any], options: Optional[Dict[str, Any]] = None, priority: int...) -> str`
  ...
  `async def update_progress(self, job_id: str, progress: int, current_stage: str, message: Optional[str] = None)`
    *Update job progress.*
  `async def complete_job(self, job_id: str, result: Dict[str, Any], bedrock_code: str)`
...
### `backend/src/services/conversion_report.py`
> Conversion Report Generator

**class ConversionReport:**
  *Conversion report for a single job.*
  `def __init__(self, job_id: str, java_code: str, bedrock_code: Optional[str] = None)`
...
**class ConversionReportGenerator:**
  *Generate conversion reports.*
  ...
  `def generate_report(self, job_id: str, java_code: str, bedrock_code: Optional[str] = None) -> ConversionReport`
...
*deps: httpx, services.ai_engine_client, services.cache, services.error_handler, src.websocket.progress_handler*
...
**class ConversionService:**
  *Service for handling conversion workflow with AI Engine integration.*
  `def __init__(self, ai_engine_client: Optional[AIEngineClient] = None, cache_service: Optional[CacheService] = None)`
  `async def _transfer_file_to_ai_engine(self, file_path: str, conversion_id: str) -> str`
    *Transfer the mod file to AI Engine.*
  `async def process_conversion(self, conversion_id: str, file_path: str, original_filename: str, target_version: str, options: Dict[str, Any]) -> Dict[str, Any]`
  ...
  `async def _update_job_status(self, conversion_id: str, status: str, progress: int, message: str) -> None`
    *Update job status in cache.*
  `async def _handle_error(self, conversion_id: str, error_message: str) -> None`
...
**class CurseForgeService:**
  *Service for interacting with CurseForge API*
  ...
  `async def search_mods(self, query: str, game_version: Optional[str] = None, category_id: Optional[int] = None, sort_order: str = 'popularit...) -> Dict[str, Any]`
    *Search for mods on CurseForge*
  `async def get_mod_info(self, mod_id: int) -> Dict[str, Any]`
    *Get detailed information about a specific mod*
  `async def get_mod_files(self, mod_id: int, game_version: Optional[str] = None, page_index: int = 0) -> Dict[str, Any]`
    *Get files/versions for a specific mod*
  `async def get_file_download_url(self, mod_id: int, file_id: int) -> str`
...
**class ResendEmailService:**
  *Resend email service.*
  `def __init__(self, api_key: str, from_email: str = 'noreply@portkit.cloud', from_name: str = 'Portkit')`
  ...
  `def _email_verification_plain(self, verification_url: str, expiry_hours: int = 24) -> str`
    *Email verification plain text template.*
  `def _email_verification_html(self, verification_url: str, expiry_hours: int = 24) -> str`
    *Email verification HTML template.*
  `def _password_reset_plain(self, reset_url: str, expiry_hours: int = 1, email: str = '') -> str`
    *Password reset plain text template.*
  `def _password_reset_html(self, reset_url: str, expiry_hours: int = 1, email: str = '') -> str`
    *Password reset HTML template.*
  `def _welcome_plain(self, user_name: str, email: str = '') -> str`
    *Welcome plain text template.*
  `def _welcome_html(self, user_name: str, email: str = '') -> str`
    *Welcome HTML template.*
  `def _conversion_complete_plain(self, conversion_id: str, download_url: str, success: bool, issues: Optional[List[str]] = None, email: str = '') -> str`
    *Conversion complete plain text template.*
  `def _conversion_complete_html(self, conversion_id: str, download_url: str, success: bool, issues: Optional[List[str]] = None, email: str = '') -> str`
    *Conversion complete HTML template.*

`def get_email_service(api_key: Optional[str] = None, from_email: str = 'noreply@portkit.cloud') -> ResendEmailService`
  *Get or create email service singleton.*

`async def send_verification_email(email: str, verification_token: str, expiry_hours: int = 24) -> bool`
  *Send email verification email.*

`async def send_password_reset_email(email: str, reset_token: str, expiry_hours: int = 1) -> bool`
  *Send password reset email.*

`async def send_welcome_email(email: str, user_name: str) -> bool`
  *Send welcome email.*

`async def send_conversion_notification(email: str, conversion_id: str, download_url: str, success: bool, issues: Optional[List[str]] = None) -> bool`
...
`def retry_with_backoff(max_retries: int = 3, base_delay: float = 1.0, max_delay: float = 60.0, exponential_base: float = 2.0, retryable_exce...)`
...
*deps: db, sqlalchemy.ext.asyncio*
...
**class ExperimentService:**
  *Service for managing A/B testing experiments.*
  ...
  `async def record_experiment_result(self, variant_id: uuid.UUID, session_id: uuid.UUID, kpi_quality: Optional[float] = None, kpi_speed: Optional[int] = N...) -> models.ExperimentResult`
...
**class FeatureFlag:**
  *Represents a single feature flag with its configuration.*
  `def __init__(self, name: str, flag_type: FeatureFlagType = FeatureFlagType.BOOLEAN, default_value: Any = False, description: str =...)`
  `def to_dict(self) -> Dict[str, Any]`
...
**class FeatureFlagManager:**
  *Central manager for feature flags.*
  ...
  `def register_flag(self, name: str, flag_type: FeatureFlagType = FeatureFlagType.BOOLEAN, default_value: Any = False, description: str =...) -> FeatureFlag`
...
### `backend/src/services/feedback_analytics.py`
...
**class FeedbackAnalyticsService:**
  *Analytics service for feedback data.*
  ...
  `def add_feature_request(self, feature: Dict[str, Any])`
    *Add feature request.*
  `def get_satisfaction_score(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]`
    *Get satisfaction score for date range.*
  `def get_feedback_by_type(self, start_date: datetime, end_date: datetime) -> Dict[str, int]`
    *Get feedback count by type.*
  `def get_bug_summary(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]`
    *Get bug report summary.*
  `def get_feature_request_summary(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]`
    *Get feature request summary.*
  `def get_conversion_feedback_correlation(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]`
    *Analyze correlation between conversion metrics and feedback.*
  `def _generate_insights(self, model_averages: Dict[str, float], duration_averages: Dict[str, float]) -> List[str]`
...
*deps: core.storage, services.audit_logger, services.malware_scanner, zipfile*
...
**class FileHandler:**
  *Service for handling JAR file operations.*
  ...
  `async def delete_uploaded_file(self, job_id: str, file_path: str, reason: str = 'conversion_complete') -> bool`
...
*deps: javalang, tree_sitter, tree_sitter_java*
