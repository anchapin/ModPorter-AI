...
class TestConversionInferenceEngine:
    ...
    async def test_infer_conversion_path_with_alternatives(
        self, engine, mock_db, sample_source_node
    ):
    ...
    async def test_infer_conversion_path_indirect_only(
        self, engine, mock_db, sample_source_node
    ):
        ...
        indirect_paths = [
            {
                ...
                "steps": [
                    ...
                    {
                        "source_concept": "Intermediate",
                        "target_concept": "Test_Bedrock",
                    },
                ],
                "path_length": 2,
                "supports_features": [],
    ...
    async def test_batch_infer_paths_success(self, engine, mock_db):
        ...
        with (
            ...
            mock_shared.return_value = [
                {"type": "relationship", "value": "converts_to"}
            ]
            mock_plan.return_value = {
                "total_groups": 1,
                "processing_groups": [
                    {"batch_number": 1, "concepts": ["TestEntity1", "TestEntity2"]}
                ],
    ...
    async def test_optimize_conversion_sequence_success(self, engine, mock_db):
        ...
        with (
            patch.object(engine, "_build_dependency_graph") as mock_build,
    ...
    async def test_learn_from_conversion_success(self, engine, mock_db):
        ...
        with (
            ...
            mock_update.return_value = {"confidence_updates": 1, "new_relationships": 0}
    ...
    async def test_find_concept_node_success(self, engine, mock_db, sample_source_node):
        ...
        with patch(
            "src.services.conversion_inference.KnowledgeNodeCRUD.search"
        ) as mock_search:
            mock_search.return_value = [sample_source_node]

            await engine._find_concept_node(mock_db, "TestEntity", "java", "1.20")
    ...
    async def test_find_direct_paths_success(self, engine, mock_db, sample_source_node):
        ...
        with patch(
            "src.services.conversion_inference.graph_db.find_conversion_paths"
        ) as mock_find:
            mock_paths = [
                {
                    "path_length": 1,
                    "confidence": 0.9,
                    "end_node": {"name": "TestEntity_Bedrock", "platform": "bedrock"},
    ...
    async def test_rank_paths_confidence(self, engine, mock_db):
        ...
        result = await engine._rank_paths(paths, "confidence", mock_db, "1.20")
    ...
    async def test_rank_paths_speed(self, engine, mock_db):
        ...
        result = await engine._rank_paths(paths, "speed", mock_db, "1.20")
    ...
    async def test_rank_paths_features(self, engine, mock_db):
        ...
        paths = [
            {"confidence": 0.7, "path_length": 2, "supports_features": ["feature1"]},
            {
                "confidence": 0.9,
                "path_length": 1,
                "supports_features": ["feature1", "feature2"],
            },
            {"confidence": 0.8, "path_length": 1, "supports_features": []},
        ]

        result = await engine._rank_paths(paths, "features", mock_db, "1.20")
    ...
    async def test_optimize_processing_order(self, engine):
        ...
        result = await engine._optimize_processing_order(concept_paths, path_analysis)
    ...
    async def test_build_dependency_graph(self, engine, mock_db):
        ...
        result = await engine._build_dependency_graph(concepts, dependencies, mock_db)
    ...
    async def test_calculate_savings(self, engine, mock_db):
        ...
        processing_groups = [{"estimated_time": 0.5}, {"estimated_time": 0.3}]
    ...
    def test_calculate_improvement_percentage(self, engine):
        ...
        result = engine._calculate_improvement_percentage(
            [{"confidence": 0.7}], [{"enhanced_accuracy": 0.85}]
        )
    ...
    def test_simulate_ml_scoring(self, engine):
        ...
        features = {"base_confidence": 0.8, "path_length": 2, "complexity": "low"}
    ...
    async def test_enhance_conversion_accuracy(self, engine, mock_db):
        ...
        with (
            patch.object(engine, "_validate_conversion_pattern") as mock_pattern,
    ...
    def test_edge_cases_extreme_values(self, engine):
        ...
        high_conf_paths = [
            {"confidence": 1.0, "path_length": 1, "supports_features": []}
        ]

        asyncio.run(
            engine._rank_paths(high_conf_paths, "confidence", AsyncMock(), "latest")
        )

        # Very low confidence
        low_conf_paths = [
            {"confidence": 0.0, "path_length": 10, "supports_features": []}
        ]

        asyncio.run(
            engine._rank_paths(low_conf_paths, "confidence", AsyncMock(), "latest")
        )
