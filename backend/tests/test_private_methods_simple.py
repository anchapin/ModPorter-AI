...
class TestFindDirectPaths:
    ...
    @pytest.mark.asyncio
    async def test_find_direct_paths_basic_success(self, engine, mock_db, mock_source_node):
...
class TestFindIndirectPaths:
    ...
    @pytest.mark.asyncio
    async def test_find_indirect_paths_basic_success(self, engine, mock_db, mock_source_node):
    ...
    @pytest.mark.asyncio
    async def test_find_indirect_paths_depth_filtering(self, engine, mock_db, mock_source_node):
    ...
    @pytest.mark.asyncio
    async def test_find_indirect_paths_confidence_filtering(self, engine, mock_db, mock_source_node):
...
class TestEnhanceConversionAccuracy:
    ...
    async def test_enhance_conversion_accuracy_basic_success(self, engine):
        ...
        engine._validate_conversion_pattern = Mock(return_value={"valid": True, "issues": []})
...
class TestValidationMethods:
    ...
    def test_validate_conversion_pattern_valid(self, engine):
        ...
        engine._validate_conversion_pattern = lambda pattern: {
            "valid": True,
            "issues": []
        } if (0 <= pattern.get("confidence", 0) <= 1 and 
                pattern.get("steps") and len(pattern["steps"]) > 0) else {
            "valid": False,
            "issues": ["Invalid confidence or missing steps"]
        }
