...
class TestConversionInferenceEngineUncoveredMethods:
    ...
    @pytest.mark.asyncio
    async def test_find_direct_paths_platform_both(
    ...
    @pytest.mark.asyncio
    async def test_find_direct_paths_no_matching_platform(
        self, engine, mock_db, sample_source_node
    ):
    ...
    @pytest.mark.asyncio
    async def test_find_direct_paths_filter_by_depth(
        self, engine, mock_db, sample_source_node
    ):
    ...
    @pytest.mark.asyncio
    async def test_find_direct_paths_sorted_by_confidence(
        self, engine, mock_db, sample_source_node
    ):
    ...
    @pytest.mark.asyncio
    async def test_find_indirect_paths_filter_by_min_confidence(
        self, engine, mock_db, sample_source_node
    ):
    ...
    @pytest.mark.asyncio
    async def test_find_indirect_paths_error_handling(
        self, engine, mock_db, sample_source_node
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_conversion_pattern_missing_required_fields(
        self, engine, mock_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_conversion_pattern_invalid_confidence(
        self, engine, mock_db
    ):
        ...
        assert result["is_valid"] is False
        assert any(
            "confidence" in error.lower() for error in result["validation_errors"]
        )
    ...
    async def test_check_platform_compatibility_unsupported_version(self, engine):
        ...
        result = await engine._check_platform_compatibility(
            "java", "bedrock", "0.16"
        )  # Very old version
