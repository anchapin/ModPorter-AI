...
from src.services.report_models import FeatureConversionDetail, AssumptionDetail
...
class TestConversionReportGenerator(unittest.TestCase):
    ...
    def test_generate_summary_report_success(self):
        ...
        self.assertEqual(
            summary["overall_success_rate"], self.success_data["overall_success_rate"]
        )
    ...
    def test_generate_summary_report_failure(self):
        ...
        self.assertEqual(
            summary["overall_success_rate"], self.failure_data["overall_success_rate"]
        )
        ...
        self.assertEqual(
            summary["download_url"], self.failure_data.get("download_url")
        )  # Expecting None here based on mock

    def test_generate_feature_analysis(self):
        analysis = self.generator.generate_feature_analysis(
            self.success_data["features_data"]
        )
        ...
        if analysis["per_feature_status"]:  # Check if list is not empty
            first_feature = cast(
                FeatureConversionDetail, analysis["per_feature_status"][0]
            )
            self.assertEqual(
                first_feature["feature_name"],
                self.success_data["features_data"][0]["feature_name"],
            )

    def test_generate_assumptions_report(self):
        assumptions = self.generator.generate_assumptions_report(
            self.success_data["assumptions_detail_data"]
        )
        ...
        if assumptions["assumptions"]:  # Check if list is not empty
            ...
            self.assertEqual(
                first_assumption["assumption_id"],
                self.success_data["assumptions_detail_data"][0]["assumption_id"],
            )

    def test_generate_developer_log(self):
        dev_log = self.generator.generate_developer_log(
            self.success_data["developer_logs_data"]
        )
        self.assertIsInstance(dev_log, dict)
        self.assertTrue(len(dev_log["code_translation_details"]) > 0)
        self.assertEqual(
            dev_log["performance_metrics"]["total_time_seconds"],
            self.success_data["developer_logs_data"]["performance"][
                "total_time_seconds"
            ],
        )
    ...
    def test_create_interactive_report_success(self):
        ...
        self.assertEqual(
            ...
            self.success_data["overall_success_rate"],
        )
    ...
    def test_create_interactive_report_failure(self):
        ...
        self.assertEqual(
            report["summary"]["overall_success_rate"],
            self.failure_data["overall_success_rate"],
        )
        self.assertTrue(len(report["failed_mods"]) > 0)

    def test_create_full_conversion_report_prd_style(self):
        report = self.generator.create_full_conversion_report_prd_style(
            self.success_data
        )
        self.assertIsInstance(report, dict)
        self.assertIsInstance(report["summary"], dict)
        ...
        if self.success_data.get("smart_assumptions_data"):
            self.assertEqual(
                len(report["smart_assumptions"]),
                len(self.success_data["smart_assumptions_data"]),
            )
        self.assertIsInstance(report["developer_log"], dict)
