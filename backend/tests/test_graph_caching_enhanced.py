...
class TestGraphCachingServiceAdvanced:
    ...
    def service(self):
        ...
        config = CacheConfig(
            max_entries=100, max_size_mb=10.0, ttl_seconds=60, enable_compression=True
        )
    ...
    async def test_cache_strategy_selection(self, service):
        ...
        # Test LRU strategy
        result_lru = await service._apply_cache_strategy(CacheStrategy.LRU, test_data)
        assert result_lru is not None

        # Test LFU strategy
        result_lfu = await service._apply_cache_strategy(CacheStrategy.LFU, test_data)
        assert result_lfu is not None

        # Test FIFO strategy
        result_fifo = await service._apply_cache_strategy(CacheStrategy.FIFO, test_data)
    ...
    async def test_cache_warm_up(self, service, mock_db):
        ...
        with patch.object(
            service, "_load_data_for_warm_up", new=AsyncMock(return_value=mock_data)
        ):
            # Warm up cache
...
class TestCacheRecoveryAndResilience:
    ...
    async def test_partial_cache_corruption_recovery(self, service):
        ...
        with patch.object(
            service,
            "_deserialize_data",
            side_effect=[Exception("Corrupted"), "valid_value"],
        ):
