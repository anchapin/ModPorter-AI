...
class TestBatchJobManagement:
    ...
    async def test_submit_batch_job_success(self, mock_db):
        ...
        with patch('src.api.batch.batch_processing_service') as mock_service:
            from src.api.batch import submit_batch_job, BatchOperationType, ProcessingMode
...
class TestBatchImportOperations:
    ...
    async def test_import_nodes_success(self, mock_db):
        ...
        with patch('src.api.batch.batch_processing_service') as mock_service:
            ...
            # Setup correct async mock
            mock_service.import_nodes = AsyncMock(return_value={
                "imported_count": 2,
                "skipped_count": 0,
                "errors": []
            })
    ...
    async def test_import_relationships_success(self, mock_db):
        ...
        with patch('src.api.batch.batch_processing_service') as mock_service:
            ...
            # Setup correct async mock
            mock_service.import_relationships = AsyncMock(return_value={
                "imported_count": 2,
                "skipped_count": 0,
                "errors": []
            })
...
class TestBatchErrorHandling:
    ...
    async def test_service_timeout_error(self, mock_db):
        ...
        with patch('src.api.batch.batch_processing_service') as mock_service:
            ...
            # Setup correct async mock
            mock_service.submit_batch_job = AsyncMock(side_effect=asyncio.TimeoutError("Service timeout"))
    ...
    async def test_database_connection_error(self, mock_db):
        ...
        with patch('src.api.batch.batch_processing_service') as mock_service:
            ...
            # Setup correct async mock
            mock_service.get_active_jobs = AsyncMock(side_effect=Exception("Database connection failed"))
    ...
    async def test_insufficient_permissions_error(self, mock_db):
        ...
        with patch('src.api.batch.batch_processing_service') as mock_service:
            ...
            # Setup correct async mock
            mock_service.delete_nodes = AsyncMock(side_effect=PermissionError("Insufficient permissions"))
...
class TestBatchUtilityFunctions:
    ...
    async def test_get_operation_description(self):
        ...
        with patch('src.api.batch.batch_processing_service') as mock_service:
            ...
            operations = ["import_nodes", "import_relationships", "export_graph", "delete_nodes"]
