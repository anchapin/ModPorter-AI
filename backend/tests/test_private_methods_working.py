...
class TestFindDirectPathsMinimal:
    ...
    @pytest.mark.asyncio
    async def test_find_direct_paths_with_successful_path(
        self, engine, mock_source_node
    ):
    ...
    @pytest.mark.asyncio
    async def test_find_direct_paths_with_database_error(
        self, engine, mock_source_node
    ):
...
class TestFindIndirectPathsMinimal:
    ...
    @pytest.mark.asyncio
    async def test_find_indirect_paths_with_successful_path(
        self, engine, mock_source_node
    ):
    ...
    @pytest.mark.asyncio
    async def test_find_indirect_paths_filtered_by_depth(
        self, engine, mock_source_node
    ):
    ...
    @pytest.mark.asyncio
    async def test_find_indirect_paths_filtered_by_confidence(
        self, engine, mock_source_node
    ):
    ...
    async def test_enhance_conversion_accuracy_with_valid_paths(self, engine):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    engine,
                    "_refine_with_ml_predictions",
                    return_value={"enhanced_confidence": 0.82},
                ):
                    with patch.object(
                        engine,
                        "_integrate_community_wisdom",
                        ...
                        with patch.object(
                            ...
                            with patch.object(
                                ...
                                assert (
                                    summary["original_avg_confidence"] == 0.675
                                )  # (0.75 + 0.60) / 2
