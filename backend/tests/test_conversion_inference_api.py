...
class TestConversionInferenceAPI:
    ...
    def sample_batch_request(self):
        ...
        return {
            ...
            "target_platform": "bedrock",
            "minecraft_version": "1.20",
            "path_options": {"max_depth": 4, "optimize_for": "speed"},
        }
    ...
    async def test_infer_conversion_path_success(
        self, client, mock_db, sample_inference_request
    ):
        """Test successful conversion path inference."""
        with (
            patch("backend.src.api.conversion_inference.get_db") as mock_get_db,
            patch.object(
                conversion_inference_engine, "infer_conversion_path"
            ) as mock_infer,
        ):
    ...
    def test_infer_conversion_path_missing_java_concept(self, client, mock_db):
        ...
        with patch("backend.src.api.conversion_inference.get_db") as mock_get_db:
            ...
            request_data = {"target_platform": "bedrock", "minecraft_version": "1.20"}

            response = client.post("/inference/path", json=request_data)

            assert response.status_code == 422  # Validation error

    async def test_infer_conversion_path_service_error(
        self, client, mock_db, sample_inference_request
    ):
    ...
    async def test_infer_conversion_path_not_found(
        self, client, mock_db, sample_inference_request
    ):
    ...
    async def test_batch_infer_paths_success(
        self, client, mock_db, sample_batch_request
    ):
        """Test successful batch path inference."""
        with (
            patch("backend.src.api.conversion_inference.get_db") as mock_get_db,
            patch.object(
                conversion_inference_engine, "batch_infer_paths"
            ) as mock_batch,
        ):
            mock_get_db.return_value = mock_db
            mock_batch.return_value = {
                ...
                "batch_results": {
                    ...
                    "TestEntity3": {"success": False, "error": "Concept not found"},
                },
                "batch_analysis": {
                ...
                "optimization_plan": {
                    ...
                    "batch_operations": [
                        {
                            "concepts": ["TestEntity1", "TestEntity2"],
    ...
    def test_batch_infer_paths_service_error(
        self, client, mock_db, sample_batch_request
    ):
    ...
    async def test_optimize_conversion_sequence_success(
        self, client, mock_db, sample_sequence_request
    ):
        """Test successful conversion sequence optimization."""
        with (
            patch("backend.src.api.conversion_inference.get_db") as mock_get_db,
            patch.object(
                conversion_inference_engine, "optimize_conversion_sequence"
            ) as mock_optimize,
        ):
            mock_get_db.return_value = mock_db
            ...
            response = client.post(
                "/inference/optimize-sequence", json=sample_sequence_request
            )
    ...
    async def test_learn_from_conversion_success(
        self, client, mock_db, sample_learning_request
    ):
        """Test successful learning from conversion results."""
        with (
            patch("backend.src.api.conversion_inference.get_db") as mock_get_db,
            patch.object(
                conversion_inference_engine, "learn_from_conversion"
            ) as mock_learn,
        ):
            mock_get_db.return_value = mock_db
    ...
    def test_learn_from_conversion_service_error(
        self, client, mock_db, sample_learning_request
    ):
    ...
    async def test_get_inference_statistics_success(self, client, mock_db):
        ...
        with (
            patch("backend.src.api.conversion_inference.get_db") as mock_get_db,
            patch.object(
                conversion_inference_engine, "get_inference_statistics"
            ) as mock_stats,
        ):
            mock_get_db.return_value = mock_db
...
class TestConversionInferenceAPIEdgeCases:
    ...
    def test_very_long_concept_names(self, client, mock_db):
        ...
        request_data = {"java_concept": long_name, "target_platform": "bedrock"}
    ...
    def test_concurrent_requests(self, client, mock_db):
        ...
        request_data = {"java_concept": "TestEntity", "target_platform": "bedrock"}
    ...
    def test_timeout_scenarios(self, client, mock_db):
        ...
        request_data = {"java_concept": "ComplexEntity", "target_platform": "bedrock"}
