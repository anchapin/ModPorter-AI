...
class TestCollaborationSession:
    ...
    def test_collaboration_session_creation(self):
        ...
        session = CollaborationSession(
            session_id="session_123", graph_id="graph_456", created_at=datetime.utcnow()
        )
...
class TestRealtimeCollaborationService:
    ...
    async def test_create_collaboration_session_success(self, service, mock_db):
        ...
        result = await service.create_collaboration_session(
            graph_id="graph_123", user_id="user_456", user_name="Test User", db=mock_db
        )
    ...
    async def test_create_collaboration_session_error(self, service, mock_db):
        ...
        with patch.object(
            service,
            "_generate_user_color",
            side_effect=Exception("Color generation failed"),
        ):
            result = await service.create_collaboration_session(
    ...
    async def test_join_collaboration_session_success(
        self, service, mock_websocket, mock_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_join_collaboration_session_not_found(
        self, service, mock_websocket, mock_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_join_collaboration_session_already_joined(
        self, service, mock_websocket, mock_db
    ):
    ...
    async def test_leave_collaboration_session_success(self, service, mock_db):
        ...
        result = await service.leave_collaboration_session(
            user_id="leaving_user", db=mock_db
        )
    ...
    async def test_leave_collaboration_session_not_in_session(self, service):
        ...
        result = await service.leave_collaboration_session(user_id="nonexistent_user")
    ...
    async def test_leave_collaboration_session_with_other_users(self, service, mock_db):
        ...
        # Add second user
        with patch("src.services.realtime_collaboration.WebSocket"):
            ...
            # First user leaves
            result = await service.leave_collaboration_session(
                user_id="creator_multi", db=mock_db
            )
            ...
            assert session_id in service.active_sessions
            assert (
                "creator_multi" not in service.active_sessions[session_id].active_users
            )
    ...
    async def test_apply_operation_success(self, service, mock_db):
        ...
        # Create session
        create_result = await service.create_collaboration_session(
            graph_id="graph_op", user_id="op_user", user_name="Operator", db=mock_db
        )
        session_id = create_result["session_id"]
        ...
        with (
            ...
            patch.object(
                service,
                "_execute_operation",
                return_value={"type": "node_created", "node_id": "node_123"},
            ),
    ...
    async def test_apply_operation_conflict(self, service, mock_db):
        ...
        with (
            patch.object(service, "_detect_conflicts", return_value=conflicts),
            patch.object(service, "_broadcast_message") as mock_broadcast,
        ):
            result = await service.apply_operation(
                session_id=session_id,
    ...
    async def test_get_session_state_success(self, service, mock_db):
        ...
        with (
            patch.object(
                service, "_execute_operation", return_value={"type": "node_created"}
            ),
            patch.object(service, "_detect_conflicts", return_value=[]),
        ):
        ...
        with patch.object(
            service, "_get_graph_state", return_value={"nodes": [], "relationships": []}
        ):
    ...
    async def test_get_user_activity_success(self, service, mock_db):
        ...
        with (
            patch.object(
                service, "_execute_operation", return_value={"type": "success"}
            ),
            patch.object(service, "_detect_conflicts", return_value=[]),
        ):
            for i, (op_type, data, target_id) in enumerate(operations_data):
                ...
                operation = CollaborativeOperation(
                    ...
                    user_name="Activity User",
                    timestamp=base_time
                    + timedelta(minutes=i - 5),  # Spread across 5 minutes
                    target_id=target_id,
                    data=data,
        ...
        result = await service.get_user_activity(
            session_id=session_id, user_id="activity_user", minutes=30
        )
    ...
    async def test_get_user_activity_not_in_session(self, service, mock_db):
        ...
        result = await service.get_user_activity(
            session_id=session_id, user_id="nonexistent_user", minutes=30
        )
    ...
    async def test_handle_websocket_message_cursor_position(self, service, mock_db):
        ...
        result = await service.handle_websocket_message(
            user_id="cursor_user", message=message, db=mock_db
        )
        ...
        session = service.active_sessions[session_id]
        assert session.active_users["cursor_user"]["cursor_position"] == {
            "x": 100,
            "y": 200,
            "node_id": "node_123",
        }
    ...
    async def test_handle_websocket_message_operation(self, service, mock_db):
        ...
        with (
            ...
            patch.object(
                service,
                "_execute_operation",
                return_value={"type": "node_created", "node_id": "ws_node_123"},
            ),
            patch.object(service, "_broadcast_message"),
        ):
            result = await service.handle_websocket_message(
                user_id="ws_op_user", message=message, db=mock_db
            )
    ...
    async def test_handle_websocket_message_unknown_type(self, service, mock_db):
        ...
        result = await service.handle_websocket_message(
            user_id="unknown_user", message=message, db=mock_db
        )
    ...
    async def test_handle_websocket_disconnect(self, service):
        ...
        service.active_sessions[session_id].websockets["disconnect_user"] = (
            mock_websocket
        )
        ...
        assert (
            service.active_sessions[session_id].active_users["disconnect_user"][
                "status"
            ]
            == "disconnected"
        )
    ...
    async def test_get_current_data_node(self, service, mock_db):
        ...
        with patch(
            "src.services.realtime_collaboration.KnowledgeNodeCRUD.get_by_id",
    ...
    async def test_get_current_data_pattern(self, service, mock_db):
        ...
        with patch(
            "src.services.realtime_collaboration.ConversionPatternCRUD.get_by_id",
    ...
    async def test_execute_operation_create_node(self, service, mock_db):
        ...
        with patch(
            "src.services.realtime_collaboration.KnowledgeNodeCRUD.create",
            return_value=mock_node,
        ):
    ...
    async def test_execute_operation_update_node(self, service, mock_db):
        ...
        with patch(
            "src.services.realtime_collaboration.KnowledgeNodeCRUD.update",
            return_value=True,
        ):
            result = await service._execute_operation(operation, mock_db)
    ...
    async def test_execute_operation_delete_node(self, service, mock_db):
        ...
        with patch(
            "src.services.realtime_collaboration.KnowledgeNodeCRUD.delete",
            return_value=True,
        ):
            result = await service._execute_operation(operation, mock_db)
...
class TestServiceIntegration:
    ...
    async def test_full_collaboration_workflow(self):
        ...
        activity_result = await service.get_user_activity(
            session_id=session_id, user_id="creator_user", minutes=60
        )
        ...
        leave_result = await service.leave_collaboration_session(
            user_id="collaborator_user", db=mock_db
        )
