...
class TestAutomatedConfidenceScoringService:
    ...
    def service(self):
        ...
        with patch.dict('sys.modules', {
            ...
            from src.services.automated_confidence_scoring import AutomatedConfidenceScoringService
    ...
    async def test_update_confidence_from_feedback(self, service, mock_db):
        ...
        with patch.object(service, 'models') as mock_models:
            ...
            result = await service.update_confidence_from_feedback(feedback_data, mock_db)
    ...
    async def test_get_confidence_trends(self, service, mock_db):
        ...
        with patch('src.services.automated_confidence_scoring.ConfidenceAssessmentCRUD') as mock_crud:
    ...
    async def test_get_assessment_history(self, service, mock_db):
        ...
        mock_history = [
            Mock(assessment_id="assess1", confidence_score=0.82, timestamp=datetime.now()),
            Mock(assessment_id="assess2", confidence_score=0.85, timestamp=datetime.now()),
            Mock(assessment_id="assess3", confidence_score=0.88, timestamp=datetime.now())
        ]
    ...
    def test_model_training_features(self, service):
        ...
        training_data = [
            {
                "features": {"code_complexity": 0.7, "api_usage": 0.8},
                "target": 0.85
            },
            {
                "features": {"code_complexity": 0.6, "api_usage": 0.7},
                "target": 0.72
            }
        ]
