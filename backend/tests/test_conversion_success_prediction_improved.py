...
class TestConversionSuccessPrediction:
    ...
    async def test_predict_conversion_success_basic(self, service, mock_db):
        ...
        with patch.object(service, 'is_trained', True):
            with patch.object(service.models.get('overall_success', Mock()), 'predict', return_value=[0.85]):
    ...
    async def test_train_model_success(self, service, mock_db):
        ...
        with patch('src.services.conversion_success_prediction.train_test_split', return_value=(training_data, [])):
            with patch.object(service.models.get('overall_success', Mock()), 'fit'):
                await service.train_model(training_data, PredictionType.OVERALL_SUCCESS, mock_db)
    ...
    async def test_get_model_performance_metrics(self, service, mock_db):
        ...
        with patch.object(service, 'is_trained', True):
            with patch('src.services.conversion_success_prediction.cross_val_score', return_value=[0.82, 0.85, 0.81, 0.83, 0.84]):
                with patch('src.services.conversion_success_prediction.accuracy_score', return_value=0.83):
    ...
    async def test_save_prediction_result(self, service, mock_db):
        ...
        with patch('src.services.conversion_success_prediction.ConversionPatternCRUD') as mock_crud:
    ...
    async def test_update_model_with_feedback(self, service, mock_db):
        ...
        with patch.object(service, 'is_trained', True):
            with patch.object(service.models.get('overall_success', Mock()), 'partial_fit'):
    ...
    async def test_export_model(self, service, mock_db):
        ...
        with patch.object(service, 'is_trained', True):
            with patch('src.services.conversion_success_prediction.joblib.dump') as mock_dump:
...
class TestPredictionTypeFeatures:
    ...
    def service(self):
        ...
        with patch.dict('sys.modules', {
            ...
            from src.services.conversion_success_prediction import ConversionSuccessPredictionService
            return ConversionSuccessPredictionService()
