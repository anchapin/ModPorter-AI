...
class TestEnhanceConversionAccuracy:
    ...
    @pytest.mark.asyncio
    async def test_enhance_conversion_accuracy_success_basic(
        self, engine, sample_conversion_paths, sample_context_data, mock_db
    ):
        """Test enhance_conversion_accuracy with successful basic enhancement"""
        # Mock all helper methods to return predictable scores
        with (
            patch.object(
                engine, "_validate_conversion_pattern", return_value=0.85
            ) as mock_validate,
            patch.object(
                engine, "_check_platform_compatibility", return_value=0.90
            ) as mock_compatibility,
            patch.object(
                engine, "_refine_with_ml_predictions", return_value=0.80
            ) as mock_ml,
            patch.object(
            ...
            patch.object(
                engine, "_generate_accuracy_suggestions", return_value=["suggestion1"]
            ) as mock_suggestions,
        ):
    ...
    @pytest.mark.asyncio
    async def test_enhance_conversion_accuracy_no_context(
        self, engine, sample_conversion_paths, mock_db
    ):
        """Test enhance_conversion_accuracy without context data"""
        with (
            patch.object(engine, "_validate_conversion_pattern", return_value=0.80),
    ...
    @pytest.mark.asyncio
    async def test_enhance_conversion_accuracy_no_database(
        self, engine, sample_conversion_paths, sample_context_data
    ):
        """Test enhance_conversion_accuracy without database connection"""
        with (
            patch.object(engine, "_validate_conversion_pattern", return_value=0.70),
            patch.object(engine, "_check_platform_compatibility", return_value=0.75),
            ...
            patch.object(engine, "_optimize_for_performance", return_value=0.85),
            patch.object(
                engine, "_generate_accuracy_suggestions", return_value=["test"]
            ),
        ):
            result = await engine.enhance_conversion_accuracy(
    ...
    async def test_enhance_conversion_accuracy_single_path(self, engine, mock_db):
        ...
        with (
            patch.object(engine, "_validate_conversion_pattern", return_value=0.90),
    ...
    @pytest.mark.asyncio
    async def test_enhance_conversion_accuracy_confidence_bounds(
        self, engine, sample_conversion_paths, sample_context_data, mock_db
    ):
        ...
        with (
            patch.object(engine, "_validate_conversion_pattern", return_value=1.5),
    ...
    @pytest.mark.asyncio
    async def test_enhance_conversion_accuracy_error_handling(
        self, engine, sample_conversion_paths, mock_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_enhance_conversion_accuracy_ranking(
        self, engine, sample_conversion_paths, sample_context_data, mock_db
    ):
        ...
        with (
            patch.object(
                engine, "_validate_conversion_pattern", side_effect=side_effect_validate
            ),
            patch.object(engine, "_check_platform_compatibility", return_value=0.80),
            ...
            assert (
                enhanced_paths[0]["enhanced_accuracy"]
                >= enhanced_paths[1]["enhanced_accuracy"]
            )
...
class TestHelperMethods:
    ...
    async def test_validate_conversion_pattern_with_database(self, engine, mock_db):
        ...
        with patch(
            "src.services.conversion_inference.ConversionPatternCRUD"
        ) as mock_crud:
            mock_crud.get_by_type = AsyncMock(
                return_value=[mock_pattern1, mock_pattern2]
            )
    ...
    @pytest.mark.asyncio
    async def test_validate_conversion_pattern_no_database_results(
        self, engine, mock_db
    ):
    ...
    async def test_generate_accuracy_suggestions_low_score(self, engine):
        ...
        assert any(
            "alternative conversion patterns" in suggestion for suggestion in result
        )
        assert any(
            "more recent Minecraft versions" in suggestion for suggestion in result
        )
