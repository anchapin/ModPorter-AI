...
class TestProgressiveLoadingService:
    ...
    @pytest.mark.asyncio
    async def test_start_progressive_load_success(self, service, mock_db, sample_viewport):
    ...
    async def test_start_progressive_load_error(self, service, mock_db):
        ...
        with patch.object(service, '_estimate_total_items', side_effect=Exception("Database error")):
            result = await service.start_progressive_load(
    ...
    @pytest.mark.asyncio
    async def test_preload_adjacent_areas_success(self, service, sample_viewport, mock_db):
    ...
    async def test_get_loading_statistics_all(self, service):
        ...
        service.viewport_history = {"viz1": [ViewportInfo(0, 0, 1, 100, 100)], "viz2": [ViewportInfo(50, 50, 2, 200, 200)]}
        ...
        assert result["statistics"]["viewport_history"]["visualizations_with_history"] == 2
    ...
    async def test_execute_lod_loading(self, service, mock_db):
        ...
        with patch.object(service, '_load_medium_chunk', 
                       return_value=[{"id": i} for i in range(100)]), \
             patch.object(service, '_load_medium_chunk', 
    ...
    async def test_execute_distance_based_loading(self, service, mock_db):
        ...
        with patch.object(service, '_load_distance_chunk', 
    ...
    async def test_execute_importance_based_loading(self, service, mock_db):
        ...
        with patch.object(service, '_load_importance_chunk', 
    ...
    async def test_execute_cluster_based_loading(self, service, mock_db):
        ...
        with patch.object(service, '_load_cluster_chunk', 
                       return_value=[{"id": i} for i in range(25)]):
    ...
    def test_cleanup_expired_caches(self, service):
        ...
        expired_cache.last_accessed = datetime.utcnow() - timedelta(seconds=400)  # Older than 300s TTL
...
class TestErrorHandling:
    ...
    async def test_loading_task_execution_error(self, service, mock_db):
        ...
        with patch.object(service, '_load_medium_chunk', side_effect=Exception("Loading error")):
            result = await service._execute_lod_loading(task, mock_db)
