...
class TestConversionInferencePrivateMethods:
    ...
    async def test_find_direct_paths_with_results(
        ...
        mock_graph_db = Mock()
        mock_graph_db.find_conversion_paths = Mock(
            return_value=[
                {
                    "path_length": 1,
                    "confidence": 0.85,
    ...
    @pytest.mark.asyncio
    async def test_find_indirect_paths_min_confidence_filter(
        self, engine, mock_db, mock_source_node
    ):
    ...
    @pytest.mark.asyncio
    async def test_find_indirect_paths_platform_filtering(
        self, engine, mock_db, mock_source_node
    ):
    ...
    def test_check_platform_compatibility_compatible(self, engine):
        ...
        path = {
            "steps": [{"platform": "java"}, {"platform": "both"}],
...
class TestConversionInferenceOptimizationMethods:
    ...
    async def test_optimize_conversion_sequence_complete(self, engine):
        ...
        engine._identify_shared_steps = Mock(
            return_value=["shared_step1", "shared_step2"]
        )
        engine._estimate_batch_time = Mock(return_value=12.5)
        engine._get_batch_optimizations = Mock(
            return_value=["parallel_processing", "caching"]
        )
        engine._generate_validation_steps = Mock(
            return_value=["validate_step1", "validate_step2"]
        )
    ...
    def test_estimate_batch_time_with_optimizations(self, engine):
        ...
        with patch.object(
            engine, "_get_batch_optimizations", return_value=["parallel_processing"]
        ):
            result = engine._estimate_batch_time(conversion_sequence)
            ...
            return_value=[
                {
                    "path_length": 3,
                    "confidence": 0.75,
                    "end_node": {"name": "target_node", "platform": "bedrock"},
                    "nodes": [
...
class TestConversionInferenceRemainingPrivateMethods:
    ...
    async def test_simulate_ml_scoring_low_confidence(self, engine):
        ...
        features = {"base_confidence": 0.4, "path_length": 5, "complexity": "high"}
    ...
    async def test_store_learning_event(self, engine, mock_db):
        ...
        event = {"type": "conversion_completed", "success": True, "confidence": 0.85}
