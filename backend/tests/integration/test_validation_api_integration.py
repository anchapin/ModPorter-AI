...
from src.api.validation import validation_jobs, validation_reports, _validation_jobs_lock, _validation_reports_lock
...
class TestValidationAPIIntegration:
    ...
    def test_full_validation_workflow(self, client, sample_validation_request):
        ...
        assert status_data["status"] == ValidationJobStatus.QUEUED
        ...
        assert final_status == ValidationJobStatus.COMPLETED, f"Job did not complete in time. Final status: {final_status}"
    ...
    def test_multiple_concurrent_validations(self, client):
        ...
        while (time.time() - start_time) < max_wait_time:
            ...
            if completed_jobs == num_jobs:
                ...
                pytest.fail(f"One or more validation jobs failed")
    ...
    def test_validation_with_complex_manifest(self, client):
        ...
        while (time.time() - start_time) < max_wait_time:
            ...
            if status == ValidationJobStatus.COMPLETED:
                ...
                pytest.fail(f"Validation job failed")
    ...
    def test_validation_job_persistence(self, client, sample_validation_request):
        ...
        with _validation_jobs_lock:
            ...
            assert stored_job.status == ValidationJobStatus.QUEUED
    ...
    def test_validation_with_minimal_data(self, client):
        ...
        minimal_request = {
            "conversion_id": "minimal_integration_test"
        }
    ...
    def test_validation_agent_error_handling(self, mock_validate, client, sample_validation_request):
        ...
        assert ValidationMessages.REPORT_NOT_AVAILABLE in report_response.json()["detail"]