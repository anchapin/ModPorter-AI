...
class TestConversionInferenceEngine:
    ...
    async def test_get_inference_statistics(self, engine, mock_db):
        ...
        with patch.object(engine, "_retrieve_statistics") as mock_stats:
            ...
            result = await engine.get_inference_statistics(mock_db, time_range="7d")
    ...
    async def test_update_confidence_thresholds(self, engine):
        ...
        new_thresholds = {"high": 0.9, "medium": 0.7, "low": 0.5}
    ...
    async def test_find_alternative_paths(self, engine, mock_db):
        ...
        with patch.object(engine, "_search_path_network") as mock_search:
            mock_search.return_value = [
                ...
                {"path": ["alternative3"], "confidence": 0.75, "complexity": "low"},
            ]
...
class TestConversionInferencePrivateMethods:
    ...
    @pytest.mark.asyncio
    async def test_find_direct_paths_with_results(
    ...
    @pytest.mark.asyncio
    async def test_find_direct_paths_filter_by_platform(
        self, engine, mock_db, mock_source_node
    ):
        ...
        with patch("src.services.conversion_inference.graph_db", mock_graph_db):
            ...
            assert all(
                path["steps"][0]["target_concept"]
                in ["bedrock_block", "universal_block"]
                for path in result
            )
    ...
    async def test_find_direct_paths_error_handling(
        ...
        mock_graph_db = Mock()
        mock_graph_db.find_conversion_paths.side_effect = Exception(
        ...
        mock_graph_db.find_conversion_paths.return_value = [
            {
                ...
                "relationships": [
                    ...
                    {
                        "type": "TRANSFORMS",
                        "start": "intermediate_block",
                        "end": "bedrock_complex_block",
                    },
                ],
                "intermediate_nodes": [
    ...
    @pytest.mark.asyncio
    async def test_find_indirect_paths_max_depth_limit(
        self, engine, mock_db, mock_source_node
    ):
        ...
        mock_graph_db.find_conversion_paths.return_value = [
            {
                ...
                "confidence": 0.40,
                "end_node": {"name": "deep_bedrock_block", "platform": "bedrock"},
            }
        ]
        ...
        with patch.object(engine, "_validate_conversion_pattern") as mock_validate:
            ...
            with patch.object(engine, "_check_platform_compatibility") as mock_compat:
                ...
                with patch.object(engine, "_refine_with_ml_predictions") as mock_ml:
                    ...
                    with patch.object(
                        engine, "_integrate_community_wisdom"
                    ) as mock_community:
                        mock_community.return_value = 0.75  # Community wisdom score

                        with patch.object(
                            engine, "_optimize_for_performance"
                        ) as mock_perf:
                            ...
                            assert all(
                                "enhanced_accuracy" in path
                                for path in result["enhanced_paths"]
                            )
                            assert all(
                                "accuracy_suggestions" in path
                                for path in result["enhanced_paths"]
                            )
                            ...
                            assert (
                                improvements["original_avg_confidence"] == 0.70
                            )  # (0.75 + 0.65) / 2
    ...
    async def test_validate_conversion_pattern_success(self, engine, mock_db):
        ...
        path = {"pattern_type": "simple_conversion", "confidence": 0.80}

        # Mock ConversionPatternCRUD - make it return a coroutine
        mock_patterns = [Mock(success_rate=0.85), Mock(success_rate=0.90)]
    ...
    async def test_check_platform_compatibility_perfect(self, engine):
        ...
        context_data = {"minecraft_version": "latest", "target_platform": "bedrock"}
    ...
    async def test_check_platform_compatibility_version_mismatch(self, engine):
        ...
        context_data = {"minecraft_version": "1.16.5", "target_platform": "bedrock"}
    ...
    async def test_refine_with_ml_predictions(self, engine):
        ...
        context_data = {"model_version": "v2.0", "prediction_accuracy": 0.85}
    ...
    async def test_optimize_for_performance(self, engine):
        ...
        context_data = {
            ...
            "resource_limits": {"max_memory": 512, "max_cpu_time": 300},
        }
...
class TestTopologicalSort:
    ...
    async def test_topological_sort_simple(self, engine):
        ...
        dependency_graph = {"A": ["B", "C"], "B": ["D"], "C": ["D"], "D": []}
