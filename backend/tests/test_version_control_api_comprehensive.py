...
class TestCommitEndpoints:
    ...
    async def test_create_commit_success(
        self, mock_db, mock_service, sample_commit_data
    ):
        """Test successful commit creation"""
        # Setup mock response
        mock_service.create_commit = AsyncMock(
            return_value={
                "success": True,
                "commit_hash": "abc123",
                "branch_name": "main",
    ...
    async def test_create_commit_missing_fields(self, mock_db, mock_service):
        ...
        assert "author_id, author_name, and message are required" in str(
            exc_info.value.detail
        )

    @pytest.mark.asyncio
    async def test_create_commit_service_error(
        self, mock_db, mock_service, sample_commit_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_commit_unexpected_error(
        self, mock_db, mock_service, sample_commit_data
    ):
...
class TestBranchEndpoints:
    ...
    async def test_create_branch_missing_fields(self, mock_service):
        ...
        assert exc_info.value.status_code == 400
        assert "branch_name, author_id, and author_name are required" in str(
            exc_info.value.detail
        )
    ...
    async def test_get_branches_success(self, mock_service):
        ...
        mock_service.branches = {"main": mock_branch1, "protected": mock_branch2}
    ...
    async def test_get_branch_status_success(self, mock_service):
        ...
        mock_service.get_branch_status.return_value = {
            "success": True,
            "status": {"ahead": 5, "behind": 2, "uncommitted_changes": 3},
        }
...
class TestMergeEndpoints:
    ...
    async def test_merge_branch_missing_fields(self, mock_db, mock_service):
        ...
        assert exc_info.value.status_code == 400
        assert "target_branch, author_id, and author_name are required" in str(
            exc_info.value.detail
        )

    @pytest.mark.asyncio
    async def test_merge_branch_with_conflicts(
    ...
    @pytest.mark.asyncio
    async def test_merge_branch_unexpected_error(
        self, mock_db, mock_service, sample_merge_data
    ):
...
class TestDiffEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_generate_diff_unexpected_error(
        self, mock_db, mock_service, sample_diff_data
    ):
...
class TestRevertEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_revert_commit_success(
        self, mock_db, mock_service, sample_revert_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_revert_commit_service_error(
        self, mock_db, mock_service, sample_revert_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_revert_commit_unexpected_error(
        self, mock_db, mock_service, sample_revert_data
    ):
...
class TestTagEndpoints:
    ...
    async def test_create_tag_missing_fields(self, mock_service):
        ...
        assert exc_info.value.status_code == 400
        assert "tag_name, commit_hash, author_id, and author_name are required" in str(
            exc_info.value.detail
        )
    ...
    async def test_get_tags_success(self, mock_service):
        ...
        mock_service.commits = {"commit123": mock_commit1, "commit456": mock_commit2}
        mock_service.tags = {"v1.0.0": "commit123", "v1.1.0": "commit456"}
...
class TestUtilityEndpoints:
    ...
    async def test_get_version_control_status_success(self, mock_service):
        ...
        mock_service.commits = {"commit1": mock_commit, "commit2": mock_commit2}
        mock_service.branches = {
            "main": mock_branch,
    ...
    async def test_search_commits_success(self, mock_service):
        ...
        mock_service.commits = {"commit1": mock_commit1, "commit2": mock_commit2}
...
class TestEdgeCasesAndErrorHandling:
    ...
    async def test_concurrent_requests(self, mock_service):
        ...
        tasks = [create_commit(commit_data, Mock()) for _ in range(10)]
...
if __name__ == "__main__":
    pytest.main(
        [
            __file__,
            "-v",
            "--cov=backend/src/api/version_control.py",
