...
class TestMLPatternRecognitionService:
    ...
    async def test_train_models_force_retrain(self, service, mock_db, sample_training_data):
        ...
        with patch.object(service, '_collect_training_data', return_value=sample_training_data), \
             patch.object(service, '_extract_features', return_value=([[1, 2, 3], [4, 5, 6]], ["direct", "entity"])), \
             patch.object(service, '_train_pattern_classifier', return_value={"accuracy": 0.8}), \
             patch.object(service, '_train_success_predictor', return_value={"mse": 0.1}), \
             patch.object(service, '_train_feature_clustering', return_value={"silhouette_score": 0.6}), \
             patch.object(service, '_train_text_vectorizer', return_value={"vocabulary_size": 100}):
    ...
    async def test_collect_training_data(self, service, mock_db):
        ...
        with patch('src.services.ml_pattern_recognition.ConversionPatternCRUD.get_by_version', 
                  return_value=[mock_pattern]), \
             patch('src.services.ml_pattern_recognition.KnowledgeNodeCRUD.get_by_type', 
                  return_value=[mock_node]), \
             patch('src.services.ml_pattern_recognition.KnowledgeRelationshipCRUD.get_by_source', 
    ...
    @pytest.mark.asyncio
    async def test_recognize_patterns_success(self, service, mock_db, sample_pattern_feature):
        """Test successful pattern recognition."""
        # Setup trained models
        service.is_trained = True
        
        # Mock the feature extraction
        with patch.object(service, '_extract_concept_features', return_value=sample_pattern_feature), \
             patch.object(service, '_predict_pattern_class', return_value={"predicted_class": "direct_conversion", "confidence": 0.8}), \
             patch.object(service, '_predict_success_probability', return_value={"predicted_success": 0.85}), \
             patch.object(service, '_find_similar_patterns', return_value=[]), \
             patch.object(service, '_generate_recommendations', return_value=["Use direct conversion"]), \
            ...
            assert result["pattern_recognition"]["predicted_pattern"]["predicted_class"] == "direct_conversion"
            assert result["pattern_recognition"]["success_probability"]["predicted_success"] == 0.85
    ...
    @pytest.mark.asyncio
    async def test_batch_pattern_recognition(self, service, mock_db, sample_pattern_feature):
        """Test batch pattern recognition."""
        service.is_trained = True
        concepts = ["Entity1", "Entity2", "Entity3"]
        
        with patch.object(service, '_extract_concept_features', return_value=sample_pattern_feature), \
             patch.object(service, 'recognize_patterns', return_value={"success": True, "pattern_recognition": {"test": "data"}}), \
             patch.object(service, '_analyze_batch_patterns', return_value={"test": "batch_analysis"}), \
             patch.object(service, '_cluster_concepts_by_pattern', return_value={"test": "clustering"}):
    ...
    async def test_extract_concept_features(self, service, mock_db):
        ...
        with patch('src.services.ml_pattern_recognition.KnowledgeNodeCRUD.search', return_value=[mock_node]), \
             patch('src.services.ml_pattern_recognition.KnowledgeRelationshipCRUD.get_by_source', return_value=[]):
            
            feature = await service._extract_concept_features("Java Entity Test", "bedrock", "1.19.2", mock_db)
    ...
    async def test_extract_concept_features_not_found(self, service, mock_db):
        ...
        with patch('src.services.ml_pattern_recognition.KnowledgeNodeCRUD.search', return_value=[]):
            feature = await service._extract_concept_features("Unknown Concept", "bedrock", "1.19.2", mock_db)
    ...
    async def test_suggest_optimizations(self, service, sample_pattern_feature):
        ...
        optimizations = await service._suggest_optimizations(sample_pattern_feature, pattern_prediction)
    ...
    async def test_get_feature_importance(self, service):
        ...
        mock_classifier = MagicMock()
        mock_classifier.feature_importances_ = np.array([0.1, 0.2, 0.15, 0.1, 0.25, 0.2])
        service.models["pattern_classifier"] = mock_classifier
    ...
    def test_calculate_feature_similarity(self, service):
        ...
        similarity = service._calculate_feature_similarity(feature_vector, training_sample)
...
class TestServiceIntegration:
    ...
    async def test_full_training_and_prediction_cycle(self):
        ...
        sample_data = [
            {
                "id": f"pattern_{i}",
                "pattern_type": "direct_conversion" if i % 2 == 0 else "entity_conversion",
        ...
        with patch.object(service, '_collect_training_data', return_value=sample_data), \
             patch.object(service, '_extract_features', return_value=(
                 [[i*0.1, i*0.2, i*0.3, i*0.4, i*0.5, i*0.6] for i in range(60)],
                 ["direct_conversion" if i % 2 == 0 else "entity_conversion" for i in range(60)]
             )), \
             patch.object(service, '_train_pattern_classifier', return_value={"accuracy": 0.85}), \
             patch.object(service, '_train_success_predictor', return_value={"mse": 0.12}), \
             patch.object(service, '_train_feature_clustering', return_value={"silhouette_score": 0.65}), \
             patch.object(service, '_train_text_vectorizer', return_value={"vocabulary_size": 150}):
            ...
            with patch.object(service, '_extract_concept_features', return_value=sample_feature), \
                 patch.object(service, '_predict_pattern_class', return_value={"predicted_class": "direct_conversion", "confidence": 0.8}), \
                 patch.object(service, '_predict_success_probability', return_value={"predicted_success": 0.85}), \
                 patch.object(service, '_find_similar_patterns', return_value=[]), \
                 patch.object(service, '_generate_recommendations', return_value=["Test recommendation"]), \
                 patch.object(service, '_identify_risk_factors', return_value=[]), \
                 patch.object(service, '_suggest_optimizations', return_value=[]), \
                 patch.object(service, '_get_feature_importance', return_value={"success_rate": 0.2}):
                
                prediction_result = await service.recognize_patterns("Test Concept", db=mock_db)
                assert prediction_result["success"] is True
                assert prediction_result["concept"] == "Test Concept"
                
                # Test batch processing
                batch_result = await service.batch_pattern_recognition(["Concept1", "Concept2"], db=mock_db)
...
class TestErrorHandling:
    ...
    async def test_training_with_corrupted_data(self, service, mock_db):
        ...
        with patch.object(service, '_collect_training_data', return_value=corrupted_data):
            result = await service.train_models(mock_db)
    ...
    async def test_prediction_with_trained_models_error(self, service, mock_db):
        ...
        with patch.object(service, '_extract_concept_features', side_effect=Exception("Database error")):
    ...
    async def test_feature_extraction_error_handling(self, service, mock_db):
        ...
        # Mock database error
        with patch('src.services.ml_pattern_recognition.KnowledgeNodeCRUD.search', 
                  side_effect=Exception("Database connection error")):
            
            feature = await service._extract_concept_features("Test Concept", "bedrock", "1.19.2", mock_db)
