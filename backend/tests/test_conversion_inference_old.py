...
class TestConversionInferenceEngine:
    ...
    async def test_infer_conversion_path_source_not_found(self, engine, mock_db):
        ...
        with (
            patch.object(engine, "_find_concept_node") as mock_find,
    ...
    async def test_infer_conversion_path_direct_path_success(
        self, engine, mock_db, sample_source_node, sample_direct_paths
    ):
        """Test successful path inference with direct paths."""
        with (
            patch.object(engine, "_find_concept_node") as mock_find,
            patch.object(engine, "_find_direct_paths") as mock_direct,
            patch.object(engine, "_find_indirect_paths"),
        ):
    ...
    async def test_infer_conversion_path_no_direct_paths(
        self, engine, mock_db, sample_source_node, sample_indirect_paths
    ):
        ...
        with (
            ...
            result = await engine.infer_conversion_path(
                java_concept="TestEntity", db=mock_db, target_platform="bedrock"
            )
    ...
    async def test_infer_conversion_path_no_paths_found(
        self, engine, mock_db, sample_source_node
    ):
        ...
        with (
            ...
            result = await engine.infer_conversion_path(
                java_concept="TestEntity", db=mock_db
            )
    ...
    async def test_infer_conversion_path_custom_options(
        self, engine, mock_db, sample_source_node
    ):
        ...
        with (
            ...
            result = await engine.infer_conversion_path(
                java_concept="TestEntity", db=mock_db, path_options=options
            )
    ...
    async def test_batch_infer_paths_success(self, engine, mock_db, sample_source_node):
        ...
        with (
            patch.object(engine, "infer_conversion_path") as mock_infer,
            ...
            result = await engine.batch_infer_paths(
                java_concepts=java_concepts, db=mock_db, target_platform="bedrock"
            )
    ...
    async def test_batch_infer_paths_empty_list(self, engine, mock_db):
        ...
        result = await engine.batch_infer_paths(java_concepts=[], db=mock_db)
    ...
    async def test_batch_infer_paths_partial_failure(self, engine, mock_db):
        ...
        with (
            ...
            result = await engine.batch_infer_paths(
                java_concepts=java_concepts, db=mock_db
            )
    ...
    async def test_optimize_conversion_sequence_success(self, engine, mock_db):
        ...
        with (
            ...
            mock_shared.return_value = [
                {
                    "concepts": ["TestEntity1", "TestEntity3"],
                    "shared_steps": ["validation"],
                }
            ]
            ...
            result = await engine.optimize_conversion_sequence(
                conversions=conversion_sequence, db=mock_db
            )
    ...
    async def test_optimize_conversion_sequence_complex_dependencies(
        self, engine, mock_db
    ):
        """Test conversion sequence optimization with complex dependencies."""
        conversion_sequence = [
            {"concept": "BaseEntity", "dependencies": []},
            {"concept": "DerivedEntity1", "dependencies": ["BaseEntity"]},
            {"concept": "DerivedEntity2", "dependencies": ["BaseEntity"]},
            {
                "concept": "FinalEntity",
    ...
    async def test_learn_from_conversion_success(
        self, engine, mock_db, sample_source_node
    ):
        ...
        with (
            ...
            result = await engine.learn_from_conversion(
                conversion_result=conversion_result, db=mock_db
            )
    ...
    async def test_learn_from_conversion_failure(self, engine, mock_db):
        ...
        with (
            patch.object(engine, "_update_knowledge_graph") as mock_update,
            patch.object(engine, "_adjust_confidence_thresholds") as mock_adjust,
            patch.object(engine, "_store_learning_event"),
            patch.object(engine, "_analyze_conversion_performance") as mock_analyze,
        ):
    ...
    async def test_get_inference_statistics_success(self, engine):
        ...
        engine.confidence_thresholds = {"high": 0.85, "medium": 0.65, "low": 0.45}
    ...
    async def test_find_concept_node_success(self, engine, mock_db, sample_source_node):
        ...
        with patch(
            "src.services.conversion_inference.KnowledgeNodeCRUD.get_by_name"
        ) as mock_get:
    ...
    async def test_find_direct_paths_success(self, engine, mock_db, sample_source_node):
        ...
        with patch(
            "src.services.conversion_inference.KnowledgeRelationshipCRUD.find_direct_conversions"
        ) as mock_find:
            # Mock relationships
            mock_relationships = [
                MagicMock(
            ...
            with patch(
                "src.services.conversion_inference.KnowledgeNodeCRUD.get_by_ids"
            ) as mock_get_nodes:
    ...
    async def test_find_indirect_paths_success(
        self, engine, mock_db, sample_source_node
    ):
        """Test successful indirect paths finding."""
        with patch(
            "src.services.conversion_inference.graph_db.find_paths"
        ) as mock_graph_find:
            # Mock path finding
            mock_paths = [
                {
    ...
    async def test_find_indirect_paths_no_results(
        self, engine, mock_db, sample_source_node
    ):
    ...
    async def test_find_indirect_paths_max_depth(
        self, engine, mock_db, sample_source_node
    ):
        ...
        with patch(
            ...
            mock_paths = [
                ...
                {
                    "nodes": [sample_source_node]
                    + [MagicMock()] * 6,  # Length 7 (too long)
    ...
    async def test_rank_paths_speed_optimization(self, engine, sample_direct_paths):
        ...
        result = await engine._rank_paths(
            paths=sample_direct_paths, optimize_for="speed"
        )

        assert len(result) == 2
        assert (
            result[0]["estimated_time"] <= result[1]["estimated_time"]
        )  # Sorted by time
    ...
    async def test_rank_paths_features_optimization(self, engine, sample_direct_paths):
        ...
        result = await engine._rank_paths(
            paths=sample_direct_paths, optimize_for="features"
        )
    ...
    async def test_suggest_similar_concepts_success(self, engine, mock_db):
        ...
        with patch(
            "src.services.conversion_inference.KnowledgeNodeCRUD.search_by_name"
        ) as mock_search:
            mock_nodes = [
    ...
    async def test_analyze_batch_paths_success(self, engine):
        ...
        batch_results = {
            ...
            "concept3": {"success": False, "error": "Concept not found"},
        }
    ...
    async def test_optimize_processing_order_success(self, engine):
        ...
        concepts = [
            ...
            {"concept": "concept3", "priority": 1, "dependencies": []},
            {
                "concept": "concept4",
                "priority": 2,
                "dependencies": ["concept2", "concept3"],
            },
        ]
    ...
    async def test_identify_shared_steps_no_shared(self, engine):
        ...
        conversions = [
            {"concept": "concept1", "conversion_steps": ["unique_step1"]},
            {"concept": "concept2", "conversion_steps": ["unique_step2"]},
        ]
...
class TestEdgeCases:
    ...
    def test_extreme_confidence_values(self, engine):
        ...
        high_conf = {"confidence": 1.0, "path_length": 1, "estimated_time": 0.1}

        # Very low confidence
        low_conf = {"confidence": 0.0, "path_length": 10, "estimated_time": 100.0}
        ...
        )  # Low confidence should have higher complexity
