...
from src.api.behavior_export import (
    export_behavior_pack,
    download_exported_pack,
    preview_export,
    get_export_formats,
    ExportRequest,
)
...
async def test_download_exported_pack_direct_call(mock_get_job, MockCacheService):
    ...
    # Setup mocks
    mock_cache_instance = MockCacheService.return_value
    mock_cache_instance.get_export_data.return_value = AsyncMock(
        return_value=PACK_CONTENT
    )()
    mock_get_job.return_value = MagicMock(status="completed")
    mock_db_session = create_mock_db_session()

    # Call function
    response = await download_exported_pack(
        conversion_id=CONVERSION_ID, db=mock_db_session
    )
...
async def test_preview_export_direct_call(mock_get_job, mock_get_files):
    ...
    mock_files = [
        MockBehaviorFile(
            "path1", "type1", '{"_template_info": {"template_name": "t1"}}'
        )
    ]
    mock_get_files.return_value = mock_files
    mock_db_session = create_mock_db_session(files_to_return=mock_files)
