...
from src.services.advanced_visualization import FilterType, LayoutAlgorithm
...
class TestVisualizationCreation:
    ...
    async def test_create_visualization_success(self):
        ...
        with patch(
            "src.api.visualization.advanced_visualization_service", mock_service
        ):
            viz_data = {
                "graph_id": "graph_456",
    ...
    async def test_create_visualization_missing_graph_id(self):
        ...
        viz_data = {"visualization_type": "force_directed", "layout": "spring"}
...
class TestVisualizationManagement:
    ...
    async def test_focus_on_node_success(self):
        ...
        with patch(
            ...
            focus_data = {"node_id": "node_456", "radius": 3, "animate": True}
...
class TestFilterPresets:
    ...
    async def test_get_specific_filter_preset_success(self):
        ...
        mock_service.filter_presets = {"java_mods": [mock_preset]}
        ...
        for format_type in formats:
            ...
            with patch(
                ...
                response = client.post(
                    "/visualizations/viz_123/export", json=export_data
                )
                assert response.status_code == 200
                data = response.json()
                assert data["success"] is True
    ...
    async def test_get_visualization_metrics_success(self):
        ...
        mock_service.get_visualization_metrics.return_value = {
            ...
            "metrics": {
                ...
                "community_metrics": {"modularity": 0.67, "number_of_communities": 5},
            },
        }
...
class TestVisualizationErrorHandling:
    ...
    async def test_update_filters_exception(self):
        ...
        mock_service = AsyncMock()
        mock_service.update_visualization_filters.side_effect = Exception(
            "Update failed"
        )
    ...
    async def test_get_types_exception(self):
        ...
        with patch(
            "src.api.visualization.VisualizationType",
    ...
    async def test_get_presets_exception(self):
        ...
        with patch(
            "src.api.visualization.advanced_visualization_service.filter_presets",
            side_effect=Exception("Preset access failed"),
        ):
            response = client.get("/filter-presets")
...
class TestVisualizationIntegration:
    ...
    async def test_complete_visualization_workflow(self):
        ...
        with patch(
            ...
            export_response = client.post(
                f"/visualizations/{viz_id}/export", json=export_data
            )
    ...
    async def test_layout_change_workflow(self):
        ...
        with patch(
            ...
            for layout in layouts:
                ...
                response = client.post(
                    "/visualizations/viz_123/layout", json=layout_data
                )
                assert response.status_code == 200
    ...
    async def test_focus_and_filter_workflow(self):
        ...
        with patch(
            ...
            focus_data = {"node_id": "central_node", "radius": 2, "animate": True}

            focus_response = client.post(
                "/visualizations/viz_123/focus", json=focus_data
            )
            ...
            filter_response = client.post(
                "/visualizations/viz_123/filters", json=filter_data
            )
...
class TestVisualizationPerformance:
    ...
    async def test_performance_stats_comprehensiveness(self):
        ...
        for i in range(5):
            mock_viz = Mock()
            mock_viz.nodes = [
                Mock() for _ in range(10 * (i + 1))
            ]  # 10, 20, 30, 40, 50 nodes
            mock_viz.edges = [
                Mock() for _ in range(15 * (i + 1))
            ]  # 15, 30, 45, 60, 75 edges
            mock_service.visualization_cache[f"viz_{i}"] = mock_viz

        mock_service.layout_cache = {
            f"viz_{i}": Mock() for i in range(3)
        }  # 3 cached layouts
        mock_service.cluster_cache = {
            f"viz_{i}": Mock() for i in range(4)
        }  # 4 cached clusters
        ...
        with patch(
            ...
            expected_stats = [
                ...
                "total_nodes",
                "total_edges",
                "average_nodes_per_visualization",
                "average_edges_per_visualization",
            ]
