...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("performance_analysis.log"), logging.StreamHandler(sys.stdout)],
)
...
class PerformanceAnalyzer:
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            f"PerformanceAnalyzer initialized with API: {self.api_base_url}, Timeout: {self.timeout}s"
        )
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                if attempt < self.retry_attempts - 1:
                    logger.warning(
                        f"Request failed (attempt {attempt + 1}/{self.retry_attempts}), retrying in {self.retry_delay}s: {str(e)}"
                    )
    ...
    def identify_bottlenecks(self, metrics: List[PerformanceMetric]) -> List[str]:
        ...
        if critical_metrics:
            ...
            for metric in critical_metrics[:5]:  # Limit to top 5
                bottlenecks.append(
                    f"   • {metric.name}: {metric.value:.2f}{metric.unit} (threshold: {metric.threshold}{metric.unit})"
                )
        ...
        if len(api_issues) >= 2:
            bottlenecks.append(
                "🔗 API Performance Degradation: Multiple endpoints responding slowly"
            )
        ...
        memory_issues = [
            m for m in metrics if "memory" in m.name.lower() and m.status in ["warning", "critical"]
        ]
        if memory_issues:
    ...
    def generate_recommendations(self, metrics: List[PerformanceMetric]) -> List[str]:
        ...
        frontend_slow = [
            m for m in metrics if m.category == "frontend" and m.status in ["warning", "critical"]
        ]
        db_slow = [
            m for m in metrics if m.category == "database" and m.status in ["warning", "critical"]
        ]
        conversion_slow = [
            m for m in metrics if m.category == "conversion" and m.status in ["warning", "critical"]
        ]
        ai_slow = [
            m for m in metrics if m.category == "ai_engine" and m.status in ["warning", "critical"]
        ]
    ...
    <script>
        ...
        if report.recommendations:
            recommendations_html = "".join(
                [f"<li class='mb-2'>{r}</li>" for r in report.recommendations]
            )
        ...
        html = html_template.format(
            ...
            metrics_data=json.dumps(
                [
                    {"name": m.name, "value": m.value, "threshold": m.threshold, "status": m.status}
