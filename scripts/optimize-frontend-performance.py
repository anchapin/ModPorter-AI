...
class FrontendPerformanceOptimizer:
    ...
    def analyze_bundle_structure(self) -> Dict[str, Any]:
        ...
        for js_file in js_files:
            ...
            chunks.append({"file": js_file.name, "size_kb": size_kb, "type": "chunk"})
    ...
    def generate_optimization_suggestions(
        self, dependency_analysis: Dict, bundle_analysis: Dict
    ) -> List[str]:
        ...
        if large_deps:
            ...
            for dep in large_deps[:3]:
                suggestions.append(
                    f"   • {dep['name']}: {dep['estimated_size_kb']}KB - consider alternatives"
                )
            ...
            if "monaco-editor" in large_dep_names:
                suggestions.append(
                    "   • Use Monaco Editor web workers and load only required languages"
                )
...
"""
    ...
    def generate_performance_budgets(self) -> Dict[str, Any]:
        ...
        return {
            "budgets": [
                {"path": "*.js", "limit": "4 MB", "type": "initial"},
                {"path": "vendor.*.js", "limit": "2 MB", "type": "initial"},
                {"path": "*.css", "limit": "500 KB", "type": "initial"},
                {"path": "*.png", "limit": "1 MB", "type": "any"},
                {"path": "*.jpg", "limit": "500 KB", "type": "any"},
            ],
    ...
    async def run_lighthouse_audit(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                performance_score = (
                    lighthouse_data["lhr"]["categories"]["performance"]["score"] * 100
                )

                return {
                    "performance_score": performance_score,
                    "fcp": lighthouse_data["lhr"]["audits"]["first-contentful-paint"][
                        "numericValue"
                    ],
                    "lcp": lighthouse_data["lhr"]["audits"]["largest-contentful-paint"][
                        "numericValue"
                    ],
                    "tti": lighthouse_data["lhr"]["audits"]["interactive"]["numericValue"],
                    "cls": lighthouse_data["lhr"]["audits"]["cumulative-layout-shift"][
                        "numericValue"
                    ],
