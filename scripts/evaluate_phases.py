...
def load_test_data(n_samples=10):
    ...
    for idx in indices:
        p = val_pairs[idx]
        user = (
            f"Mod Description: {p['instruction']}\n\nJava Source:\n{p['java_source'][:2000]}\n\n"
...
def evaluate_model(model_path, model_name, tokenizer, samples, max_new_tokens=512):
    ...
    all_components = {
        k: []
        for k in ["manifest", "js", "json_validity", "bedrock_apis", "length", "hallucination"]
    }
    ...
    for i, sample in enumerate(samples):
        ...
        if i < 3:
            ...
            print(
                f"    manifest={rewards['manifest']:.2f} js={rewards['js']:.2f} "
...
def main():
    ...
    print("=" * 70)
    print(
        f"  {'Model':<40} {'Total':>8} {'Manifest':>10} {'JS':>8} {'JSON':>8} {'Bedrock':>8} {'Halluc':>8}"
    )
    print("  " + "-" * 90)
    for name, r in results.items():
        c = r["components"]
        print(
            f"  {name:<40} {r['total']:>8.3f} {c['manifest']:>10.3f} {c['js']:>8.3f} {c['json_validity']:>8.3f} {c['bedrock_apis']:>8.3f} {c['hallucination']:>8.3f}"
        )
