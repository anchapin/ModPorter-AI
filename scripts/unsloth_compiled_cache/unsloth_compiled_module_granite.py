...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.granite.modeling_granite import (__name__, Callable, Optional, torch, nn, ACT2FN, Cache, GenerationMixin, use_kernel_func_from_hub, BaseModelOutputWithPast, CausalLMOutputWithPast, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, PreTrainedModel, Unpack, TransformersKwargs, can_return_tuple, maybe_autocast, GraniteConfig, GranitePreTrainedModel, GraniteModel, GraniteForCausalLM)
...
class GraniteAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return GraniteAttention_forward(self, hidden_states=hidden_states, position_embeddings=position_embeddings, attention_mask=attention_mask, past_key_values=past_key_values, **kwargs)


@torch.compile(fullgraph = True, dynamic = True, options = torch_compile_options)
def GraniteRMSNorm_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class GraniteRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return GraniteRMSNorm_forward(self, hidden_states=hidden_states)
...
class GraniteMLP(nn.Module):
    ...
    def forward(self, x):
        return GraniteMLP_forward(self, x=x)


@torch.compile(fullgraph = False, dynamic = True, options = torch_compile_options)
@torch.no_grad()
@dynamic_rope_update  # power user: used with advanced RoPE types (e.g. dynamic rope)
def GraniteRotaryEmbedding_forward(self, x, position_ids):
...
class GraniteRotaryEmbedding(nn.Module):
    ...
    def compute_default_rope_parameters(
        config: GraniteConfig | None = None,
    ...
    def forward(self, x, position_ids):
        return GraniteRotaryEmbedding_forward(self, x=x, position_ids=position_ids)


@torch.compiler.disable(recursive = False)
@can_return_tuple
def GraniteForCausalLM_forward(
    self,
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    if RETURN_HIDDEN_STATES:
    ...
    elif (() == () and (self.config.logits_scaling) == ()) and (UNSLOTH_ENABLE_CCE) and NOT_RETURN_LOGITS and self.loss_function.__name__.endswith("ForCausalLMLoss") and labels is not None and not requires_grad_:
        ...
        loss = unsloth_fused_ce_loss(
            ...
            logit_scale_divide   = (self.config.logits_scaling) if (self.config.logits_scaling) != () else 0,
...
class GraniteForCausalLM(GranitePreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return GraniteForCausalLM_forward(self, input_ids=input_ids, attention_mask=attention_mask, position_ids=position_ids, past_key_values=past_key_values, inputs_embeds=inputs_embeds, labels=labels, use_cache=use_cache, logits_to_keep=logits_to_keep, **kwargs)
