...
describe('ConversionUpload Component', () => {
  ...
  describe('Visual Interface and Form Logic', () => {
    ...
    test('shows URL input option as alternative', () => {
      render(<ConversionUpload />);
      expect(
        screen.getByPlaceholderText(/curseforge.*modrinth/i)
      ).toBeInTheDocument();
  ...
  describe('URL Validation', () => {
    test('validates URL types according to PRD specs', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Please enter a valid CurseForge or Modrinth URL/)
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Smart Assumptions UI', () => {
    test('displays smart assumptions toggle with explanation', async () => {
      ...
      const toggle = screen.getByRole('checkbox', {
        name: /enable smart assumptions/i,
      });
      ...
      const infoButton = screen.getByLabelText(
        /learn more about smart assumptions/i
      );
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            /intelligent assumptions to convert incompatible features/i
          )
        ).toBeInTheDocument();
      });
    });
    ...
    test('shows smart assumptions examples when info button clicked', async () => {
      ...
      const infoButton = screen.getByRole('button', {
        name: /learn more about smart assumptions/i,
      });
      await act(async () => {
        await user.click(infoButton);
      });
      expect(
        screen.getByText(/Custom dimensions.*Large structures/i)
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Complex machinery.*Simplified blocks/i)
      ).toBeInTheDocument();
    });
  });
  ...
  describe('Basic Functionality', () => {
    ...
    test('shows dependencies toggle', () => {
      render(<ConversionUpload />);
      const dependenciesToggle = screen.getByRole('checkbox', {
        name: /include dependencies/i,
      });
  ...
  describe('API Integration Tests', () => {
    test('handles file upload and conversion initiation', async () => {
      ...
      expect(
        screen.getByRole('button', { name: /Remove test-mod.zip/i })
      ).toBeInTheDocument();
    });
