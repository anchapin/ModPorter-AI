...
export const BlockList: React.FC = () => {
  const { addonData, selectedBlockId, setSelectedBlockId, isLoading } =
    useEditorContext();

  if (isLoading && !addonData) {
    // Show loading only if addonData is not yet available
    return <div className="block-list-loading">Loading blocks...</div>;
  }

  if (!addonData || !addonData.blocks || addonData.blocks.length === 0) {
    return (
      <div className="block-list-empty">No blocks found in this addon.</div>
    );
  }
