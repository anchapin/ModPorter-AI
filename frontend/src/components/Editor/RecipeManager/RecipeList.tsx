...
export const RecipeList: React.FC = () => {
  const { addonData, selectedRecipeId, setSelectedRecipeId, isLoading } =
    useEditorContext();
  ...
  if (!addonData || !addonData.recipes || addonData.recipes.length === 0) {
    return (
      <div className="recipe-list-status">No recipes found in this addon.</div>
    );
  }
  ...
  const getRecipeDisplayName = (recipeData: any): string => {
    ...
    if (typeof recipeData === 'object' && recipeData !== null) {
      const description =
        recipeData['minecraft:recipe_shaped']?.description?.identifier ||
        recipeData['minecraft:recipe_furnace']?.description?.identifier ||
        recipeData['minecraft:recipe_brewing_container']?.description
          ?.identifier ||
      ...
      const resultItem =
        recipeData['minecraft:recipe_shaped']?.result?.item ||
      ...
      if (typeof resultItem === 'string')
        return resultItem.split(':').pop() || resultItem;
      if (typeof resultItem === 'object' && resultItem?.item)
        return (
          String(resultItem.item).split(':').pop() || String(resultItem.item)
        );
    }
