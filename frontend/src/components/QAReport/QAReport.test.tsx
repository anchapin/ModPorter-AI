...
describe('QAReport', () => {
  test('shows loading state initially', () => {
    ...
    expect(
      screen.getByText('Loading QA Report for Task ID: test-task-123...')
    ).toBeInTheDocument();
  });
  ...
  test('shows error when no task ID provided', () => {
    ...
    expect(
      screen.getByText('Error: Task ID is required to fetch a QA report.')
    ).toBeInTheDocument();
  });
  ...
  test('displays QA report data after loading', async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText('QA Report (Task ID: test-task-123)')
        ).toBeInTheDocument();
      },
      { timeout: 3000 }
    );
