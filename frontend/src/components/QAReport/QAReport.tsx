...
const QAReport: React.FC<QAReportProps> = ({
  taskId,
  apiBaseUrl = '/api/qa',
}) => {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      ...
      try {
        ...
        console.log(
          `Fetching QA report for task ID: ${taskId} from ${apiBaseUrl}/report/${taskId}`
        );
        ...
        const mockApiResponse = {
          ...
          report: {
            ...
            performance_tests: {
              cpu_avg: '20%',
              memory_peak: '250MB',
              details: [{ name: 'Load Test', metrics: 'OK' }],
            },
            compatibility_tests: {
              versions_tested: ['1.19.0', '1.20.0'],
              issues: 0,
              details: [{ name: 'Version X', status: 'passed' }],
            },
            recommendations: [
              'Review item placement logic.',
              "Optimize texture loading for 'custom_block_A'.",
            ],
        ...
        if (mockApiResponse.success && mockApiResponse.report) {
          ...
          setError(
            'Failed to fetch report (mock error). Task might not be completed or found.'
          );
        }
      } catch (e) {
  ...
  if (loading) {
    return (
      <div className="qa-report-loading">
        Loading QA Report for Task ID: {taskId}...
      </div>
    );
  }
  ...
  if (!report) {
    return (
      <div className="qa-report-empty">
        No report data available for Task ID: {taskId}.
      </div>
    );
  }
    ...
    <div className="qa-report-container">
      ...
      <div className="qa-section overall-summary">
        ...
        <p>
          ...
          {report.overall_quality_score?.toFixed(2) ?? 'N/A'}
        </p>
        <p>Total Tests: {report.summary?.total_tests ?? 'N/A'}</p>
        <p>Passed: {report.summary?.passed ?? 'N/A'}</p>
        <p>
          Failed:{' '}
          {report.summary?.total_tests && report.summary?.passed
            ? report.summary.total_tests - report.summary.passed
            : 'N/A'}
        </p>
      </div>
