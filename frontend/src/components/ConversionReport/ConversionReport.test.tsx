...
describe('ConversionReport Component', () => {
  ...
  test('renders successful conversion report correctly', () => {
    ...
    const mockSuccessReport = Successful.args?.conversionResult as
      | InteractiveReport
      | undefined;

    if (!mockSuccessReport) {
      throw new Error(
        'Mock data for successfulConversion is not correctly loaded from stories.'
      );
    }

    render(
      <ConversionReport
        conversionResult={mockSuccessReport}
        jobStatus="completed"
      />
    );
    ...
    expect(
      screen.getByText(
        mockSuccessReport.summary.overall_success_rate.toFixed(1) + '%'
      )
    ).toBeInTheDocument();

    if (mockSuccessReport.summary.download_url) {
      expect(
        screen.getByRole('link', { name: /Download .mcaddon/i })
      ).toHaveAttribute('href', mockSuccessReport.summary.download_url);
    }

    if (
      mockSuccessReport.converted_mods &&
      mockSuccessReport.converted_mods.length > 0
    ) {
      expect(
        screen.getByText(
          new RegExp(
            'Converted Mods \\(' +
              mockSuccessReport.converted_mods.length +
              '\\)',
            'i'
          )
        )
      ...
      expect(
        screen.getAllByText(
          new RegExp(mockSuccessReport.converted_mods[0].name, 'i')
        )[0]
      ).toBeInTheDocument();
    }

    if (
      mockSuccessReport.smart_assumptions_report &&
      mockSuccessReport.smart_assumptions_report.assumptions.length > 0
    ) {
      expect(
        screen.getByText(/Detailed Smart Assumptions/i)
      ).toBeInTheDocument();
    }
    if (mockSuccessReport.feature_analysis) {
      expect(
        screen.getByText(/Detailed Feature Analysis/i)
      ).toBeInTheDocument();
    }
  ...
  test('renders failed conversion report correctly', () => {
    const mockFailedReport = Failed.args?.conversionResult as
      | InteractiveReport
      | undefined;

    if (!mockFailedReport) {
      throw new Error(
        'Mock data for failedConversion is not correctly loaded from stories.'
      );
    }

    render(
      <ConversionReport
        conversionResult={mockFailedReport}
        jobStatus="failed"
      />
    );
    ...
    expect(
      screen.getByText(
        mockFailedReport.summary.overall_success_rate.toFixed(1) + '%'
      )
    ).toBeInTheDocument();

    if (
      mockFailedReport.failed_mods &&
      mockFailedReport.failed_mods.length > 0
    ) {
      expect(
        screen.getByText(
          new RegExp(
            'Failed Mods \\(' + mockFailedReport.failed_mods.length + '\\)',
            'i'
          )
        )
      ).toBeInTheDocument();
      ...
      expect(
        screen.getAllByText(
          new RegExp(mockFailedReport.failed_mods[0].name, 'i')
        )[0]
      ).toBeInTheDocument();
      // And its first error, if available
      if (
        mockFailedReport.failed_mods[0].errors &&
        mockFailedReport.failed_mods[0].errors.length > 0
      ) {
        ...
        expect(
          screen.getByText(mockFailedReport.failed_mods[0].errors[0])
        ).toBeInTheDocument();
      }
    }
    ...
    if (mockFailedReport.summary.download_url) {
      ...
      expect(
        screen.queryByRole('link', { name: /Download .mcaddon/i })
      ).not.toBeInTheDocument();
    }
  });

  test('displays developer log content if available', () => {
    const mockReportWithDevLog = Successful.args?.conversionResult as
      | InteractiveReport
      | undefined;

    if (!mockReportWithDevLog) {
      throw new Error(
        'Mock data for successfulConversion (for dev log test) is not correctly loaded.'
      );
    }

    if (
      !mockReportWithDevLog.developer_log ||
      !mockReportWithDevLog.developer_log.performance_metrics ||
      mockReportWithDevLog.developer_log.performance_metrics
        .total_time_seconds === undefined
    ) {
      console.warn(
        'Skipping dev log content test: mock data for performance_metrics.total_time_seconds is incomplete or undefined.'
      );
    ...
    render(
      <ConversionReport
        conversionResult={mockReportWithDevLog}
        jobStatus="completed"
      />
    );
    ...
    const perfMetricsPreElement = screen.getByText((content, element) => {
      return (
        ...
        content.includes('"total_time_seconds"')
      );
    });
    expect(perfMetricsPreElement).toBeInTheDocument();
    expect(perfMetricsPreElement.textContent).toContain(
      `"total_time_seconds": ${mockReportWithDevLog.developer_log.performance_metrics.total_time_seconds}`
    );

    // Check for a log entry message if available
    if (
      mockReportWithDevLog.developer_log.code_translation_details &&
      mockReportWithDevLog.developer_log.code_translation_details.length > 0
    ) {
      expect(
        screen.getByText(
          new RegExp(
            mockReportWithDevLog.developer_log.code_translation_details[0]
              .message,
            'i'
          )
        )
