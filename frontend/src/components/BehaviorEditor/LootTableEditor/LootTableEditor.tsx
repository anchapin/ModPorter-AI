...
export const LootTableEditor: React.FC<LootTableEditorProps> = ({
  ...
  const updateLootTable = useCallback(
  ...
  const updatePool = useCallback(
    (poolId: string, updates: Partial<LootPool>) => {
      updateLootTable({
        ...lootTable,
  ...
  const addEntry = useCallback(
    (poolId: string) => {
      ...
      updatePool(poolId, {
        entries: [
          ...lootTable.pools.find((p) => p.id === poolId)!.entries,
          newEntry,
        ],
      });
    },
  ...
  const deleteEntry = useCallback(
    (poolId: string, entryId: string) => {
      updatePool(poolId, {
        entries: lootTable.pools
          .find((p) => p.id === poolId)!
  ...
  const updateEntry = useCallback(
    (poolId: string, entryId: string, updates: Partial<LootEntry>) => {
      updatePool(poolId, {
        entries: lootTable.pools
          .find((p) => p.id === poolId)!
  ...
  const addCondition = useCallback(
    (target: { type: 'pool' | 'entry'; id: string; poolId?: string }) => {
      ...
      setTimeout(() => {
        if (target.type === 'pool') {
          updatePool(target.id, {
            conditions: [
              ...lootTable.pools.find((p) => p.id === target.id)!.conditions,
              newCondition,
            ],
          });
        } else {
          updateEntry(target.poolId!, target.id, {
            conditions: [
              ...lootTable.pools
                .find((p) => p.id === target.poolId)!
                .entries.find((e) => e.id === target.id)!.conditions,
              newCondition,
            ],
          });
        }
    ...
    (target: { type: 'pool' | 'entry'; id: string; poolId?: string }) => {
      ...
      setTimeout(() => {
        if (target.type === 'pool') {
          updatePool(target.id, {
            functions: [
              ...lootTable.pools.find((p) => p.id === target.id)!.functions,
              newFunction,
            ],
          });
        } else {
          updateEntry(target.poolId!, target.id, {
            functions: [
              ...lootTable.pools
                .find((p) => p.id === target.poolId)!
                .entries.find((e) => e.id === target.id)!.functions,
              newFunction,
            ],
          });
        }
  ...
  const generatePreview = useCallback(() => {
    const preview = {
      ...
      total_entries: lootTable.pools.reduce(
    ...
    <Box sx={{ p: 2, height: '100%', overflow: 'auto' }}>
      ...
      >
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          <Button variant="outlined" onClick={exportToJSON}>
            Export JSON
          </Button>
        </Box>
      </Box>
          ...
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, md: 6 }}>
              <TextField
                ...
                value={lootTable.id}
                onChange={(e) =>
                  updateLootTable({ ...lootTable, id: e.target.value })
                }
                disabled={readOnly}
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    updateLootTable({ ...lootTable, type: e.target.value })
                  }
                  disabled={readOnly}
                >
                  ...
                  <MenuItem value="advancement_reward">
                    Advancement Reward
                  </MenuItem>
      ...
      <Alert severity="info" sx={{ mb: 2 }}>
        This is a placeholder implementation for the Loot Table Editor. Full
        functionality including: visual condition editing, function
        configuration, and real-time preview will be implemented in the next
        phase.
      </Alert>
          ...
          lootTable.pools.map((pool) => (
            <Accordion
              ...
              onChange={(_, isExpanded) =>
                setExpandedPool(isExpanded ? pool.id : false)
              }
              sx={{ mb: 1 }}
            >
              <AccordionSummary expandIcon={<ExpandMore />}>
                ...
                >
                  ...
                  <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                    ...
                    <Chip
                      label={
                        pool.rolls.type === 'exact'
                          ? `${pool.rolls.value} rolls`
                          : `${pool.rolls.min}-${pool.rolls.max} rolls`
                      }
                      size="small"
                    />
                    {!readOnly && (
                          ...
                          <Switch
                            ...
                            onChange={(e) =>
                              updatePool(pool.id, {
                                rolls: {
                                  ...pool.rolls,
                                  type: e.target.checked ? 'range' : 'exact',
                                },
                              })
                            }
                            disabled={readOnly}
                  ...
                  <Grid size={{ xs: 12 }}>
                    <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                      <Button
                        ...
                        startIcon={<Add />}
                        onClick={() =>
                          addCondition({ type: 'pool', id: pool.id })
                        }
                        disabled={readOnly}
                      >
                        Add Condition
                      ...
                      <Button
                        ...
                        startIcon={<Settings />}
                        onClick={() =>
                          addFunction({ type: 'pool', id: pool.id })
                        }
                        disabled={readOnly}
                      >
                        Add Function
      ...
      >
        ...
        <DialogContent>
          <Alert severity="info">
            Condition configuration will be available in the next implementation
            phase.
          </Alert>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          <Alert severity="info">
            Function configuration will be available in the next implementation
            phase.
          </Alert>
        </DialogContent>
        <DialogActions>
