...
const mockFetch = vi.fn((url: string, options?: any) => {
  ...
  if (url.includes('/upload') && options?.method === 'POST') {
    ...
    const uploadHeaders = new Map([['content-type', 'application/json']]);
  ...
  if (url.includes('/status')) {
    ...
    const statusHeaders = new Map([['content-type', 'application/json']]);
  ...
  if (url.includes('/download')) {
    return Promise.resolve({
      ...
      headers: new Map([
        ['Content-Disposition', 'attachment; filename="converted.mcaddon"'],
      ]),
    });
  }
