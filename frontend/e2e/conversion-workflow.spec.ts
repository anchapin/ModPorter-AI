...
test.describe('ModPorter AI - Conversion Workflow', () => {
  ...
  test('should navigate to conversion page', async ({ page }) => {
    ...
    await page.click(
      '[data-testid="start-conversion"], button:has-text("Start"), a:has-text("Convert")'
    );

    // Should be on conversion page
    await expect(page.url()).toContain('/convert');
    await expect(
      page.locator('[data-testid="conversion-page"], h1:has-text("Convert")')
    ).toBeVisible();
  });
  ...
  test('should handle file upload', async ({ page, testJarFile }) => {
    ...
    await page.click(
      '[data-testid="start-conversion"], button:has-text("Start")'
    );
    ...
    if (await fileInput.isVisible()) {
      ...
      await expect(
        page.locator('[data-testid="uploaded-file"], .file-name')
  ...
  test('should show conversion options', async ({ page }) => {
    ...
    const conversionOptions = page.locator(
      '[data-testid="conversion-options"], .options, form'
    );
    if (await conversionOptions.isVisible()) {
      ...
      const blockOption = page.locator(
        '[data-testid="option-blocks"], input[value="blocks"], label:has-text("Blocks")'
      );
  ...
  test('should start conversion process', async ({ page }) => {
    ...
    const demoButton = page.locator(
      '[data-testid="demo-file"], button:has-text("Demo"), button:has-text("Sample")'
    );
    ...
    const startButton = page.locator(
      '[data-testid="start-conversion-btn"], button:has-text("Convert"), button:has-text("Start")'
    );
    if (await startButton.isVisible()) {
      ...
      const progress = page.locator(
        '[data-testid="progress"], .progress-bar, .conversion-progress'
      );
  ...
  test('should display conversion results', async ({ page }) => {
    ...
    const demoButton = page.locator(
      '[data-testid="demo-file"], button:has-text("Demo")'
    );
    if (await demoButton.isVisible()) {
      await demoButton.click();

      const startButton = page.locator(
        '[data-testid="start-conversion-btn"], button:has-text("Convert")'
      );
      if (await startButton.isVisible()) {
        await startButton.click();
        ...
        const results = page.locator(
          '[data-testid="conversion-results"], .results, .report'
        );
        await expect(results).toBeVisible({ timeout: 60000 });

        // Check for download button
        const downloadButton = page.locator(
          '[data-testid="download-btn"], button:has-text("Download")'
        );
  ...
  test('should handle conversion errors gracefully', async ({ page }) => {
    ...
    if (await startButton.isVisible()) {
      ...
      const errorMessage = page.locator(
        '[data-testid="error"], .error-message, .validation-error'
      );
  ...
  test('should navigate to behavior editor', async ({ page }) => {
    ...
    const behaviorEditorLink = page.locator(
      '[data-testid="behavior-editor"], a:has-text("Behavior"), button:has-text("Behavior")'
    );
    if (await behaviorEditorLink.isVisible()) {
      ...
      await expect(
        page.locator(
          '[data-testid="behavior-editor-page"], h1:has-text("Behavior")'
        )
      ).toBeVisible();
    }
  });
  ...
  test('should be responsive on mobile', async ({ page }) => {
    ...
    const navToggle = page.locator(
      '[data-testid="nav-toggle"], button[aria-label*="menu"], .hamburger'
    );
    if (await navToggle.isVisible()) {
      ...
      const mobileMenu = page.locator(
        '[data-testid="mobile-menu"], .mobile-nav, .dropdown'
      );
