...
test.describe('ModPorter AI - API Integration', () => {
    ...
    if (await startButton.isVisible()) {
      ...
      if (await demoButton.isVisible()) {
        ...
        if (await convertButton.isVisible()) {
          ...
          const conversionRequests = apiRequests.filter(
            (req) =>
    ...
    if (await demoButton.isVisible()) {
      ...
      if (await convertButton.isVisible()) {
        ...
        const errorMessage = page.locator(
          '[data-testid="error"], .error-message, .api-error'
        );
  ...
  test('should handle behavior editor API calls', async ({ page }) => {
    ...
    const behaviorLink = page.locator(
      '[data-testid="behavior-editor"], a:has-text("Behavior")'
    );
    if (await behaviorLink.isVisible()) {
      ...
      page.on('request', (request) => {
        if (
          request.url().includes('/api/behavior') ||
  ...
  test('should handle authentication API if present', async ({ page }) => {
    ...
    const loginButton = page.locator(
      '[data-testid="login"], button:has-text("Login"), a:has-text("Login")'
    );
    const signupButton = page.locator(
      '[data-testid="signup"], button:has-text("Sign Up"), a:has-text("Sign")'
    );

    if ((await loginButton.isVisible()) || (await signupButton.isVisible())) {
      const authButton = (await loginButton.isVisible())
        ? loginButton
        : signupButton;
      ...
      if (await authForm.isVisible()) {
        ...
        const submitButton = page.locator(
          'button[type="submit"], button:has-text("Login"), button:has-text("Sign Up")'
        );
