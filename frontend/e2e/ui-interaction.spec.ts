...
test.describe('Portkit - UI Interactions', () => {
  ...
  test('should handle responsive navigation on mobile', async ({ page }) => {
    ...
    const menuToggle = page.locator(
      '[data-testid="nav-toggle"], button[aria-label*="menu"], .hamburger, .menu-toggle'
    );

    if (await menuToggle.isVisible()) {
      // Initially mobile menu should be hidden or collapsed
      const mobileMenu = page.locator(
        '[data-testid="mobile-menu"], .mobile-nav, .dropdown-menu'
      );
  ...
  test('should handle drag and drop file upload', async ({ page }) => {
    ...
    const dropzone = page.locator(
      '[data-testid="dropzone"], .dropzone, [data-testid="file-upload"]'
    );
    ...
    if (await dropzone.isVisible()) {
      ...
      const dataTransfer = await page.evaluateHandle(() => {
        ...
        const file = new File(['test content'], 'test-mod.jar', {
      ...
      const uploadedFile = page.locator(
        '[data-testid="uploaded-file"], .file-name, .file-item'
      );
    ...
    if ((await forms.count()) > 0) {
      ...
      if (await submitButton.isVisible()) {
        ...
        const validationErrors = page.locator(
          '.error, .invalid, [data-testid="error"], .validation-error'
        );
  ...
  test('should handle modal and dialog interactions', async ({ page }) => {
    ...
    const modalTriggers = page.locator(
      '[data-testid="modal-trigger"], [data-testid="dialog-trigger"], button:has-text("Settings"), button:has-text("Help")'
    );
    ...
    if ((await modalTriggers.count()) > 0) {
      ...
      const modal = page.locator(
        '[data-testid="modal"], .modal, .dialog, [role="dialog"]'
      );
  ...
  test('should handle tooltips and hover states', async ({ page }) => {
    ...
    const tooltipElements = page.locator(
    ...
    if ((await tooltipElements.count()) > 0) {
      ...
      const tooltip = page.locator(
        '.tooltip, [data-testid="tooltip"], [role="tooltip"]'
      );
  ...
  test('should handle loading states', async ({ page }) => {
    ...
    const loadingTriggers = page.locator(
      'button:has-text("Load"), button:has-text("Refresh"), [data-testid="load-trigger"]'
    );
    ...
    if ((await loadingTriggers.count()) > 0) {
      ...
      const loadingIndicators = page.locator(
        '[data-testid="loading"], .loading, .spinner, [data-testid="spinner"]'
      );
