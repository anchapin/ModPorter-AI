...
async def simulate_conversion(job_id: str):
    ...
    jobs[job_id]["message"] = "Analyzing mod structure..."

    await asyncio.sleep(3)
    jobs[job_id]["progress"] = 50
    jobs[job_id]["message"] = "Converting blocks and items..."

    await asyncio.sleep(3)
    jobs[job_id]["progress"] = 75
    jobs[job_id]["message"] = "Generating Bedrock format..."

    await asyncio.sleep(2)
    jobs[job_id]["status"] = "completed"
    jobs[job_id]["progress"] = 100
    jobs[job_id]["message"] = "Conversion completed successfully!"
    jobs[job_id]["result_url"] = f"/api/v1/convert/{job_id}/download"

    # Create a mock output file
    output_filename = f"{job_id}_converted.mcaddon"
    output_path = os.path.join(outputs_dir, output_filename)
    with open(output_path, "w") as f:
        f.write(f"Mock converted file for job {job_id}\nOriginal file: {jobs[job_id]['original_filename']}\nConverted at: {datetime.utcnow().isoformat()}")
