    ...
    def track_call(
        ...
        with self._lock:
            ...
            if total_cost >= self._cost_alert_threshold:
                self._trigger_alert(
                    "cost_threshold",
                    {"total_cost": total_cost, "threshold": self._cost_alert_threshold},
                )
    ...
    def get_total_usage(self) -> Dict:
        ...
        with self._lock:
            ...
            total_input_tokens = sum(
                stats.total_input_tokens for stats in self._model_stats.values()
            )
            total_output_tokens = sum(
                stats.total_output_tokens for stats in self._model_stats.values()
            )
            total_cost = sum(stats.total_cost for stats in self._model_stats.values())
...
def track_llm_call(
    model: str, provider: str, input_tokens: int, output_tokens: int, duration: float, **kwargs
) -> LLMCall:
    """
