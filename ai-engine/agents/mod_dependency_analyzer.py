...
class ModDependencyAnalyzer:
    ...
    def analyze_from_manifest(
        self, manifest_data: Dict[str, Any], source: str = "curseforge"
        ...
        try:
            ...
            warnings = self._generate_warnings(
                graph, circular_deps, version_conflicts, missing_deps
            )
            ...
            return DependencyAnalysisResult(
                graph=DependencyGraph(), success=False, error_message=str(e)
            )
    ...
    def _build_dependency_graph(
        self, manifest_data: Dict[str, Any], source: str
    ) -> DependencyGraph:
    ...
    def _generate_warnings(
        ...
        if circular_deps:
            warnings.append(f"Found {len(circular_deps)} circular dependency(ies) in modpack")

        if version_conflicts:
            warnings.append(f"Found {len(version_conflicts)} version conflict(s) between mods")
    ...
    def resolve_conflicts(
        ...
        for conflict in result.version_conflicts:
            if strategy == ConflictResolution.EXCLUDE_CONFLICTING:
                ...
                conflict.resolved_version = min(conflict.versions, key=self._version_sort_key)
    ...
    def generate_report(self, result: DependencyAnalysisResult) -> Dict[str, Any]:
        ...
        """
        report = {"success": result.success, "total_mods": len(result.graph.mods), "load_order": []}
        ...
        for mod_id in result.recommended_load_order:
            ...
            if mod:
                report["load_order"].append(
                    {"id": mod_id, "name": mod.name, "version": mod.version}
                )
        ...
        if result.circular_dependencies:
            report["circular_dependencies"] = [
                {"mods": dep.mods, "description": dep.description}
        ...
        if result.missing_dependencies:
            report["missing_dependencies"] = [
                {"id": dep.mod_id, "name": dep.mod_name, "type": dep.dependency_type.value}
