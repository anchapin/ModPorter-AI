...
class FilePackager:
    ...
    def package_addon(self, addon_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                logger.info(
                    f"Successfully created .mcaddon: {output_path} ({result['file_size']} bytes)"
                )
    ...
    def _organize_pack_structure(
        self,
        temp_path: Path,
        behavior_pack_path: Optional[str],
        resource_pack_path: Optional[str],
        source_directories: List[str],
    ) -> Dict[str, Any]:
        """Organize pack directories into proper Bedrock structure."""
        pack_info = {"behavior_packs": [], "resource_packs": [], "total_files": 0}
