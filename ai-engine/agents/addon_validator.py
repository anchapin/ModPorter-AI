...
class AddonValidator:
    ...
    def _validate_behavior_pack_structure(
        self, files: List[str], pack_name: str, result: Dict[str, Any]
    ):
    ...
    def _validate_single_manifest(
        self, manifest: Dict[str, Any], path: str, result: Dict[str, Any]
    ):
        ...
        known_capabilities = ["chemistry", "experimental_custom_ui", "script_eval"]

        for capability in capabilities:
            if capability not in known_capabilities:
                result["warnings"].append(f"Unknown capability '{capability}' in {path}")
    ...
    def _validate_texture_file(
        self, zipf: zipfile.ZipFile, file_path: str, file_size: int, result: Dict[str, Any]
    ):
        """Validate texture files."""
        # Check file size
        if file_size > 5 * 1024 * 1024:  # 5MB
            result["warnings"].append(
                f"Large texture file: {file_path} ({file_size / 1024 / 1024:.1f}MB)"
            )
        ...
        try:
            with zipf.open(file_path) as f:
                ...
                if content.count("{") != content.count("}"):
                    result["warnings"].append(
                        f"Potential syntax error in {file_path}: mismatched braces"
                    )
        except Exception as e:
    ...
    def _check_bedrock_compatibility(self, zipf: zipfile.ZipFile, result: Dict[str, Any]):
        ...
        for manifest_path in manifest_files:
            try:
                ...
                if (
                    min_engine
                    and self._compare_versions(min_engine, compatibility["min_version_detected"])
                    > 0
                ):
