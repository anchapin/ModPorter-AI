...
class RAGCrew:
    def __init__(self, model_name: str = "gpt-4", llm=None, use_tool_registry: bool = True):
        ...
        if self.use_tool_registry:
            ...
            logger.info(
                f"Tool registry initialized with {len(self.tool_registry.list_available_tools())} tools"
            )
        else:
    ...
    def validate_tool_configuration(self) -> dict:
        ...
        """
        validation_results = {"valid_tools": [], "invalid_tools": [], "warnings": []}
    ...
    def test_web_search_integration(
        self, test_query: str = "Minecraft Bedrock Edition scripting API"
    ) -> dict:
        """
        ...
        try:
            ...
            if not web_search_tool:
                return {"status": "error", "message": "WebSearchTool not found in researcher tools"}
...
if __name__ == "__main__":
    ...
    print(
        f"Loaded Writer tools: {[tool.__name__ if hasattr(tool, '__name__') else str(tool) for tool in rag_crew.writer.tools]}"
    )
