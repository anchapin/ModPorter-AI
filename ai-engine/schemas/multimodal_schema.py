...
class EmbeddingVector(BaseModel):
    ...
    confidence_score: Optional[float] = Field(
        None, description="Model confidence in embedding quality"
    )
    similarity_threshold: Optional[float] = Field(
        None, description="Minimum similarity for matches"
    )

    # Processing metadata
    model_version: Optional[str] = Field(None, description="Version of the embedding model")
    preprocessing_steps: List[str] = Field(
        default_factory=list, description="Applied preprocessing steps"
    )
...
class ImageMetadata(BaseModel):
    ...
    minecraft_asset_type: Optional[str] = Field(
        None, description="Type of Minecraft asset (block, item, entity)"
    )
    texture_category: Optional[str] = Field(
        None, description="Texture category (blocks, items, entities)"
    )
    animation_frames: Optional[int] = Field(
        None, description="Number of animation frames if animated"
    )

    # Processing information
    preprocessing_applied: List[str] = Field(
        default_factory=list, description="Applied image preprocessing"
    )
...
class CodeMetadata(BaseModel):
    ...
    # Conversion metadata
    conversion_confidence: Optional[float] = Field(
        None, description="Confidence in conversion accuracy"
    )
    conversion_notes: List[str] = Field(
        default_factory=list, description="Notes about conversion process"
    )
...
class SearchQuery(BaseModel):
    ...
    # Model preferences
    preferred_models: Optional[List[EmbeddingModel]] = Field(
...
class SearchResult(BaseModel):
    ...
    embedding_model_used: EmbeddingModel = Field(
    ...
    match_explanation: Optional[str] = Field(
        None, description="Explanation of why this result matched"
    )
    ...
    return {
        "tables": {
                ...
                "indexes": [
                    {
                        ...
                        "columns": ["content_type", "processing_status"],
                    },
            ...
            "embedding_vectors": {
                "columns": [
                    {
                        "name": "document_id",
                        "type": "VARCHAR(255)",
                        "foreign_key": "multimodal_documents.id",
                    },
                ...
                "indexes": [
                    {
                        "name": "idx_model_dimension",
                        "columns": ["model_name", "embedding_dimension"],
                    },
                    {
