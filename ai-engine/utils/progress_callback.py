...
class ProgressCallback:
    ...
    async def send_progress(
        ...
        try:
            ...
            await self.redis_client.set(key, json.dumps(progress_data), ex=3600)
    ...
    async def broadcast_agent_update(
        self, agent: str, progress: int, message: str, details: Optional[Dict[str, Any]] = None
    ):
        """Broadcast an agent progress update"""
        await self.send_progress(
            agent=agent, status="in_progress", progress=progress, message=message, details=details
        )
