    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            return _execute_with_retry(
                func, rate_limiter, rate_config or RateLimitConfig(), *args, **kwargs
            )
...
def _execute_with_retry(
    func: Callable, rate_limiter: RateLimiter, rate_config: RateLimitConfig, *args, **kwargs
) -> Any:
        ...
        try:
            ...
            if any(keyword in error_msg for keyword in ["rate limit", "429", "too many requests"]):
                if attempt < rate_config.max_retries:
                    wait_time = min(
                        rate_config.base_delay * (rate_config.backoff_factor**attempt),
                        rate_config.max_delay,
                    )
                    logger.warning(
                        f"Rate limit hit, retrying in {wait_time:.2f}s (attempt {attempt + 1}/{rate_config.max_retries})"
                    )
                    time.sleep(wait_time)
                    continue
                else:
            ...
            # Check if it's a temporary error
                ...
                for keyword in ["timeout", "connection", "temporary", "503", "502"]
            ):
                if attempt < rate_config.max_retries:
                    wait_time = min(
                        rate_config.base_delay * (rate_config.backoff_factor**attempt),
                    ...
                    logger.error(
                        f"Temporary error persisted after {rate_config.max_retries} retries"
                    )
                    raise e
...
class RateLimitedChatOpenAI:
    ...
    def generate(self, messages, **kwargs):
        ...
        return _execute_with_retry(
            self._base_llm.generate, self.rate_limiter, self.rate_config, messages, **kwargs
        )

    def predict(self, text, **kwargs):
        """Implement the predict method with rate limiting"""
        return _execute_with_retry(
            self._base_llm.predict, self.rate_limiter, self.rate_config, text, **kwargs
        )
...
def create_ollama_llm(model_name: str = "llama3.2", base_url: str = None, **kwargs):
    ...
    try:
        ...
        clean_model_name = (
            model_name.replace("ollama/", "") if model_name.startswith("ollama/") else model_name
        )
        ...
        wrapper = LiteLLMOllamaWrapper(model_name=clean_model_name, base_url=base_url, **kwargs)
