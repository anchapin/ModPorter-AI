...
class JavaCodeChunker:
    ...
    def __init__(self):
        self.class_pattern = re.compile(
            r"^\s*(?:public|private|protected)?\s*(?:static|final)?\s*class\s+(\w+)", re.MULTILINE
        )
        ...
        self.field_pattern = re.compile(
            r"^\s*(?:public|private|protected)?\s*(?:static|final)?\s*\w+\s+(\w+)\s*[=;]",
            re.MULTILINE,
        )
        ...
        for class_match in self.class_pattern.finditer(code):
            ...
            if class_content:
                ...
                method_chunks = self._extract_methods_from_class(
                    class_content, class_name, class_start
                )
    ...
    def _extract_methods_from_class(
        self, class_content: str, class_name: str, class_start_line: int
    ) -> List[Chunk]:
                ...
                if current_section:
                    chunks.append(
                        Chunk(
                            ...
                            chunk_type=ChunkType.CONFIGURATION,
                            metadata={"config_key": current_key, "config_type": "yaml_section"},
                        )
                    )
...
class AdvancedChunker:
    ...
    def chunk_content(
        self, content: str, content_type: str, file_path: str = "", **kwargs
    ) -> List[Chunk]:
        """
        ...
        elif content_type == "configuration" or file_path.endswith(
