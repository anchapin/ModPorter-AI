...
class BedrockDocsScraper:
                ...
                try:
                    ...
                    if e.response.status_code in [429, 503, 504]:  # Rate limited or server errors
                        if attempt < max_retries - 1:
                            ...
                            logger.warning(
                                f"HTTP {e.response.status_code} for {url}, retrying in {wait_time}s"
                            )
                            await asyncio.sleep(wait_time)
                            continue
                    raise
        ...
        for selector in selectors:
            ...
            for element in elements:
                ...
                if code_text and len(code_text) > 10:  # Filter out small snippets
                    ...
                    code_blocks.append(
                        {"code": code_text, "language": language, "selector": selector}
                    )
    ...
    def _extract_structured_data(self, soup: BeautifulSoup, url: str) -> Dict:
        ...
        """
        structured_data = {"tables": [], "lists": [], "metadata": {}, "headings": []}
    ...
    def _parse_html_content(self, html_content: str, base_url: str) -> tuple[str, List[str], Dict]:
        ...
        # Update statistics
        if code_blocks:
            self.stats["api_examples_found"] += len(
                [cb for cb in code_blocks if cb["language"] in ["json", "javascript"]]
            )
            self.stats["json_schemas_found"] += len(
        ...
        for a_tag in soup.find_all("a", href=True):
            ...
            if self._is_relevant_link(full_url, base_domain, a_tag.get_text(strip=True)):
                ...
                if (
                    clean_url not in self.scraped_urls
                    and clean_url not in self.failed_urls
                    and validators.url(clean_url)
                ):
    ...
    async def scrape_site(
        ...
        try:
            if "text/html" in content_type:
                documents.extend(
                    await self._process_html_content(
                        response.text, start_url, max_depth, current_depth
                    )
                )
                ...
                documents.extend(
                    await self._process_text_content(response.text, start_url, content_type)
                )
            else:
    ...
    async def _process_html_content(
        self, html_content: str, url: str, max_depth: int, current_depth: int
    ) -> List[Document]:
        """
    ...
    async def _scrape_with_semaphore(
        self, semaphore: asyncio.Semaphore, url: str, max_depth: int, current_depth: int
    ) -> List[Document]:
        """
    ...
    async def _process_text_content(
        self, text_content: str, url: str, content_type: str
    ) -> List[Document]:
        """
    ...
    async def scrape_documentation(self) -> List[Document]:
        ...
        logger.info(
            f"Starting comprehensive Bedrock documentation scrape from {len(self.target_urls)} sources"
        )
        ...
        for site_name, base_url in self.target_urls.items():
            ...
            try:
                ...
                logger.info(
                    f"Finished scraping {site_name}. Found {len(site_documents)} documents."
                )
    ...
    async def _generate_scraping_report(self) -> None:
        ...
        report = {
            ...
            "quality_metrics": {
                "success_rate": (
                    self.stats["total_urls_scraped"] / max(self.stats["total_urls_discovered"], 1)
                )
                * 100,
                "average_documents_per_url": self.stats["total_documents_created"]
                / max(self.stats["total_urls_scraped"], 1),
...
async def main():
    ...
    try:
        ...
        for i, doc in enumerate(documents[:5]):  # Print info for the first 5 documents
            logger.info(
                f"Doc {i + 1}: Source: {doc.source}, Type: {doc.doc_type}, Length: {len(doc.content)}"
            )
