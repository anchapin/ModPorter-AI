...
class ContextTrimmer:
    ...
    def trim_context(
        ...
        """
        available_tokens = (
            self.max_tokens - self.estimate_tokens(system_prompt) - completion_reserve
        )
...
class CostTracker:
    ...
    def get_session_stats(self) -> Dict:
        ...
        return {
            ...
            "average_tokens_per_request": sum(u.total_tokens for u in self._usage)
            // len(self._usage),
        }
...
class TokenOptimizer:
    ...
    def __init__(
        ...
        self.cache = (
            PromptCache(max_size=cache_size, ttl_hours=cache_ttl_hours) if enable_caching else None
        )
    ...
    def get_stats(self) -> Dict:
        ...
        if self.enable_caching and self.cache:
            ...
            if self.total_requests > 0:
                stats["cache_hit_rate"] = round(self.cache_hits / self.total_requests * 100, 2)
    ...
    def check_budget(self) -> Dict[str, str]:
        ...
        if daily_cost >= thresholds["daily_budget_critical"]:
            return {
                "status": "critical",
                "message": f"Daily cost ${daily_cost:.2f} exceeds critical threshold ${thresholds['daily_budget_critical']}",
            }
        elif daily_cost >= thresholds["daily_budget_warning"]:
            return {
                "status": "warning",
                "message": f"Daily cost ${daily_cost:.2f} exceeds warning threshold ${thresholds['daily_budget_warning']}",
            }
...
def get_token_optimizer(model: str = "gpt-4-turbo", **kwargs) -> TokenOptimizer:
...
def optimize_llm_call(
    ...
    optimized_prompt, used_context = optimizer.optimize_prompt(
        system_prompt or "You are a helpful assistant.", prompt, context_items
    )
