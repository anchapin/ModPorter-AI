    ...
    def validate_configuration(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if not torch.cuda.is_available() and self.gpu_enabled:
                return (
                    False,
                    f"GPU enabled but PyTorch CUDA not available for {self.gpu_type.value}",
                )
            ...
            if self.gpu_type == GPUType.NVIDIA:
                try:
                    ...
                    if "CUDAExecutionProvider" not in ort.get_available_providers():
                        return (
                            False,
                            "NVIDIA GPU selected but CUDA execution provider not available",
                        )
                except ImportError:
                ...
                try:
                    ...
                    if not any(
                        p in providers for p in ["ROCMExecutionProvider", "DmlExecutionProvider"]
                    ):
                        return (
                            False,
                            "AMD GPU selected but no ROCm/DirectML execution provider available",
                        )
                except ImportError:
                    pass  # ONNX Runtime not required for validation
