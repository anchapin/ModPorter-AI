...
class CodeAwareEmbeddingGenerator:
        ...
        try:
            if inspect.iscoroutinefunction(self.base_generator.generate_embeddings):
                ...
                embeddings = await asyncio.to_thread(
                    self.base_generator.generate_embeddings, [enhanced_content]
                )
    ...
    def _extract_code_features(self, chunk: Chunk) -> Dict[str, Any]:
        ...
        minecraft_keywords = self.code_keywords.get("minecraft", [])
        features["minecraft_keyword_count"] = sum(
            content.count(keyword) for keyword in minecraft_keywords
        )
...
class ImageEmbeddingGenerator:
    ...
    async def generate_image_embedding(
        self, image_path: str, metadata: Dict[str, Any] = None
...
class MultiModalEmbeddingGenerator:
    ...
    async def _generate_text_embedding(
        self, text: str, strategy: EmbeddingStrategy
    ) -> EmbeddingResult:
        ...
        try:
            ...
            if inspect.iscoroutinefunction(self.text_generator.generate_embeddings):
                ...
                embeddings = await asyncio.to_thread(
                    self.text_generator.generate_embeddings, [text]
                )
        except Exception as e:
    ...
    async def _generate_multimodal_embedding(
        self, content_dict: Dict[str, Any], strategy: EmbeddingStrategy
    ) -> Optional[EmbeddingResult]:
        """
        ...
        if "text" in content_dict and content_dict["text"]:
            text_result = await self._generate_text_embedding(
                content_dict["text"], EmbeddingStrategy.TEXT_ONLY
            )
            if text_result:
        ...
        if "image_path" in content_dict and content_dict["image_path"]:
            image_result = await self.image_generator.generate_image_embedding(
                content_dict["image_path"], content_dict.get("metadata", {})
            )
            if image_result:
    ...
    async def batch_generate_embeddings(
        ...
        logger.info(
            f"Generated {len([r for r in results if r is not None])} embeddings out of {len(contents)} items"
        )
        return results
