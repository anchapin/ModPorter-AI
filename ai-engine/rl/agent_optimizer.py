...
class AgentPerformanceOptimizer:
    ...
    def track_agent_performance(
        self, agent_type: str, episodes: List[TrainingEpisode]
        ...
        logger.info(
            f"Performance analysis completed for {agent_type}. "
    ...
    def _generate_optimization_recommendations(
        ...
        if avg_reward < self.performance_thresholds["poor"]:
            ...
            recommendations.append(
                "Recommend: Immediate model retraining with enhanced reward signals"
            )
        ...
        if improvement_trend == "declining":
            ...
            recommendations.append(
                "Suggest: Performance has plateaued - try new training strategies"
            )
    ...
    def _get_agent_specific_recommendations(
        self, agent_type: str, quality_breakdown: Dict[str, float]
    ) -> List[str]:
        ...
        if agent_type == "java_analyzer":
            if quality_breakdown.get("completeness", 0) < 0.7:
                recommendations.append(
                    "Java Analyzer: Improve parsing accuracy for complex mod structures"
                )

        elif agent_type == "asset_converter":
            if quality_breakdown.get("user_experience", 0) < 0.6:
                recommendations.append(
                    "Asset Converter: Focus on texture quality and visual fidelity"
                )
            ...
            if quality_breakdown.get("performance", 0) < 0.6:
                recommendations.append(
                    "Conversion Planner: Optimize planning algorithms for efficiency"
                )
    ...
    def _determine_training_priority(
        self, average_reward: float, improvement_trend: str, success_rate: float
    ) -> str:
        """Determine training priority level for the agent."""

        # Critical priority
        if (
            average_reward < self.performance_thresholds["critical"]
            or success_rate < 0.3
            or improvement_trend == "declining"
        ):
            return "critical"

        # High priority
        if average_reward < self.performance_thresholds["poor"] or success_rate < 0.5:
            return "high"

        # Medium priority
        if average_reward < self.performance_thresholds["good"] or improvement_trend == "stable":
            return "medium"
    ...
    def compare_agents(self, agent_types: List[str]) -> AgentComparisonReport:
        ...
        for agent_type in agent_types:
            if agent_type in self.performance_history and self.performance_history[agent_type]:
    ...
    def _calculate_performance_rankings(
        ...
        sorted_agents = sorted(
            agent_metrics.items(), key=lambda x: x[1].average_reward, reverse=True
        )
    ...
    def _identify_strengths_weaknesses(
        ...
        for agent_type, metrics in agent_metrics.items():
            ...
            strengths[agent_type] = (
                agent_strengths if agent_strengths else ["No significant strengths identified"]
            )
            weaknesses[agent_type] = (
                agent_weaknesses if agent_weaknesses else ["No significant weaknesses identified"]
            )
    ...
    def _generate_ensemble_recommendations(
        ...
        if len(strong_areas) > 1:
            recommendations.append(
                "Consider ensemble approach leveraging different agent strengths"
            )
        ...
        return (
            recommendations
            if recommendations
            else ["No specific ensemble recommendations at this time"]
        )
    ...
    def get_system_wide_metrics(self) -> Dict[str, Any]:
        ...
        system_metrics = {
            ...
            "system_performance": {
                ...
                "reward_variance": statistics.variance(all_rewards)
    ...
    def _generate_system_recommendations(
        self, latest_metrics: Dict[str, AgentPerformanceMetrics]
    ) -> List[str]:
        ...
        if critical_agents:
            recommendations.append(
                f"Critical: {', '.join(critical_agents)} require immediate attention"
            )
        ...
        if statistics.mean(all_rewards) < self.performance_thresholds["acceptable"]:
            recommendations.append(
                "System-wide performance below threshold - review training data quality"
            )
        ...
        if max(episodes_per_agent) > 2 * min(episodes_per_agent):
            recommendations.append(
                "Unbalanced training data - ensure all agents receive adequate episodes"
            )

        return (
            recommendations
            if recommendations
            else ["System performance is within normal parameters"]
        )
