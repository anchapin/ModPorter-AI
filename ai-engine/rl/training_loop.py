...
class RLTrainingLoop:
    ...
    async def run_training_cycle(self) -> TrainingMetrics:
        ...
        try:
            ...
            logger.info(
                f"Training cycle completed. Episodes: {len(self.episodes)}, "
    ...
    async def _fetch_training_data(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            training_data = await fetch_training_data_from_backend(
                backend_url=self.backend_url, skip=0, limit=self.config["training_data_limit"]
            )
    ...
    async def _create_episode_from_data(
        self, training_item: Dict[str, Any]
    ...
    def _calculate_performance_trend(self, agent_type: str) -> str:
        ...
        first_half_avg = sum(recent_rewards[: len(recent_rewards) // 2]) / (
            len(recent_rewards) // 2
        )
        second_half_avg = sum(recent_rewards[len(recent_rewards) // 2 :]) / (
            len(recent_rewards) - len(recent_rewards) // 2
        )
    ...
    def _generate_training_recommendations(self) -> List[str]:
        ...
        if self.training_metrics.average_reward < 0.3:
            recommendations.append(
                "Overall performance is low - consider adjusting reward parameters"
            )
        ...
        for agent_type, perf in self.training_metrics.agent_performance.items():
            if perf["average_reward"] < 0.2:
                ...
                recommendations.append(
                    f"{agent_type} agent performing well - good candidate for knowledge transfer"
                )
