...
class RewardSignalGenerator:
    ...
    def generate_reward_signal(
        ...
        logger.info(
            f"Reward signal generated: {total_reward:.3f} "
    ...
    def _calculate_base_reward(
        self, conversion_metadata: Dict[str, Any], quality_metrics: QualityMetrics
    ) -> float:
    ...
    def _calculate_quality_bonus(self, quality_metrics: QualityMetrics, agent_type: str) -> float:
        ...
        if agent_type == "java_analyzer":
            ...
            completeness_bonus = (quality_metrics.completeness_score - 0.5) * agent_config.get(
                "completeness_emphasis", 1.0
            )
            quality_bonus += completeness_bonus

        elif agent_type == "asset_converter":
            # Asset converter gets bonus for asset quality
            asset_bonus = (quality_metrics.asset_conversion_score - 0.5) * agent_config.get(
                "asset_quality_emphasis", 1.0
            )
            quality_bonus += asset_bonus

        elif agent_type == "behavior_translator":
            # Behavior translator gets bonus for correctness
            correctness_bonus = (quality_metrics.correctness_score - 0.5) * agent_config.get(
                "correctness_emphasis", 1.0
            )
            ...
            quality_bonus += error_detection_bonus * agent_config.get(
                "error_detection_emphasis", 1.0
            )
    ...
    def analyze_reward_trends(self, reward_history: List[RewardSignal]) -> Dict[str, Any]:
        ...
        if analysis["agent_performance"]:
            best_agent = max(
                analysis["agent_performance"].items(), key=lambda x: x[1]["average_reward"]
            )
            worst_agent = min(
                analysis["agent_performance"].items(), key=lambda x: x[1]["average_reward"]
            )
    ...
    def save_reward_signals(self, reward_signals: List[RewardSignal], output_path: str) -> None:
