...
class DataCollectionStore:
    ...
    def get_example(self, example_id: str) -> Optional[ConversionExample]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor = conn.execute(
                    "SELECT * FROM conversion_examples WHERE example_id = ?", (example_id,)
                )
                row = cursor.fetchone()
    ...
    def get_examples_by_status(
        self, status: LabelStatus, limit: int = 100
    ...
    def get_training_data(
        self, min_quality: float = 0.5, limit: int = 1000
    ) -> List[ConversionExample]:
        """
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute(
                    ...
                    WHERE example_id = ?
                """,
                    (quality_label, labeler_id, now, json.dumps(issues or []), now, example_id),
                )
                conn.commit()
    ...
    def get_metrics(self) -> CollectionMetrics:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute("SELECT * FROM collection_metrics WHERE id = 1")
                row = cursor.fetchone()
    ...
    def export_training_data(
        self, output_path: str, export_format: str = "jsonl", min_quality: float = 0.5
    ) -> int:
        """
...
class DataCollectionPipeline:
    ...
    async def collect_conversion_result(
        ...
        example = ConversionExample(
            ...
            completeness_score=quality_metrics.get("completeness_score")
            if quality_metrics
            else None,
    ...
    def _determine_outcome(
        self, metadata: Dict[str, Any], quality: Optional[Dict[str, Any]]
    ) -> ConversionOutcome:
    ...
    async def process_user_feedback(
        ...
        return await asyncio.to_thread(
            self.store.add_user_feedback, example.example_id, feedback_type, comment, rating
        )
    ...
    def _find_example_by_job_id(self, job_id: str) -> Optional[ConversionExample]:
        ...
        try:
            with sqlite3.connect(self.store.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute(
                    "SELECT * FROM conversion_examples WHERE job_id = ?", (job_id,)
                )
                row = cursor.fetchone()
    ...
    def create_labeling_task(self, example_id: str, priority: int = 1) -> LabelingTask:
        ...
        try:
            with sqlite3.connect(self.store.db_path) as conn:
                ...
                """,
                    (task.task_id, task.example_id, task.priority, task.status, task.created_at),
                )
                conn.commit()
        except Exception as e:
    ...
    def submit_label(
        self, task_id: str, quality_label: str, labeler_id: str, issues: Optional[List[str]] = None
    ) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.store.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute("SELECT * FROM labeling_tasks WHERE task_id = ?", (task_id,))
                row = cursor.fetchone()
            ...
            success = self.store.update_label(example_id, quality_label, labeler_id, issues)
    ...
    def auto_label_examples(self) -> int:
        ...
        for example in unlabeled:
            ...
            success = self.store.update_label(example.example_id, quality_label, "auto_labeler")
    ...
    def export_for_training(self, output_format: str = "jsonl", min_quality: float = 0.5) -> str:
        """
        ...
        count = self.store.export_training_data(str(output_path), output_format, min_quality)
        ...
        return {
            ...
            "feedback": {
                ...
                "positive_rate": (
                    ...
                    if metrics.total_user_feedback > 0
                    else 0
                ),
            },
        }
...
async def collect_conversion(
    ...
    return await pipeline.collect_conversion_result(
        job_id, original_mod_path, converted_addon_path, conversion_metadata, quality_metrics
    )
