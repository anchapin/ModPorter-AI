    ...
    def _create_test_jar(self, mod_id: str, block_name: str = "copper_block") -> Path:
        ...
        with zipfile.ZipFile(jar_path, "w") as jar:
            ...
            block_model = {
                ...
                "textures": {"all": f"{mod_id}:block/{block_name}"},
            }
            jar.writestr(
                f"assets/{mod_id}/models/block/{block_name}.json", json.dumps(block_model, indent=2)
            )
...
"""
            jar.writestr(
                f"net/{mod_id}/blocks/{block_name.title().replace('_', '')}Block.java", java_code
            )
...
"""
    ...
    def test_step3_packaging_agent_mvp(self):
        ...
        with tempfile.TemporaryDirectory() as temp_build_dir:
            ...
            result = self.packaging_agent.build_mcaddon_mvp(
                temp_dir=temp_build_dir, output_path=str(output_path), mod_name="test_mod"
            )
    ...
    def test_performance_benchmarks(self):
        ...
        for mod_id, block_count, description in benchmark_configs:
            ...
            with zipfile.ZipFile(jar_path, "w") as jar:
                ...
                for i in range(block_count):
                    ...
                    jar.writestr(
                        f"net/{mod_id}/blocks/{block_name.title().replace('_', '')}Block.java",
                        java_code,
                    )
        ...
        self.assertLess(
            slowest, 120.0, "Slowest case should be under 2 minutes (CI environments are slower)"
        )
