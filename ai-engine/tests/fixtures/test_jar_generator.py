...
class JarGenerator:
    ...
    def create_mod_jar(
        self, mod_name: str, blocks: List[str] = None, items: List[str] = None
    ) -> str:
...
"""
            java_files[
                f"com/example/{mod_name.lower()}/blocks/{block.title().replace('_', '')}.java"
            ] = block_class
...
"""
            java_files[
                f"com/example/{mod_name.lower()}/items/{item.title().replace('_', '')}.java"
            ] = item_class
...
def create_test_mod_suite(output_dir=None) -> Dict[str, str]:
    ...
    generator = JarGenerator(output_dir_str)

    mod_suite = {}

    # Simple mod with basic blocks
    mod_suite["simple_blocks"] = generator.create_mod_jar(
        "simple_blocks", blocks=["stone_block", "wood_block"], items=["stone_ingot"]
    )
    ...
    mod_suite["minimal_mod"] = generator.create_mod_jar(
        "minimal_mod", blocks=["basic_block"], items=["basic_item"]
    )
