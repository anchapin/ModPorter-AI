...
class TestBedrockTemplates:
    ...
    def basic_context(self):
        ...
        return {"namespace": "mod", "entity_name": "test_entity"}
    ...
    def test_hostile_mob_template(self, template_engine, basic_context):
        ...
        result = template_engine.render_template("entity", {"behavior": "hostile"}, basic_context)
    ...
    def test_passive_mob_template(self, template_engine, basic_context):
        ...
        context = {"namespace": "mod", "entity_name": "sheep", "entity_family": "passive"}
        result = template_engine.render_template("entity", {"behavior": "passive"}, context)
        data = json.loads(result)
        assert "minecraft:entity" in data
    ...
    def test_tool_template(self, template_engine):
        ...
        result = template_engine.render_template("item", {"type": "tool"}, context)
        data = json.loads(result)
        assert "minecraft:item" in data

    def test_armor_template(self, template_engine):
        """Test armor item template generates valid JSON."""
        context = {"namespace": "mod", "item_name": "diamond_helmet", "armor": 6, "toughness": 2}
        result = template_engine.render_template("item", {"type": "armor"}, context)
        data = json.loads(result)
        assert "minecraft:item" in data

    def test_consumable_template(self, template_engine):
        """Test consumable item template generates valid JSON."""
        context = {"namespace": "mod", "item_name": "apple", "food": 4}
        result = template_engine.render_template("item", {"type": "consumable"}, context)
        data = json.loads(result)
        assert "minecraft:item" in data
    ...
    def test_shaped_crafting_recipe(self, template_engine):
        ...
        context = {
            ...
            "pattern": ["X", "X", "|"],
            "recipe_keys": {"X": {"item": "minecraft:diamond"}, "|": {"item": "minecraft:stick"}},
            "result": {"item": "minecraft:diamond_sword", "count": 1},
        }
        result = template_engine.render_template("recipe", {"type": "shaped"}, context)
        data = json.loads(result)
    ...
    def test_shapeless_recipe(self, template_engine):
        ...
        result = template_engine.render_template("recipe", {"type": "shapeless"}, context)
        data = json.loads(result)
    ...
    def test_smelting_recipe(self, template_engine):
        ...
        result = template_engine.render_template("recipe", {"type": "smelting"}, context)
        data = json.loads(result)
    ...
    def test_container_block_template(self, template_engine):
        ...
        context = {"namespace": "mod", "block_name": "chest", "container_type": "chest"}
        result = template_engine.render_template("block", {"container": True}, context)
        data = json.loads(result)
    ...
    def test_interactive_block_template(self, template_engine):
        ...
        context = {"namespace": "mod", "block_name": "button", "interactive_type": "click"}
        result = template_engine.render_template("block", {"interactive": True}, context)
        data = json.loads(result)
        assert "minecraft:block" in data
    ...
    def test_validate_template_output(self, template_engine):
        ...
        assert (
            template_engine.validate_template_output(valid_block, TemplateType.BASIC_BLOCK) is True
        )
