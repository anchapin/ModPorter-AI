...
class TestLabelingTask:
    ...
    def test_task_creation(self):
        ...
        task = LabelingTask(task_id="task_001", example_id="ex_001", priority=2)
    ...
    def test_task_to_dict(self):
        ...
        task = LabelingTask(task_id="task_001", example_id="ex_001")
...
class TestCollectionMetrics:
    ...
    def test_metrics_creation(self):
        ...
        metrics = CollectionMetrics(total_examples=100, labeled_examples=50, verified_examples=25)
...
class TestDataCollectionStore:
    ...
    def test_update_label(self, store, sample_example):
        ...
        success = store.update_label(sample_example.example_id, "good", "labeler_001", ["issue1"])
    ...
    def test_add_user_feedback(self, store, sample_example):
        ...
        success = store.add_user_feedback(
            sample_example.example_id, "thumbs_up", "Great conversion!", 5
        )
...
class TestDataCollectionPipeline:
    ...
    async def test_process_user_feedback(self, pipeline):
        ...
        success = await pipeline.process_user_feedback(
            job_id="job_feedback", feedback_type="thumbs_up", comment="Great job!", rating=5
        )
...
class TestEdgeCases:
    ...
    async def test_process_feedback_nonexistent_job(self, pipeline):
        ...
        success = await pipeline.process_user_feedback(
            job_id="nonexistent_job", feedback_type="thumbs_up"
        )
    ...
    def test_determine_outcome_variations(self, pipeline):
        ...
        outcome = pipeline._determine_outcome({"status": "completed"}, {"overall_score": 0.9})
        assert outcome == ConversionOutcome.SUCCESS

        # Partial with medium quality
        outcome = pipeline._determine_outcome({"status": "completed"}, {"overall_score": 0.6})
        assert outcome == ConversionOutcome.PARTIAL

        # Failure
        outcome = pipeline._determine_outcome({"status": "failed"}, None)
        assert outcome == ConversionOutcome.FAILURE

        # Error/unknown
        outcome = pipeline._determine_outcome({"status": "unknown"}, None)
