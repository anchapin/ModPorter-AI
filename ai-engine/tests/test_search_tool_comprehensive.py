...
def search_tool_instance(mock_vector_client):
    ...
    with patch.object(SearchTool, 'vector_client', mock_vector_client):
        tool = SearchTool()
...
class TestSearchToolInitialization:
    ...
    def test_get_tools_returns_list(self, search_tool_instance):
        ...
        assert len(tools) > 0
        # Should return callable tools
        for tool in tools:
            assert callable(tool) or hasattr(tool, '__call__')
...
class TestSemanticSearch:
    ...
    async def test_semantic_search_json_input(self, search_tool_instance, sample_search_results):
        ...
        result = await SearchTool.semantic_search(query_data)
    ...
    async def test_semantic_search_string_input(self, search_tool_instance, sample_search_results):
        ...
        result = await SearchTool.semantic_search("block implementation")
    ...
    async def test_semantic_search_exception_handling(self, search_tool_instance):
        ...
        result = await SearchTool.semantic_search("test query")
        result_data = json.loads(result)
...
class TestDocumentSearch:
    ...
    async def test_document_search_basic(self, search_tool_instance, sample_search_results):
        ...
        result = await SearchTool.document_search(query_data)
    ...
    @pytest.mark.asyncio
    async def test_document_search_with_source_filter(self, search_tool_instance, sample_search_results):
        """Test document search with source filtering."""
        filtered_results = [r for r in sample_search_results if r["source"] == "bedrock_docs"]
        ...
        for result_item in result_data["results"]:
            assert result_item["source"] == "bedrock_docs"
...
class TestSimilaritySearch:
    ...
    async def test_similarity_search_basic(self, search_tool_instance, sample_search_results):
        ...
        result = await SearchTool.similarity_search(query_data)
    ...
    @pytest.mark.asyncio
    async def test_similarity_search_filters_by_threshold(self, search_tool_instance):
        """Test that similarity search respects threshold."""
        low_similarity_results = [
            {"id": "doc_1", "content": "test", "similarity": 0.5},
            {"id": "doc_2", "content": "test", "similarity": 0.8},
        ]
        ...
        assert all(r.get("similarity", 1.0) >= 0.7 for r in result_data["results"])
...
class TestComponentLookup:
    ...
    async def test_component_lookup_basic(self, search_tool_instance, sample_search_results):
        ...
        result = await SearchTool.component_lookup(query_data)
...
class TestConversionExamples:
    ...
    async def test_conversion_examples_search(self, search_tool_instance, sample_search_results):
        ...
        result = await SearchTool.conversion_examples(query_data)
...
class TestSchemaValidationLookup:
    ...
    async def test_schema_lookup_basic(self, search_tool_instance, sample_search_results):
        ...
        result = await SearchTool.schema_validation_lookup(query_data)
...
class TestBedrockAPISearch:
    ...
    async def test_bedrock_api_search(self, search_tool_instance, sample_search_results):
        ...
        result = await SearchTool.bedrock_api_search(query_data)
...
class TestSearchToolPrivateMethods:
    ...
    def test_attempt_fallback_search(self, search_tool_instance):
        """Test _attempt_fallback_search private method."""
        fallback_results = [
            {"id": "fallback_1", "content": "result"}
        ]
...
class TestSearchToolErrorHandling:
    ...
    async def test_json_decode_error_handling(self, search_tool_instance):
        ...
        # Invalid JSON should be treated as string query
        result = await SearchTool.semantic_search("not a json string")
        
        result_data = json.loads(result)
        assert result_data.get("query") == "not a json string"
    
    @pytest.mark.asyncio
    async def test_network_error_handling(self, search_tool_instance):
        """Test handling of network errors."""
        search_tool_instance._perform_semantic_search = AsyncMock(
            side_effect=ConnectionError("Network unreachable")
        )
        
        result = await SearchTool.semantic_search("test")
        result_data = json.loads(result)
        
        assert "error" in result_data
    
    @pytest.mark.asyncio
    async def test_generic_exception_handling(self, search_tool_instance):
...
class TestSearchToolIntegration:
    ...
    async def test_multiple_searches_in_sequence(self, search_tool_instance, sample_search_results):
        ...
        for query in queries:
            result = await SearchTool.semantic_search(query)
    ...
    async def test_search_with_various_input_formats(self, search_tool_instance, sample_search_results):
        ...
        result1 = await SearchTool.semantic_search(json_input)
        
        # Test string input
        result2 = await SearchTool.semantic_search("test")
