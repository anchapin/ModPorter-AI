...
class TestBedrockManifestGenerator:
    ...
    def test_capabilities_determination(self):
        ...
        mod_data_with_ui = {
            ...
            "version": "1.0.0",
            "features": [{"type": "custom_ui", "name": "custom_menu"}],
        }
    ...
    def test_version_parsing_edge_cases(self):
        ...
        for version_input, expected in test_cases:
            ...
            assert result == expected, (
                f"Failed for input {version_input}: expected {expected}, got {result}"
            )

    def test_single_manifest_generation(self):
        """Test generation of individual manifests."""
        bp_manifest = self.generator.generate_single_manifest(
            PackType.BEHAVIOR, self.sample_mod_data
        )
        rp_manifest = self.generator.generate_single_manifest(
            PackType.RESOURCE, self.sample_mod_data
        )
...
class TestBlockItemGenerator:
        ...
        self.sample_java_items = [
            {
                "id": "test_item",
                "namespace": "testmod",
                "properties": {"max_stack_size": 64, "is_tool": False},
                "tags": ["misc"],
            }
        ]
        ...
        sample_recipes = [
            {
                ...
                "result": {"item": "testmod:test_block", "count": 1},
            }
        ]
    ...
    def test_block_properties_parsing(self):
        ...
        java_block = {
            "id": "light_block",
            "properties": {"hardness": 5.0, "light_level": 15, "flammable": True, "solid": False},
        }
        ...
        self.sample_java_entities = [
            {
                ...
                "namespace": "testmod",
                "category": "passive",
                "attributes": {"max_health": 30.0, "movement_speed": 0.3, "attack_damage": 0.0},
                "behaviors": [
    ...
    def test_validate_mcaddon_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with zipfile.ZipFile(mcaddon_path, "w") as zipf:
                # Add behavior pack
                zipf.writestr(
                    "behavior_packs/test_bp/manifest.json",
                    json.dumps(
                        {
                            "format_version": 2,
                            "header": {
