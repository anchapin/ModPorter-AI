...
class TestTaskStateLogging:
    ...
    def test_task_started_logging(self, caplog):
        ...
        task = TaskNode(
            task_id="start_test", agent_name="test_agent", agent_type="analyzer", input_data={}
        )
    ...
    def test_task_completed_logging(self, caplog):
        ...
        task = TaskNode(
            task_id="complete_test", agent_name="test_agent", agent_type="analyzer", input_data={}
        )
    ...
    def test_task_failed_logging(self, caplog):
        ...
        task = TaskNode(
            task_id="fail_test", agent_name="test_agent", agent_type="analyzer", input_data={}
        )
...
class TestDeadlockDetection:
    ...
    def test_has_permanently_failed_tasks(self, task_graph):
        ...
        task = TaskNode(
            task_id="fail_task", agent_name="agent", agent_type="type", input_data={}, max_retries=1
        )
...
class TestWorkerPoolMonitoring:
    ...
    def test_worker_pool_get_stats(self):
        ...
        with WorkerPool(
            max_workers=4, worker_type=WorkerType.THREAD, enable_monitoring=False
        ) as pool:
            stats = pool.get_worker_stats()
...
class TestParallelExecution:
    ...
    def test_dynamic_task_spawning(self):
        ...
        def spawn_callback(result):
            ...
            if result.get("spawn"):
                return [TaskNode(f"spawned_{i}", f"agent_{i}", "spawned", {}) for i in range(2)]
            return []

        task = TaskNode("main_task", "agent", "type", {}, spawn_callback=spawn_callback)
    ...
    def test_orchestrator_create_workflow(self, orchestrator):
        ...
        try:
            workflow = orchestrator.create_conversion_workflow(
                mod_path=temp_path, output_path="/tmp/output", temp_dir="/tmp/temp"
            )
...
class TestMemoryLeakDetection:
    ...
    def test_worker_cleanup_on_failure(self):
        ...
        with WorkerPool(
            max_workers=1, worker_type=WorkerType.THREAD, enable_monitoring=True
        ) as pool:
            future = pool.submit_task(task, failing_executor)
