...
class TestJavaAnalyzerAgentComprehensive:
    ...
    def test_extract_mod_metadata_tool_source(self, agent, tmp_path):
        ...
        source_path.mkdir()
        (source_path / "fabric.mod.json").write_text(
            json.dumps({"id": "fabric_mod", "version": "1.1.0"})
        )
    ...
    def test_identify_features_tool_jar(self, agent, tmp_path):
        jar_path = tmp_path / "test_mod.jar"
        with zipfile.ZipFile(jar_path, "w") as jar:
            jar.writestr("fabric.mod.json", json.dumps({"id": "test_mod", "version": "1.0.0"}))
            jar.writestr(
                "com/example/MyBlock.java", b"package com.example; public class MyBlock {}"
            )
