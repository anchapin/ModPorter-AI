...
def mock_embedding_generator():
    ...
    gen.dimensions = 768
    gen.generate_embedding = AsyncMock(
        return_value=MagicMock(embedding=np.array([0.1] * 768, dtype=np.float32))
    )
    return gen
...
class TestEmbeddingGeneratorCreation:
    ...
    async def test_create_local_generator(self):
        ...
        with patch("utils.vector_db_client.LocalEmbeddingGenerator"):
            with patch("utils.vector_db_client.httpx.AsyncClient"):
                with patch("utils.vector_db_client.get_embedding_config") as mock_config:
...
class TestDocumentIndexing:
    ...
    async def test_index_document_success(self, sample_document):
        ...
        with patch("utils.vector_db_client.httpx.AsyncClient") as mock_client_class:
            with patch("utils.vector_db_client.get_embedding_config", return_value={}):
                with patch.object(
                    VectorDBClient, "_create_embedding_generator"
                ) as mock_gen_creator:
                    mock_gen = MagicMock()
                    ...
                    result = await client.index_document(
                        sample_document["content"], sample_document["source"]
                    )
            ...
            with patch("utils.vector_db_client.get_embedding_config", return_value={}):
                with patch.object(
                    ...
                    await client.search_documents(
                        "test query", top_k=5, document_source_filter="test_source"
                    )
        ...
        with patch("utils.vector_db_client.httpx.AsyncClient") as mock_client_class:
            with patch("utils.vector_db_client.get_embedding_config", return_value={}):
                with patch.object(
                    ...
                    mock_gen = MagicMock()
                    mock_gen.generate_embedding.return_value = MagicMock(embedding=test_embedding)
                    mock_gen.model_name = "test-model"
                    mock_gen.dimensions = 768
                    mock_gen_creator.return_value = mock_gen
        ...
        with patch("utils.vector_db_client.httpx.AsyncClient") as mock_client_class:
            with patch("utils.vector_db_client.get_embedding_config", return_value={}):
                with patch.object(
                    ...
                    await client.search_documents("test query")
