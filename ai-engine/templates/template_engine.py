...
class TemplateSelector:
    ...
    def _select_block_template(self, properties: Dict[str, Any]) -> TemplateType:
        ...
        if any(
            keyword in str(properties).lower()
            for keyword in ["inventory", "container", "chest", "storage"]
        ):
        ...
        if any(
            keyword in str(properties).lower()
            for keyword in ["interact", "click", "gui", "interface"]
        ):
        ...
        if any(
            keyword in str(properties).lower()
            for keyword in ["machine", "process", "energy", "power"]
        ):
        ...
        if any(
            keyword in str(properties).lower()
            for keyword in ["multiblock", "structure", "formation"]
        ):
    ...
    def _select_item_template(self, properties: Dict[str, Any]) -> TemplateType:
        ...
        if any(
            keyword in str(properties).lower()
            for keyword in ["tool", "pickaxe", "axe", "shovel", "hoe"]
        ):
        ...
        if any(
            keyword in str(properties).lower()
            for keyword in ["weapon", "sword", "damage", "attack"]
        ):
            # Check for ranged weapon
            if any(
                keyword in str(properties).lower()
                for keyword in ["bow", "crossbow", "gun", "ranged"]
            ):
        ...
        if any(
            keyword in str(properties).lower()
            for keyword in ["armor", "helmet", "chestplate", "leggings", "boots"]
        ):
        ...
        if any(
            keyword in str(properties).lower()
            for keyword in ["food", "consumable", "potion", "drink", "eat"]
        ):
    ...
    def _select_entity_template(self, properties: Dict[str, Any]) -> TemplateType:
        ...
        if any(
            keyword in str(properties).lower()
            for keyword in ["hostile", "aggressive", "attack", "damage"]
        ):
        ...
        if any(
            keyword in str(properties).lower()
            for keyword in ["npc", "villager", "trader", "merchant"]
        ):
        ...
        if any(
            keyword in str(properties).lower()
            for keyword in ["projectile", "arrow", "bullet", "thrown"]
        ):
...
class TemplateEngine:
    ...
    def _get_template_by_name(self, template_name: str) -> BaseTemplate:
        ...
        return JinjaTemplate(
            template_path, TemplateType.BASIC_BLOCK, self.jinja_env, self.templates_dir
        )
    ...
    def _get_template_category(self, template_type: TemplateType) -> TemplateCategory:
        ...
        elif template_type.value.endswith("_item") or template_type.value in [
            "tool",
            "weapon",
            "armor",
            "consumable",
