...
class BedrockScraperTool(BaseTool):
    ...
    description: str = (
        "A tool to scrape comprehensive Bedrock Edition documentation from official sources."
    )
    ...
    def _scrape_specific_site(self, site_url: str, params: Dict) -> str:
        ...
        try:
            ...
            return json.dumps(
                {"error": f"Site scraping failed: {str(e)}", "site_url": site_url, "success": False}
            )
    ...
    def _extract_api_examples(self) -> str:
        ...
        try:
            ...
            return json.dumps(
                {"error": f"API example extraction failed: {str(e)}", "success": False}
            )
    ...
    def _process_json_schemas(self) -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Schema processing failed: {str(e)}", "success": False})

    async def _async_scrape_all(self, params: Dict):
        """Async wrapper for full documentation scraping."""
        scraper = BedrockDocsScraper(
            rate_limit_seconds=self.rate_limit, cache_ttl_seconds=params.get("cache_ttl", 86400)
        )
