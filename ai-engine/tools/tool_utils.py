...
class ToolRegistry:
    ...
    def get_tools_by_category(self, category: str) -> List[Any]:
        ...
        for tool_name, tool_info in self._registered_tools.items():
            ...
            if (
                category.lower() in tool_name.lower()
                or category.lower() in tool_info.get("description", "").lower()
            ):
                tool = self.get_tool_by_name(tool_name)
                if tool:
                ...
                for class_info in tool_info["classes"]:
                    ...
                    if not issubclass(tool_class, BaseTool):
                        validation_result["errors"].append(
                            f"Class {class_info['name']} is not a BaseTool subclass"
                        )
                        validation_result["valid"] = False
                    ...
                    for attr in required_attrs:
                        if not hasattr(tool_class, attr):
                            validation_result["warnings"].append(
                                f"Class {class_info['name']} missing '{attr}' attribute"
                            )
