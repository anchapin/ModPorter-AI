    ...
    try:
        ...
        if response.sources:
            ...
            for i, source in enumerate(response.sources[:3], 1):
                logger.info(
                    f"  {i}. {source.document.source_path} (score: {source.final_score:.2f})"
                )
...
async def demo_query_expansion():
    ...
    try:
        ...
        logger.info(
            f"Expansion confidence: {expansion_metadata.get('expansion_confidence', 0.0):.2f}"
        )
...
async def demo_contextual_awareness():
    ...
    try:
        ...
        await rag_agent.query(query_text="What are Minecraft blocks?", session_id=session_id)
        ...
        response2 = await rag_agent.query(
            query_text="How do I create custom ones?", session_id=session_id
        )
...
async def demo_agent_capabilities():
    ...
    try:
        rag_agent = AdvancedRAGAgent(
            enable_query_expansion=True, enable_reranking=True, enable_multimodal=False
        )
