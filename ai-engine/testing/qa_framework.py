...
class TestScenarioGenerator:
    ...
    def generate_dynamic_scenarios(
        self, base_scenario: Dict[str, Any], variations: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(
            f"Generated {len(generated_scenarios)} dynamic scenarios for base: {base_scenario.get('name')}"
        )
...
class TestFramework:
    ...
    def collect_results(
        ...
        success: bool,
        details_from_execution: str,
        execution_time_ms: int,
        error_message_arg: Optional[str] = None,
...
if __name__ == "__main__":
    ...
    dummy_data = {
        "scenarios": [
            ...
            },
            {
                ...
                "steps": [
                    "Acquire ingredient A",
                    "Acquire ingredient B",
                    "Open crafting table",
                    "Place ingredients in pattern",
                    "Retrieve crafted item",
                ],
    ...
    if loaded_via_generator:
        base_functional_scenario = next(
            (s for s in loaded_via_generator if s["category"] == "functional"), None
        )
        if base_functional_scenario:
            dynamic_functional = scenario_gen.generate_dynamic_scenarios(
                base_functional_scenario, 2
            )
    ...
    loaded_scenarios = framework.load_scenarios(
        dummy_scenarios_path
    )  # This line was already here, kept for original flow
