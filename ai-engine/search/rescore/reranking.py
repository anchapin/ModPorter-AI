...
class ReRankingEngine:
    ...
    def _semantic_rerank(
        ...
        reranked = sorted(
            results, key=lambda x: x.metadata.get("semantic_score", 0.0), reverse=True
        )
    ...
    def _keyword_rerank(
        ...
        reranked = sorted(results, key=lambda x: x.metadata.get("keyword_score", 0.0), reverse=True)
        return self._apply_diversity(reranked)[: self.config.top_k]
    ...
    def _hybrid_rerank(self, query: SearchQuery, results: List[SearchResult]) -> List[SearchResult]:
        ...
        for result in results:
            ...
            combined = self.config.weight_semantic * semantic + self.config.weight_keyword * keyword
            result.metadata["hybrid_score"] = combined

        # Sort by combined score
        reranked = sorted(results, key=lambda x: x.metadata.get("hybrid_score", 0.0), reverse=True)
        return self._apply_diversity(reranked)[: self.config.top_k]
    ...
    def _cross_encoder_rerank(
        ...
        print(f"Cross-encoder re-ranking with model: {self.config.cross_encoder_model}")
    ...
    def _apply_diversity(self, results: List[SearchResult]) -> List[SearchResult]:
        ...
        for result in results:
            ...
            if category in used_categories and used_categories[category] > 0:
                ...
                current_score = result.metadata.get(
...
class EnsembleReRanker:
    ...
    def rerank(self, query: SearchQuery, results: List[SearchResult]) -> List[SearchResult]:
        ...
        return sorted(results, key=lambda x: x.metadata.get("ensemble_score", 0.0), reverse=True)[
