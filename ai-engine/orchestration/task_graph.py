...
class TaskNode:
    ...
    def mark_failed(self, error: str):
        ...
        duration_str = f"{self.duration:.2f}s" if self.duration is not None else "unknown duration"
        logger.error(
            f"Task {self.task_id} ({self.agent_name}) failed after {duration_str}: {error}"
        )
...
class TaskGraph:
    ...
    def get_completion_stats(self) -> Dict[str, Any]:
        ...
        completed_tasks = sum(
            1 for task in self.nodes.values() if task.status == TaskStatus.COMPLETED
        )
        ...
        total_duration = sum(
            task.duration for task in self.nodes.values() if task.duration is not None
        )
