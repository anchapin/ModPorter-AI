...
async def main():
    ...
    try:
        ...
        for i, doc in enumerate(scraped_documents):
            logger.info(
                f"Processing document {i + 1}/{len(scraped_documents)}: {doc.source} (Type: {doc.doc_type})"
            )
            ...
            if not doc.content or not isinstance(doc.content, str):
                logger.warning(
                    f"Document {doc.source} has no content or content is not a string. Skipping."
                )
                continue

            text_chunks = Chunker.chunk_document(
                document=doc.content, chunk_size=CHUNK_SIZE, overlap=CHUNK_OVERLAP
            )
            ...
            for chunk_index, chunk_content in enumerate(text_chunks):
                ...
                logger.debug(
                    f"Indexing chunk {chunk_index + 1}/{len(text_chunks)} from {doc.source}..."
                )
            ...
            logger.info(
                f"Indexed {doc_chunks_indexed}/{len(text_chunks)} chunks for document {doc.source}."
            )

        logger.info("Knowledge base population completed.")
        logger.info(
            f"Successfully processed and attempted indexing for {successfully_indexed_docs}/{len(scraped_documents)} documents."
        )
        logger.info(f"Total chunks indexed: {total_chunks_indexed}")

    except Exception as e:
        logger.error(
            f"An error occurred during the knowledge base population pipeline: {e}", exc_info=True
        )
    finally:
