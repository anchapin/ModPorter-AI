...
class RetrievalMetrics:
    ...
    def normalized_discounted_cumulative_gain(
        ...
        ideal_order = sorted(
            relevant_docs, key=lambda x: relevance_scores.get(x, 0.0), reverse=True
        )
        idcg = 0.0
...
class GenerationMetrics:
    ...
    def keyword_coverage(answer: str, required_keywords: List[str]) -> float:
        ...
        covered_keywords = sum(
            1 for keyword in required_keywords if keyword.lower() in answer_lower
        )
    ...
    def keyword_prohibition_compliance(answer: str, prohibited_keywords: List[str]) -> float:
        ...
        answer_lower = answer.lower()
        violated_keywords = sum(
            1 for keyword in prohibited_keywords if keyword.lower() in answer_lower
        )
...
class RAGEvaluator:
    ...
    async def evaluate_single_query(
        self, rag_agent, golden_item: GoldenDatasetItem
    ) -> EvaluationResult:
        """
        ...
        try:
            ...
            response = await rag_agent.query(
                ...
                content_types=[ContentType(ct) for ct in golden_item.content_types]
                if golden_item.content_types
                else None,
            ...
            metrics["keyword_prohibition_compliance"] = (
                GenerationMetrics.keyword_prohibition_compliance(
                    response.answer, golden_item.prohibited_keywords
                )
            )
            metrics["answer_length_appropriateness"] = (
                GenerationMetrics.answer_length_appropriateness(
            ...
            # Diversity metrics
            metrics["content_type_diversity"] = DiversityMetrics.content_type_diversity(
                response.sources
            )
    ...
    async def evaluate_full_dataset(
        self, rag_agent, dataset_items: Optional[List[GoldenDatasetItem]] = None
    ) -> Dict[str, Any]:
        """
        ...
        for item in dataset_items:
            try:
                ...
                logger.info(
                    f"Evaluated query {item.query_id}: {len(result.passed_tests)} passed, {len(result.failed_tests)} failed"
                )
            except Exception as e:
        ...
        logger.info(
            f"Full dataset evaluation completed. Overall score: {report.get('overall_score', 0.0):.3f}"
        )
    ...
    def _compile_evaluation_report(self, results: List[EvaluationResult]) -> Dict[str, Any]:
        ...
        generation_metrics = [
            "keyword_coverage",
            "keyword_prohibition_compliance",
        ...
        for result in successful_evaluations:
            ...
            golden_item = next(
                (item for item in self.golden_dataset if item.query_id == result.query_id), None
            )
        ...
        report = {
            ...
            "performance_breakdown": {
                "by_query_type": {
                    qtype: np.mean(scores) for qtype, scores in query_type_performance.items()
                },
                "by_difficulty": {
                    diff: np.mean(scores) for diff, scores in difficulty_performance.items()
                },
                "by_domain": {
            ...
            "recommendations": self._generate_recommendations(
                metric_summaries, test_pass_rates, category_scores
            ),
        }
    ...
    def _generate_recommendations(
        ...
        if category_scores.get("retrieval", 0.0) < 0.6:
            recommendations.append(
                "Consider improving retrieval quality: precision and recall are below optimal levels"
            )

            if metric_summaries.get("precision_at_5", {}).get("mean", 0.0) < 0.5:
                recommendations.append(
                    "Low precision: Review relevance scoring and ranking algorithms"
                )

            if metric_summaries.get("recall_at_5", {}).get("mean", 0.0) < 0.5:
                recommendations.append(
                    "Low recall: Consider expanding query or improving document coverage"
                )
        ...
        if test_pass_rates.get("confidence_threshold", 0.0) < 0.7:
            recommendations.append(
                "Low confidence scores: Review confidence calculation and thresholds"
            )

        if not recommendations:
            recommendations.append(
                "System performance is within acceptable ranges across all categories"
            )
