...
class MultiModalEmbeddingGenerator:
    ...
    def setup_models(self):
        ...
        try:
            ...
            self.models["text"] = EmbeddingGenerator(
        ...
        try:
            ...
            if document.content_type == ContentType.TEXT:
                if document.content_text:
                    vector = await self.embedding_generator.generate_text_embedding(
                ...
                content = {"text": document.content_text, "metadata": document.content_metadata}
        ...
        for doc_id, document in self.documents.items():
            ...
            for embedding in doc_embeddings:
                ...
                if len(query_vector) == len(embedding.embedding_vector):
                    ...
                    logger.debug(
                        f"Dimension mismatch: query={len(query_vector)}, doc={len(embedding.embedding_vector)}"
                    )
            ...
            if max_vector_score == 0.0:
                ...
                final_score = (
                    config.vector_weight * max_vector_score + config.keyword_weight * keyword_score
                )
            ...
            if final_score >= query.similarity_threshold:
                result = SearchResult(
                    ...
                    embedding_model_used=best_embedding_model
                    or EmbeddingModel.SENTENCE_TRANSFORMER,
...
class AdvancedRAGPrototype:
    ...
    async def test_search_scenarios(self):
        ...
        for result in results4:
            logger.info(
                f"  - {result.document.id} ({result.document.content_type}): {result.final_score:.3f}"
            )
